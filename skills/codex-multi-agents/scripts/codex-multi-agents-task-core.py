...
MERGE_USERS = {"李白"}
AUTO_ASSIGNABLE_TASK_TYPES: set[str] = {
    "execute",
    "spec",
    "build",
    "review",
...
def plan_has_done_merge(done_file: str, plan_doc: str) -> bool:
    """判断 DONE.md 中是否已有该计划的 merge 完成记录。


    功能说明:
    - 读取 DONE.md 的 `备注` 列中由 `-done` 写入的任务类型与计划书元数据。
    - 只有 `任务类型=merge` 且 `计划书=<plan_doc>` 同时匹配时，才视为计划已完成 merge。

    使用示例:
    - ok = plan_has_done_merge("./DONE.md", "ARCHITECTURE/plan/demo.md")
    ...
    """
    if not done_file.strip() or not os.path.exists(done_file):
        return False
    _, done_table = parse_done_table(done_file)
    for row in done_table["rows"]:
        note = row[6].strip() if len(row) > 6 else ""
        if "任务类型=merge" in note and f"计划书={plan_doc}" in note:
            return True
    return False
...
def _is_plan_task_from_doc(plan_doc: str) -> bool:
    """判断任务行是否绑定计划书。


    功能说明:
    - 使用 TODO 表中的 `计划书` 字段判断任务是否为计划级任务。
    - 空字符串和 `None` 视为普通任务，其它值视为计划级任务。

    使用示例:
    - is_plan = _is_plan_task_from_doc("ARCHITECTURE/plan/demo.md")
...
def _ensure_next_transition_allowed(
    current_kind: str,
    next_kind: str,
    plan_doc: str,
    has_direct_assignee: bool,
    auto_flag: bool,
) -> None:
    """校验 `-next` 当前阶段到下一阶段的公开流转边界。


    功能说明:
    - 计划级任务按 `execute -> review -> archive_acceptance -> merge` 校验。
    - 普通任务按 `execute -> review -> merge` 校验。
    - `archive_acceptance` 只能由计划级 `review` 通过 `-to` 或 `-auto` 直接进入。
    - `merge` 与 `other` 没有公开 `-next` 当前阶段语义，必须默认拒绝。

    使用示例:
    - _ensure_next_transition_allowed("review", "archive_acceptance", "ARCHITECTURE/plan/demo.md", False, True)
    ...
    - 功能实现: skills/codex-multi-agents/scripts/codex-multi-agents-task-core.py
    """
    is_plan_task = _is_plan_task_from_doc(plan_doc)
    if current_kind == "merge":
        fail(RC_DATA, "task merge cannot use -next; complete with -done")
    if current_kind == "other":
        fail(RC_DATA, "task other cannot use -next; no transition is defined")

    if next_kind == "archive_acceptance":
        if not is_plan_task:
            fail(RC_DATA, "normal task cannot use archive_acceptance")
        if current_kind != "review":
            fail(RC_DATA, f"plan task {current_kind} cannot next to archive_acceptance")
        if not (has_direct_assignee or auto_flag):
            fail(RC_DATA, "plan task review to archive_acceptance requires -to or -auto")
        return

    if current_kind == "archive_acceptance":
        if not is_plan_task:
            fail(RC_DATA, "normal task cannot use archive_acceptance")
        if next_kind in {"merge", "execute"}:
            return
        fail(RC_DATA, "plan task archive_acceptance must next to merge or execute")

    execute_like_kinds = {"execute", "spec", "build", "refactor"}

    if is_plan_task and current_kind in execute_like_kinds:
        if next_kind == "review":
            return
        if next_kind == "merge":
            fail(RC_DATA, f"plan task {current_kind} cannot next to merge")
        fail(RC_DATA, f"plan task {current_kind} must next to review")

    if is_plan_task and current_kind == "review":
        if next_kind in {"archive_acceptance", "execute"}:
            return
        if next_kind == "merge":
            fail(RC_DATA, "plan task review must next to archive_acceptance or execute")
        fail(RC_DATA, "plan task review must next to archive_acceptance or execute")

    if not is_plan_task and current_kind in execute_like_kinds:
        if next_kind == "review":
            return
        if next_kind == "merge":
            fail(RC_DATA, f"normal task {current_kind} cannot next to merge")
        fail(RC_DATA, f"normal task {current_kind} must next to review")

    if not is_plan_task and current_kind == "review":
        if next_kind in {"merge", "execute"}:
            return
        fail(RC_DATA, "normal task review must next to merge or execute")

    fail(RC_DATA, f"task {current_kind} cannot use -next; no transition is defined")
...
def is_specialist_candidate(kind: str, duty: str) -> bool:
    ...
    if not agent_matches_type(kind, duty):
        return False
    if kind in {"execute", "spec", "build", "review", "archive_acceptance"} and is_substitute_duty(duty):
        return False
    return True
...
def is_assignment_allowed_for_task(kind: str, duty: str) -> bool:
    ...
    功能说明:
    - `merge` 仅允许合并专职。
    - `execute/spec/build/review/archive_acceptance/refactor` 允许对应专职，必要时允许全能替补。
    - `other` 保持无职责限制。
    ...
    if is_specialist_candidate(kind, duty):
        return True
    return kind in {"execute", "spec", "build", "review", "archive_acceptance", "refactor"} and is_substitute_duty(duty)
...
def pick_next_auto_assignee(
    ...
    功能说明:
    - 在专职候选集合中随机选择接手人。
    - 专职候选为空时，对 execute/spec/build/review/archive_acceptance/refactor 退到候补候选集合。
    ...
    for candidate_row in agents_rows:
        ...
        if is_specialist_candidate(kind, duty):
            ...
            continue
        if kind in {"execute", "spec", "build", "review", "archive_acceptance", "refactor"} and is_substitute_duty(duty):
...
def pick_ready_task_auto_dispatch(
    ...
    - 仅对 execute/spec/build/review/archive_acceptance/merge/refactor 七类任务执行自动启动。
...
def main() -> int:
    ...
    if op == "dispatch":
        ...
        if normalized_task_type == "archive_acceptance":
            fail(RC_DATA, "archive_acceptance tasks can only be entered through plan task -next")
        ...
        done_note = f"任务类型={row[5].strip()}；计划书={row[7].strip()}"
        done_rows.append([row[0], row[4], assignee, "已完成", finished_at, log_file, done_note])
        ...
        row = exec_rows[idx]
        current_kind = normalize_task_type(row[5], RC_DATA, f"running task {task_id}")
        next_kind = normalize_task_type(type_kind, RC_DATA, f"next task {task_id}")
        _ensure_next_transition_allowed(
            current_kind,
            next_kind,
            row[7],
            bool(to.strip()),
        ...
        row = exec_rows.pop(idx)
        assignee = row[8].strip()
        next_list_row = [row[0], row[1], row[2], row[3], message.strip(), next_kind, row[6], row[7], "", row[11]]
        ...
        normalized_task_type = normalize_task_type(type_kind, RC_DATA, "new task")
        if normalized_task_type == "archive_acceptance":
            fail(RC_DATA, "archive_acceptance tasks cannot be created with -new")
        ...
        list_rows.append([new_id, from_val, created_at, worktree_val, info, normalized_task_type, depends_val, plan_doc_val, to, record_file])
        ...
        if not plan_has_done_merge(done_file, normalized_plan_doc):
            fail(RC_DATA, f"plan is missing merge before done-plan: {normalized_plan_doc}")
