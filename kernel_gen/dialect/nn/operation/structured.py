...
class _NnStructuredHelpers :
    ...
    def is_int_or_symbol_type (attr :Attribute )->bool :
        ...
        return _NnStructuredHelpers .is_symbol_int_type (attr )or isinstance (attr ,IntegerType )
    ...
    def static_int_from_operand (operand :SSAValue )->int |None :
        ...
        if owner_name =="builtin.unrealized_conversion_cast":
            if owner .operands :
                return _NnStructuredHelpers .static_int_from_operand (owner .operands [0 ])
        return None
...
class _NnStructuredRules :
    ...
    使用示例:
    - _NN_STRUCTURED.is_symbol_expr_attr(...)
    """
    ...
    def is_symbol_expr_attr (attr :Attribute )->bool :
        ...
        使用示例:
        - _NN_STRUCTURED.is_symbol_expr_attr(SymbolExprAttr.from_expr("N"))
    ...
    def dim_expr_text (dim :Attribute )->str :
        ...
        使用示例:
        - _NN_STRUCTURED.dim_expr_text(SymbolExprAttr.from_expr("4"))
        ...
        if not _NN_STRUCTURED .is_symbol_expr_attr (dim ):
    ...
    def static_int_from_expr_text (expr :str )->int |None :
        ...
        使用示例:
        - _NN_STRUCTURED.static_int_from_expr_text("4")
    ...
    def static_int_from_dim (dim :Attribute )->int |None :
        ...
        使用示例:
        - _NN_STRUCTURED.static_int_from_dim(SymbolExprAttr.from_expr("4"))
        ...
        return _NN_STRUCTURED .static_int_from_expr_text (_NN_STRUCTURED .dim_expr_text (dim ))
    ...
    def collect_int_dims (dims :Sequence [Attribute ])->list [int ]|None :
        ...
        使用示例:
        - _NN_STRUCTURED.collect_int_dims([SymbolExprAttr.from_expr("1"), SymbolExprAttr.from_expr("2")])
        ...
        values :list [int ]=[]
        for dim in dims :
            value =_NN_STRUCTURED .static_int_from_dim (dim )
            if value is None :
                return None
            values .append (value )
        return values

_NN_STRUCTURED =_NnStructuredRules ()
...
value_error_phrase :str ,
    ...
    for operand in operands :
        if not _NnStructuredHelpers .is_int_or_symbol_type (operand .type ):
            core_raise_verify_error (_ERROR_SCENE ,type_error_phrase )
        static_value =_NnStructuredHelpers .static_int_from_operand (operand )
        if static_value is not None :
            if allow_zero :
                if static_value <0 :
                    core_raise_verify_error (_ERROR_SCENE ,value_error_phrase )
            elif static_value <=0 :
                core_raise_verify_error (_ERROR_SCENE ,value_error_phrase )
...
result_shape :Sequence [Attribute ],
    ...
    if len (lhs_shape )!=2 or len (rhs_shape )!=2 or len (result_shape )!=2 :
        core_raise_verify_error (_ERROR_SCENE ,"nn.matmul requires rank-2 memory types")

    if lhs_shape [1 ]!=rhs_shape [0 ]:
        core_raise_verify_error (_ERROR_SCENE ,"nn.matmul contracting dimensions must match")

    if result_shape [0 ]!=lhs_shape [0 ]or result_shape [1 ]!=rhs_shape [1 ]:
        core_raise_verify_error (_ERROR_SCENE ,"nn.matmul result shape must match lhs/rhs")
...
pad_after :int ,
    ...
    padded_input_dim = input_dim + pad_before + pad_after
    valid_extent = padded_input_dim - effective_kernel
...
class NnImg2col1dOp (IRDLOperation ):
    ...
    def verify_ (self )->None :
        ...
        if not isinstance (input_type ,NnMemoryType ):
            core_raise_verify_error (_ERROR_SCENE ,"operand-must-be-rank-3-nn-memory")
        if not isinstance (result_type ,NnMemoryType ):
            core_raise_verify_error (_ERROR_SCENE ,"result-rank-must-be-4")
        input_type .verify ()
        result_type .verify ()
        ...
        if input_type .space .space .data !=self .space .space .data :
            core_raise_verify_error (_ERROR_SCENE ,"result-space-matches-input")
        if result_type .space .space .data !=input_type .space .space .data :
            core_raise_verify_error (_ERROR_SCENE ,"result-space-matches-input")
        if result_type .element_type !=input_type .element_type :
            core_raise_verify_error (_ERROR_SCENE ,"result-element-type-matches-input")

        input_dims =_NN_STRUCTURED .collect_int_dims (input_type .shape .data )
        ...
        if w_out <=0 :
            core_raise_verify_error (_ERROR_SCENE ,"result-shape-stride-must-match-img2col1d-contract")
        ...
        for actual_dim ,expected_dim in zip (result_type .shape .data ,expected_shape ,strict =True ):
            if _NN_STRUCTURED .static_int_from_dim (actual_dim )!=expected_dim :
                core_raise_verify_error (_ERROR_SCENE ,"result-shape-stride-must-match-img2col1d-contract")

        result_strides =_NN_STRUCTURED .collect_int_dims (result_type .stride .data )
        if result_strides is None :
            core_raise_verify_error (_ERROR_SCENE ,"result-shape-stride-must-match-img2col1d-contract")
        expected_stride =_NnStructuredHelpers .build_contiguous_stride (expected_shape )
...
class NnImg2col2dOp (IRDLOperation ):
    ...
    def verify_ (self )->None :
        ...
        result_type =self .result .type
        if not isinstance (input_type ,NnMemoryType ):
            core_raise_verify_error (_ERROR_SCENE ,"operand-must-be-rank-4-nn-memory")
        if not isinstance (result_type ,NnMemoryType ):
            core_raise_verify_error (_ERROR_SCENE ,"result-rank-must-be-6")
        input_type .verify ()
        result_type .verify ()
        ...
        w_out =_img2col_output_dim (w_dim ,kw_value ,sw_value ,dw_value ,pl_value ,pr_value )
        if h_out <=0 :
            core_raise_verify_error (_ERROR_SCENE ,"result-shape-stride-must-match-img2col2d-contract")
        if w_out <=0 :
            core_raise_verify_error (_ERROR_SCENE ,"result-shape-stride-must-match-img2col2d-contract")
...
class NnMatmulOp (IRDLOperation ):
    ...
    def verify_ (self )->None :
        ...
        """
        lhs_type =_NnStructuredHelpers .verify_memory_type (self .lhs .type ,"lhs")
        rhs_type =_NnStructuredHelpers .verify_memory_type (self .rhs .type ,"rhs")
        result_type =_NnStructuredHelpers .verify_memory_type (self .result .type ,"result")

        self .space .verify ()
        if lhs_type .space .space .data !=rhs_type .space .space .data :
            core_raise_verify_error (_ERROR_SCENE ,"nn.matmul operands must use the same space")
        if lhs_type .space .space .data !=self .space .space .data :
            core_raise_verify_error (_ERROR_SCENE ,"nn.matmul attribute space must match operand space")
        if result_type .space .space .data !=self .space .space .data :
            core_raise_verify_error (_ERROR_SCENE ,"nn.matmul attribute space must match result space")
        ...
        if lhs_type .element_type !=rhs_type .element_type or lhs_type .element_type !=result_type .element_type :
            core_raise_verify_error (_ERROR_SCENE ,"nn.matmul operand/result element_type must match")
