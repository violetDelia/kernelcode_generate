...
from kernel_gen.dialect.nn.common import dims_equal, raise_verify_error, verify_memory_type
...
def _verify_broadcast_compat(input_type: NnMemoryType, result_type: NnMemoryType) -> None:
    ...
    if len(result_dims) < len(input_dims):
        raise_verify_error("result-rank-must-be-greater-or-equal-to-input")

    for input_dim, result_dim in zip(reversed(input_dims), reversed(result_dims), strict=False):
        if dims_equal(input_dim, result_dim):
            continue
        if _static_int_from_dim(input_dim) == 1:
            continue
        raise_verify_error("result-shape-must-match-broadcast-contract")
...
def _verify_transpose_perm(perm: ArrayAttr, rank: int) -> list[int]:
    ...
    if len(perm.data) != rank:
        raise_verify_error("nn.transpose perm must match input rank")
    ...
    for entry in perm.data:
        ...
        if isinstance(entry, IntegerAttr) and isinstance(entry.value, IntAttr):
            ...
            continue
        raise_verify_error("nn.transpose perm must be a permutation of 0..rank-1")
...
def _verify_transpose_layout(
    ...
    for expected_dim, actual_dim in zip(expected_shape, result_type.shape.data, strict=True):
        if not dims_equal(expected_dim, actual_dim):
            raise_verify_error("nn.transpose result shape must match permuted input")
    ...
    for expected_dim, actual_dim in zip(expected_stride, result_type.stride.data, strict=True):
        if not dims_equal(expected_dim, actual_dim):
            raise_verify_error("nn.transpose result stride must be contiguous")
...
def _verify_select_op(op: "NnSelectOp") -> None:
    ...
    cond_type = verify_memory_type(op.cond.type, "cond")
    ...
    if cond_type.element_type != i1:
        raise_verify_error("nn.select cond element_type must be i1")

    op.space.verify()
    if lhs_type.space.space.data != rhs_type.space.space.data:
        raise_verify_error("nn.select operands must use the same space")
    if lhs_type.space.space.data != op.space.space.data:
        raise_verify_error("nn.select attribute space must match operand space")
    if result_type.space.space.data != op.space.space.data:
        raise_verify_error("nn.select attribute space must match result space")

    if lhs_type.shape != rhs_type.shape or lhs_type.shape != result_type.shape:
        raise_verify_error("nn.select shape must match across operands and result")
    if lhs_type.stride != rhs_type.stride or lhs_type.stride != result_type.stride:
        raise_verify_error("nn.select stride must match across operands and result")
    if lhs_type.element_type != rhs_type.element_type:
        raise_verify_error("nn.select operand element_type must match")
    if result_type.element_type != lhs_type.element_type:
        raise_verify_error("nn.select result element_type must match operand element_type")
...
def _verify_cast_op(op: "NnCastOp") -> None:
    ...
    if input_type.space.space.data != op.space.space.data:
        raise_verify_error("nn.cast attribute space must match operand space")
    if result_type.space.space.data != op.space.space.data:
        raise_verify_error("nn.cast attribute space must match result space")

    if input_type.shape != result_type.shape:
        raise_verify_error("nn.cast shape must match input")
    if input_type.stride != result_type.stride:
        raise_verify_error("nn.cast stride must match input")
...
class NnTransposeOp(IRDLOperation):
    ...
    def verify_(self) -> None:
        ...
        self.space.verify()
        if input_type.space.space.data != result_type.space.space.data:
            raise_verify_error("nn.transpose input/result must use the same space")
        if input_type.space.space.data != self.space.space.data:
            raise_verify_error("nn.transpose attribute space must match type space")

        if input_type.element_type != result_type.element_type:
            raise_verify_error("nn.transpose element_type must match")
