...
def _verify_binary_memory_op(op: "_BaseNnBinaryOp", compare_result: bool) -> None:
    ...
    lhs_type = verify_memory_type(op.lhs.type, "lhs")
    rhs_type = verify_memory_type(op.rhs.type, "rhs")
    result_type = verify_memory_type(op.result.type, "result")

    op.space.verify()
    if lhs_type.space.space.data != rhs_type.space.space.data:
        raise_verify_error("nn op operands must use the same space")
    if lhs_type.space.space.data != op.space.space.data:
        raise_verify_error("nn op attribute space must match operand space")
    if result_type.space.space.data != op.space.space.data:
        raise_verify_error("nn op attribute space must match result space")

    if lhs_type.shape != rhs_type.shape or lhs_type.shape != result_type.shape:
        raise_verify_error("nn op shape must match across operands and result")
    if lhs_type.stride != rhs_type.stride or lhs_type.stride != result_type.stride:
        raise_verify_error("nn op stride must match across operands and result")
    if lhs_type.element_type != rhs_type.element_type:
        raise_verify_error("nn op operand element_type must match")

    if compare_result:
        if result_type.element_type != i1:
            raise_verify_error("nn compare result element_type must be i1")
    elif result_type.element_type != lhs_type.element_type:
        raise_verify_error("nn arithmetic result element_type must match operand element_type")
...
def _verify_add_op(op: "NnAddOp") -> None:
    ...
    if not lhs_is_memory and not rhs_is_memory:
        raise_verify_error("nn.add requires at least one nn.memory operand")
    ...
    memory_type = verify_memory_type(lhs_type if lhs_is_memory else rhs_type, "memory operand")
    if memory_type.space.space.data != op.space.space.data:
        raise_verify_error("nn.add attribute space must match memory operand space")
    if result_type.space.space.data != memory_type.space.space.data:
        raise_verify_error("nn.add result space must match memory operand")

    if result_type.shape != memory_type.shape:
        raise_verify_error("nn.add result shape must match memory operand")
    if result_type.stride != memory_type.stride:
        raise_verify_error("nn.add result stride must match memory operand")
    ...
    if promoted_type is None:
        raise_verify_error("nn.add scalar element_type must be i32/f16/f32 or symbol.int")
    if result_type.element_type != promoted_type:
        raise_verify_error("nn.add result element_type must match promoted element_type")
...
def _verify_mixed_scalar_binary_op(op: "_BaseNnBinaryOp", op_name: str) -> None:
    ...
    rhs_is_memory = isinstance(rhs_type, NnMemoryType)
    if not lhs_is_memory and not rhs_is_memory:
        raise_verify_error(f"{op_name} requires at least one nn.memory operand")
    ...
    memory_type = verify_memory_type(lhs_type if lhs_is_memory else rhs_type, "memory operand")
    if memory_type.space.space.data != op.space.space.data:
        raise_verify_error(f"{op_name} attribute space must match memory operand space")
    if result_type.space.space.data != memory_type.space.space.data:
        raise_verify_error(f"{op_name} result space must match memory operand")

    if result_type.shape != memory_type.shape:
        raise_verify_error(f"{op_name} result shape must match memory operand")
    if result_type.stride != memory_type.stride:
        raise_verify_error(f"{op_name} result stride must match memory operand")

    scalar_type = rhs_type if lhs_is_memory else lhs_type
    promoted_type = _promote_add_dtype(memory_type.element_type, scalar_type)
    if promoted_type is None:
        raise_verify_error(f"{op_name} scalar element_type must be i32/f16/f32 or symbol.int")
    if result_type.element_type != promoted_type:
        raise_verify_error(f"{op_name} result element_type must match promoted element_type")
