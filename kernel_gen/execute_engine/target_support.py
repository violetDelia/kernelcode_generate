"""ExecuteEngine builtin target support implementation.


功能说明:
- 承接执行引擎内置 target 的 include 注入、entry shim、编译单元生成和真实编译支持。
- 只返回纯标准库字段组成的编译产物描述，供 `compiler.py` 装配既有执行引擎公开入口。
- 不参与 `kernel_gen.execute_engine` 包根导出，也不承接 strategy registry 或执行期 ABI。

API 列表:
- `class BuiltinTargetSupportArtifacts(soname_path: str, source_path: str, command: tuple[str, ...], stdout: str, stderr: str, return_code: int, allow_absent_memory_arg_specs: tuple[tuple[int, str, int], ...] = (), cleanup: Callable[[], None] | None = None)`
- `build_builtin_target_support_artifacts(request: _TargetSupportRequest) -> BuiltinTargetSupportArtifacts`

使用示例:
- from kernel_gen.execute_engine.target_support import build_builtin_target_support_artifacts
...
class _TargetSupportRequest(Protocol):
    """内置 target 支持所需的请求字段协议。

    功能说明:
    - 用最小字段协议避免本模块运行期反向导入执行引擎主入口文件。
    - 调用方只需提供源码、target、函数名、entry 名和编译选项。
...
class BuiltinTargetSupportArtifacts:
    """内置 target 编译产物描述。
    ...
    使用示例:
    - artifacts = BuiltinTargetSupportArtifacts("libkernel.so", "kernel.cpp", ("g++",), "", "", 0)
    """
...
def _target_support_error(failure_phrase: str, detail: str = "") -> KernelCodeError:
    """构造 execute_engine target 支持错误对象。

    功能说明:
    - 复用执行引擎公开失败短语，保证拆分后错误语义不变。
    - 错误对象继续归属 `EXECUTE_ENGINE` 模块。

    使用示例:
    - err = _target_support_error(_COMPILE_FAILED, "compiler is empty")
    """
    ...
    *,
    ...
    try:
        ...
        if dry_run:
            ...
            return BuiltinTargetSupportArtifacts(
...
def _dtype_code_from_name(dtype: str | None) -> int:
    ...
    功能说明:
    - 仅为 template shim 和 allow-absent metadata 校验提供最小 dtype 编码。
    - 未识别 dtype 返回 0。
...
def _extract_allow_absent_memory_arg_specs(source: str | None) -> tuple[tuple[int, str, int], ...]:
    ...
    for match in _ALLOW_ABSENT_MEMORY_ARGS_PATTERN.finditer(source):
        ...
        for item in body.split(";"):
            ...
            parts = text.split(":")
            if len(parts) != 3:
                raise _target_support_error(_RUNTIME_THROW_OR_ABORT, "invalid allow-absent memory metadata")
            index_text, dtype_text, rank_text = parts
            try:
                index = int(index_text)
                rank = int(rank_text)
            except ValueError as exc:
                raise _target_support_error(_RUNTIME_THROW_OR_ABORT, "invalid allow-absent memory metadata") from exc
...
def _allow_absent_memory_arg_map(metadata: tuple[tuple[int, str, int], ...]) -> dict[int, tuple[int, str, int]]:
...
def _runtime_param_declaration_lines(
    ...
    runtime_arg_index = 0
    allow_absent_map = _allow_absent_memory_arg_map(allow_absent_memory_arg_specs)
    for spec in params:
...
def _resolve_compiler_name(compiler: str | None) -> str:
    ...
    if not isinstance(compiler, str) or not compiler.strip():
        raise _target_support_error(_COMPILE_FAILED, "compiler is empty")
    return compiler


def build_builtin_target_support_artifacts(request: _TargetSupportRequest) -> BuiltinTargetSupportArtifacts:
    """生成内置 target 编译产物。
    ...
    if source is None or not isinstance(source, str) or not source.strip():
        raise _target_support_error(_SOURCE_EMPTY_OR_INVALID, "source is empty")
    include_family = _source_include_family(source)
    if include_family == "mixed":
        raise _target_support_error(_TARGET_HEADER_MISMATCH, "source includes mixed target include families")
    ...
    if "#error" in source:
        raise _target_support_error(_COMPILE_FAILED, "source contains #error directive")
    if function is None or not isinstance(function, str) or not function.strip():
        raise _target_support_error(_SYMBOL_RESOLVE_FAILED, "function is empty")
    if not isinstance(entry_point, str) or not entry_point.strip():
        raise _target_support_error(_SYMBOL_RESOLVE_FAILED, "entry_point is empty")
    ...
    if not target_headers:
        raise _target_support_error(_TARGET_HEADER_MISMATCH, f"unsupported target: {target}")
    ...
    try:
        if artifacts.return_code != 0:
            raise _target_support_error(_COMPILE_FAILED, f"compiler returned non-zero ({artifacts.return_code})")
        if not Path(artifacts.soname_path).exists():
            raise _target_support_error(_COMPILE_FAILED, "compile output is missing")
    except Exception:
