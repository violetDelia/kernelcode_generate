"""CUDA SM86 builtin compile strategy implementation.
...
API 列表:
- `build_cuda_sm86_compile_artifacts(request: "CompileRequest", compiler: str, compiler_flags: tuple[str, ...], link_flags: tuple[str, ...]) -> BuiltinCompileArtifacts`

使用示例:
- artifacts = build_cuda_sm86_compile_artifacts(request, "nvcc", ("-std=c++17",), ())

关联文件:
- spec: spec/execute_engine/strategy.md
- spec: spec/execute_engine/execute_engine_target.md
- test: test/execute_engine/test_cuda_sm86_strategy.py
...
def build_cuda_sm86_compile_artifacts(
    request: "CompileRequest",
    compiler: str,
    compiler_flags: tuple[str, ...],
    link_flags: tuple[str, ...],
) -> BuiltinCompileArtifacts:
    """生成 CUDA SM86 target 编译产物。

    功能说明:
    - 解析公开 SourceBundle aggregate，写出真实 `.cu/.cuh` artifact。
    - 使用 `nvcc -arch=sm_86 -shared -Xcompiler -fPIC` 编译为 `.so`，并返回 slot C ABI 编译结果。
    ...
    try:
        ...
        if lines and lines[0].startswith(BUNDLE_MARKER_PREFIX):
            ...
            compile_unit = BuiltinStrategySupport.compose_compile_unit(
                ...
                include_lines_for_target=BuiltinStrategySupport.include_lines_for_target("cuda_sm86"),
                entry_shim_source="",
            )
        ...
        return BuiltinCompileArtifacts(
            ...
            command=command,
            stdout=result.stdout or f"cuda-sm86 compile: {' '.join(command)}",
