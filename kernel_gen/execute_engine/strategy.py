...
def _strategy_error(failure_phrase: str, detail: str = "") -> KernelCodeError:
    ...
    使用示例:
    - err = _strategy_error("compile_failed", "duplicate compile strategy: cpu")
    """
...
def _validate_strategy_target(target: str) -> None:
    ...
    if not isinstance(target, str) or not _TARGET_NAME_PATTERN.fullmatch(target):
        raise _strategy_error(_TARGET_HEADER_MISMATCH, "invalid target name")
    try:
        target_registry.get_target_hardware(target, "__compile_strategy_probe__")
    except ValueError as exc:
        raise _strategy_error(_TARGET_HEADER_MISMATCH, f"target not registered: {target}") from exc
...
def register_compile_strategy(target: str, strategy: CompileStrategy, *, override: bool = False) -> None:
    ...
    if not isinstance(strategy, CompileStrategy):
        raise _strategy_error(_COMPILE_FAILED, "compile strategy must define compile")
    if target in _COMPILE_STRATEGIES and not override:
        raise _strategy_error(_COMPILE_FAILED, f"duplicate compile strategy: {target}")
...
def get_compile_strategy(target: str) -> CompileStrategy:
    ...
    if strategy is None:
        raise _strategy_error(_TARGET_HEADER_MISMATCH, f"missing compile strategy: {target}")
    return strategy
