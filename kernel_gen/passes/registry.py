...
- 内置 pipeline 包含 `cuda-sm89-lowering`，用于 CUDA SM89 Tensor Core 后端 lowering。
...
def load_builtin_passes() -> None:
    ...
    - 当前内置 pipeline 包含 `default-lowering`、`npu-demo-lowering` 与 `cuda-sm89-lowering`。
    ...
    if "cuda-sm89-lowering" not in _PIPELINE_REGISTRY:
        register_pipeline("cuda-sm89-lowering")(build_cuda_sm89_lowering_pipeline)
