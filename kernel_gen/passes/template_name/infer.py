"""Template-name inference pass implementation.
...
from kernel_gen.passes.template_name.constraints import build_template_constraints
from kernel_gen.passes.template_name.default_constraints import register_default_template_constraints
from kernel_gen.passes.template_name.graph import Same, TemplateNameGraph, TemplateNameSolution, TemplateNameValue
...
def _is_entry_point_func(func_op: func.FuncOp) -> bool:
    """判断函数是否为 entry_point host。

    功能说明:
    - 只检查 `func.func` 公开 attributes 中是否存在 `entry_point` 标记。
...
def _is_entry_point_pattern_func(host_name: str, func_op: func.FuncOp) -> bool:
    """判断函数是否为指定 host 的 pattern 函数。

    功能说明:
    - pattern 函数名必须以 `<host>_pattern` 开头。
    - 函数必须携带 `kernel.transform_pipeline` 属性，避免普通同名前缀函数被误连。

    使用示例:
    - if _is_entry_point_pattern_func("entry", func_op): ...
    """

    return func_op.sym_name.data.startswith(f"{host_name}_pattern") and "kernel.transform_pipeline" in func_op.attributes


def _same_position_memory_arg_constraints(
    source: func.FuncOp,
    target: func.FuncOp,
    *,
    error_prefix: str,
) -> tuple[Same, ...]:
    """按函数参数位置构造 memory template 等价约束。

    功能说明:
    - 只连接两侧同位置 `NnMemoryType` 参数。
    - 一侧是 memory 而另一侧不是 memory 时稳定失败，避免生成不完整模板族。

    使用示例:
    - constraints = _same_position_memory_arg_constraints(host, pattern, error_prefix="entry_point pattern")
    """

    source_args = tuple(source.args)
    target_args = tuple(target.args)
    if len(source_args) != len(target_args):
        raise _template_infer_error(f"{error_prefix} arg count must match host args")
    constraints: list[Same] = []
    for index, (source_arg, target_arg) in enumerate(zip(source_args, target_args, strict=True)):
        if not isinstance(source_arg.type, NnMemoryType) and not isinstance(target_arg.type, NnMemoryType):
            continue
        if not isinstance(source_arg.type, NnMemoryType) or not isinstance(target_arg.type, NnMemoryType):
            raise _template_infer_error(f"{error_prefix} memory arg type must match host arg type")
        constraints.append(
            Same(
                TemplateNameValue(source_arg, source, "block_arg", index),
                TemplateNameValue(target_arg, target, "block_arg", index),
            )
        )
    return tuple(constraints)


def _entry_point_pattern_template_constraints(funcs: dict[str, func.FuncOp]) -> tuple[Same, ...]:
    """构造 entry_point host 到 pattern 函数的模板等价约束。

    功能说明:
    - `entry_point` host 的同位置 memory 参数是 pattern 函数模板名真源。
    - 非 pattern 函数不参与该传播，避免 unrelated helper 函数被合并。

    使用示例:
    - constraints = _entry_point_pattern_template_constraints(funcs)
    """

    constraints: list[Same] = []
    for host in funcs.values():
        if not _is_entry_point_func(host):
            continue
        host_name = host.sym_name.data
        for candidate in funcs.values():
            if candidate is host or not _is_entry_point_pattern_func(host_name, candidate):
                continue
            constraints.extend(
                _same_position_memory_arg_constraints(
                    host,
                    candidate,
                    error_prefix="entry_point pattern",
                )
            )
    return tuple(constraints)
...
class TemplateNameInferPass(Pass):
    ...
    功能说明:
    - 对 module 内 `func.func` 统一求解 template-name 图。
    ...
    def apply(self, ctx: Context, module: ModuleOp) -> None:
        ...
        graph.add_constraints(_entry_point_pattern_template_constraints(funcs))
