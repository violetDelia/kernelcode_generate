...
class PassManager:
    ...
    def run(self: "PassManager", target: object) -> object:
        ...
        tile_like_names = [name for name in ("tile", "tile-elewise", "tile-reduce") if name in pass_names]
        if "symbol-loop-hoist" in pass_names:
            hoist_index = pass_names.index("symbol-loop-hoist")
            if tile_like_names:
                tile_name = max(tile_like_names, key=pass_names.index)
                tile_index = pass_names.index(tile_name)
                if hoist_index < tile_index:
                    raise ValueError(
                        f"SymbolLoopHoistRequiresSymbolFor: symbol-loop-hoist must run after {tile_name}"
                    )
        ...
        for tile_name in tile_like_names:
            tile_index = pass_names.index(tile_name)
            if "buffer-results-to-out-params" in pass_names:
                buffer_index = pass_names.index("buffer-results-to-out-params")
                if tile_index < buffer_index:
                    raise ValueError(
                        f"TilePassOrderError: {tile_name} must run after buffer-results-to-out-params"
                    )
            if "lower-dma-memory-hierarchy" in pass_names:
                dma_hierarchy_index = pass_names.index("lower-dma-memory-hierarchy")
                if tile_index > dma_hierarchy_index:
                    raise ValueError(
                        f"TilePassOrderError: {tile_name} must run before lower-dma-memory-hierarchy"
                    )
