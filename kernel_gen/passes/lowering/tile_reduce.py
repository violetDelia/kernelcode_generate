...
class TileReducePass(ModulePass):
    ...
    def apply(self: "TileReducePass", ctx: Context, module: ModuleOp) -> None:
        ...
        for op in module.ops:
            ...
            plans, _ordered_tile_names = tile_module._plan_tile_ops(op, block, tile_reduce=True)
            ...
            for plan in reduce_plans:
                ...
                new_ops.extend(tile_module._rewrite_matmul_plan(plan, tile_values=tile_values, tile_reduce=True))
                plan.op.attributes["tile.analysis"] = analysis_attr
                plan.op.attributes["tile.tile_exprs"] = tile_exprs_attr
