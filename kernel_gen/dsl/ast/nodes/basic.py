...
class MemoryAST(ValueAST):
    ...
    def type_from_memory(
        ...
        assert isinstance(ctx, Context)
        shape_attr = ArrayAttr([IntAttr(dim) if isinstance(dim, int) else StringAttr(str(dim)) for dim in memory.get_shape()])
        stride_attr = ArrayAttr([IntAttr(dim) if isinstance(dim, int) else StringAttr(str(dim)) for dim in memory.get_stride()])
