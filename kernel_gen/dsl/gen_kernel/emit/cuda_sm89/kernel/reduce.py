"""CUDA SM89 `kernel.reduce` emit registration.

功能说明:
- 注册 `target="cuda_sm89"` 的 `kernel.reduce` op emit。
- 单 op emit 返回 canonical diagnostic token；ModuleOp SourceBundle 由 final IR traversal 统一生成 wrapper calls。

API 列表:
- 无公开 API；`_emit_cuda_sm89_kernel_reduce(op: KernelReduceOp, ctx: EmitCContext) -> str` 仅作为 emit registry 装饰器入口存在。
...
from ..constants import CUDA_SM89_KERNEL_OP_REDUCE, CUDA_SM89_TARGET_NAME


@emit_c_impl(KernelReduceOp, target=CUDA_SM89_TARGET_NAME)
def _emit_cuda_sm89_kernel_reduce(op: KernelReduceOp, ctx: EmitCContext) -> str:
    """发射 CUDA SM89 `kernel.reduce` diagnostic token。
    ...
    使用示例:
    - token = _emit_cuda_sm89_kernel_reduce(op, EmitCContext())
    """

    expected_token = CUDA_SM89_KERNEL_OP_REDUCE
    op_name = op.name
    if op_name != expected_token:
        raise ctx.emit_error("cuda_sm89.kernel.reduce", f"unexpected op {op_name}")
