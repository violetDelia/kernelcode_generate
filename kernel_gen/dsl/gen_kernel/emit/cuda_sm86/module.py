...
- 将 lowered IR summary 和 SourceBundle 拼装委派给 package-local 文件级 API。
...
from .detect import summarize_cuda_sm86_module
...
def _emit_cuda_sm86_module(module_op: ModuleOp, ctx: EmitCContext) -> dict[str, str]:
    ...
    - 先生成 lowered IR summary，再构建 SourceBundle，root `__init__.py` 不承载业务逻辑。
    ...
    if not source_bundle:
        raise ctx.emit_error(CUDA_SM86_TARGET_NAME, "unsupported kernel family")
    if artifact_count < 2:
