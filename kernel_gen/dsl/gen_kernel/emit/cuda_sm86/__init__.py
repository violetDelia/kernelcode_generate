"""CUDA SM86 emit package registration.

功能说明:
- 聚合 CUDA SM86 target include、kernel op 和 `ModuleOp` backend 注册模块。
- 保持 `target="cuda_sm86"` 通过 emit registry 自动加载，不暴露包外 helper。
...
- 功能实现: kernel_gen/dsl/gen_kernel/emit/cuda_sm86/include.py
- 功能实现: kernel_gen/dsl/gen_kernel/emit/cuda_sm86/kernel/__init__.py
