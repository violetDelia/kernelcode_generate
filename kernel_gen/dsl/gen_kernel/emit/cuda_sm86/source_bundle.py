...
功能说明:
- 遍历 `cuda-sm86-lowering` 后的最终 IR，生成 trace comments、stable hash 和 CUDA SourceBundle artifacts。
...
CUDA_SM86_COMPUTE_OPS: frozenset[str] = frozenset(
    {"kernel.binary_elewise", "kernel.exp", "kernel.img2col2d", "kernel.matmul", "kernel.reduce"}
)
CUDA_SM86_SPACE_TO_CPP: dict[str, str] = {
...
class CudaSm86SourceBuilder:
    ...
    def __init__(self, module_op: ModuleOp, ctx: EmitCContext) -> None:
        ...
        功能说明:
        - 保存公开 emit context，用于按现有 CUDA SM86 unsupported 文本构造失败。
    ...
    def collect_trace(self) -> CudaSm86IrTrace:
        ...
        return CudaSm86IrTrace(
            ...
            kernel_symbol=f"kg_cuda_sm86_generated_kernel_{stable_hash}",
            device_body_symbol=f"kg_cuda_sm86_device_body_{stable_hash}",
    ...
    def validate_supported_compute(self, counts: dict[str, int]) -> None:
        """校验 final IR 至少包含当前 CUDA SM86 第一阶段支持的 compute op。
        ...
        if not any(counts.get(op_name, 0) > 0 for op_name in CUDA_SM86_COMPUTE_OPS):
    ...
    def operation_executable_word(self, record: str, index: int) -> str:
        ...
        - 该常量进入 generated body 的 `kg_cuda_sm86_ir_record_words` 数组，供 source semantic tests 观察 record 顺序。
    ...
    def render_generated_source(self, records: tuple[str, ...], stable_hash: str) -> str:
        ...
        - generated kernel 在 device 侧构造 `cuda_sm86::KernelContext`，再调用 generated device body；device body 按 block/region 递归发射 wrapper call。
        ...
        kernel_symbol = f"kg_cuda_sm86_generated_kernel_{stable_hash}"
        body_symbol = f"kg_cuda_sm86_device_body_{stable_hash}"
        entry_symbol = f"kg_cuda_sm86_execute_{stable_hash}_ir"
        host_ctx_symbol = f"kg_cuda_sm86_host_ctx_{stable_hash}"
        ...
        for walked_op in func_ops:
            ...
            helper_name = f"kg_cuda_sm86_device_func_{safe_func_name}"
                    ...
                    for block_arg in block.args:
                        ...
                        if isinstance(block_arg.type, NnMemoryType):
                            ...
                            if isinstance(element_type, Float32Type):
                                ...
                                raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: arch.launch")
            ...
            parameters = ", ".join(
                ["cuda_sm86::KernelContext& ctx", "int* kg_cuda_sm86_status", *parameter_names]
            )
            body = "\n".join(
...
struct {host_ctx_symbol} {{
  cuda_sm86::ArgSlot* slots;
  unsigned long long count;
}};

{callee_functions}

__device__ void {body_symbol}(cuda_sm86::KernelContext& ctx, cuda_sm86::ArgSlot* slots, unsigned long long count, int* kg_cuda_sm86_status) {{
  (void)ctx;
  const unsigned long long kg_cuda_sm86_ir_record_words[] = {{{record_words}}};
  (void)kg_cuda_sm86_ir_record_words;
{declarations}
{statements}
}}

__global__ void {kernel_symbol}(cuda_sm86::ArgSlot* slots, unsigned long long count, int* kg_cuda_sm86_status) {{
  cuda_sm86::KernelContext device_ctx;
  if (threadIdx.x == 0) {{
    {body_symbol}(device_ctx, slots, count, kg_cuda_sm86_status);
  }}
}}

int {entry_symbol}(cuda_sm86::ArgSlot* slots, unsigned long long count) {{
  if (slots == nullptr && count != 0ull) {{
    return -1;
  }}
  {host_ctx_symbol} host_ctx{{nullptr, count}};
  cuda_sm86::ArgSlot* kg_cuda_sm86_host_device_slots = nullptr;
  cuda_sm86::ArgSlot* kg_cuda_sm86_device_slots = nullptr;
  long long** kg_cuda_sm86_device_shapes = nullptr;
  long long** kg_cuda_sm86_device_strides = nullptr;
  void** kg_cuda_sm86_device_data = nullptr;
  unsigned long long* kg_cuda_sm86_element_counts = nullptr;
  int kg_cuda_sm86_host_status = 0;
  Status launch_status = kOk;
  if (count != 0ull) {{
    kg_cuda_sm86_host_device_slots = new (std::nothrow) cuda_sm86::ArgSlot[count]();
    kg_cuda_sm86_device_shapes = new (std::nothrow) long long*[count]();
    kg_cuda_sm86_device_strides = new (std::nothrow) long long*[count]();
    kg_cuda_sm86_device_data = new (std::nothrow) void*[count]();
    kg_cuda_sm86_element_counts = new (std::nothrow) unsigned long long[count]();
    if (kg_cuda_sm86_host_device_slots == nullptr || kg_cuda_sm86_device_shapes == nullptr ||
        kg_cuda_sm86_device_strides == nullptr || kg_cuda_sm86_device_data == nullptr ||
        kg_cuda_sm86_element_counts == nullptr) {{
      kg_cuda_sm86_host_status = -1;
    }}
  }}
  if (kg_cuda_sm86_host_status == 0) {{
    for (unsigned long long kg_cuda_sm86_slot_index = 0; kg_cuda_sm86_slot_index < count; ++kg_cuda_sm86_slot_index) {{
      kg_cuda_sm86_host_device_slots[kg_cuda_sm86_slot_index] = slots[kg_cuda_sm86_slot_index];
      if (slots[kg_cuda_sm86_slot_index].kind != 1) {{
        continue;
      }}
      if (slots[kg_cuda_sm86_slot_index].rank == 0ull || slots[kg_cuda_sm86_slot_index].shape == nullptr ||
          slots[kg_cuda_sm86_slot_index].stride == nullptr || slots[kg_cuda_sm86_slot_index].dtype_code != 1) {{
        kg_cuda_sm86_host_status = -1;
        break;
      }}
      const unsigned long long kg_cuda_sm86_rank = slots[kg_cuda_sm86_slot_index].rank;
      KG_CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&kg_cuda_sm86_device_shapes[kg_cuda_sm86_slot_index]), kg_cuda_sm86_rank * sizeof(long long)));
      KG_CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&kg_cuda_sm86_device_strides[kg_cuda_sm86_slot_index]), kg_cuda_sm86_rank * sizeof(long long)));
      cuda_sm86::detail::copy_host_to_device<long long>(
          kg_cuda_sm86_device_shapes[kg_cuda_sm86_slot_index],
          slots[kg_cuda_sm86_slot_index].shape,
          kg_cuda_sm86_rank);
      cuda_sm86::detail::copy_host_to_device<long long>(
          kg_cuda_sm86_device_strides[kg_cuda_sm86_slot_index],
          slots[kg_cuda_sm86_slot_index].stride,
          kg_cuda_sm86_rank);
      kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index] = cuda_sm86::detail::element_count(slots[kg_cuda_sm86_slot_index]);
      if (slots[kg_cuda_sm86_slot_index].data != nullptr && kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index] != 0ull) {{
        kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index] =
            cuda_sm86::detail::device_alloc<float>(kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index]);
        cuda_sm86::detail::copy_host_to_device<float>(
            reinterpret_cast<float*>(kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index]),
            reinterpret_cast<const float*>(slots[kg_cuda_sm86_slot_index].data),
            kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index]);
      }}
      kg_cuda_sm86_host_device_slots[kg_cuda_sm86_slot_index].shape = kg_cuda_sm86_device_shapes[kg_cuda_sm86_slot_index];
      kg_cuda_sm86_host_device_slots[kg_cuda_sm86_slot_index].stride = kg_cuda_sm86_device_strides[kg_cuda_sm86_slot_index];
      kg_cuda_sm86_host_device_slots[kg_cuda_sm86_slot_index].data = kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index];
    }}
  }}
  if (kg_cuda_sm86_host_status == 0 && count != 0ull) {{
    KG_CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&kg_cuda_sm86_device_slots), count * sizeof(cuda_sm86::ArgSlot)));
    cuda_sm86::detail::copy_host_to_device<cuda_sm86::ArgSlot>(kg_cuda_sm86_device_slots, kg_cuda_sm86_host_device_slots, count);
  }}
  int* kg_cuda_sm86_device_status = nullptr;
  if (kg_cuda_sm86_host_status == 0) {{
    KG_CUDA_CHECK(cudaMalloc(reinterpret_cast<void**>(&kg_cuda_sm86_device_status), sizeof(int)));
    KG_CUDA_CHECK(cudaMemset(kg_cuda_sm86_device_status, 0, sizeof(int)));
    host_ctx.slots = kg_cuda_sm86_device_slots;
    launch_status = cuda_sm86::launch<1, 256, 1, 49152, {kernel_symbol}>(
        host_ctx, kg_cuda_sm86_device_slots, count, kg_cuda_sm86_device_status);
    KG_CUDA_CHECK(cudaMemcpy(&kg_cuda_sm86_host_status, kg_cuda_sm86_device_status, sizeof(int), cudaMemcpyDeviceToHost));
  }}
  if (kg_cuda_sm86_host_status == 0) {{
    for (unsigned long long kg_cuda_sm86_slot_index = 0; kg_cuda_sm86_slot_index < count; ++kg_cuda_sm86_slot_index) {{
      if (slots[kg_cuda_sm86_slot_index].kind == 1 && slots[kg_cuda_sm86_slot_index].data != nullptr &&
          kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index] != nullptr &&
          kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index] != 0ull) {{
        cuda_sm86::detail::copy_device_to_host<float>(
            reinterpret_cast<float*>(slots[kg_cuda_sm86_slot_index].data),
            reinterpret_cast<const float*>(kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index]),
            kg_cuda_sm86_element_counts[kg_cuda_sm86_slot_index]);
      }}
    }}
  }}
  if (kg_cuda_sm86_device_status != nullptr) {{
    KG_CUDA_CHECK(cudaFree(kg_cuda_sm86_device_status));
  }}
  cuda_sm86::detail::device_free<cuda_sm86::ArgSlot>(kg_cuda_sm86_device_slots);
  for (unsigned long long kg_cuda_sm86_slot_index = 0; kg_cuda_sm86_slot_index < count; ++kg_cuda_sm86_slot_index) {{
    cuda_sm86::detail::device_free<long long>(kg_cuda_sm86_device_shapes == nullptr ? nullptr : kg_cuda_sm86_device_shapes[kg_cuda_sm86_slot_index]);
    cuda_sm86::detail::device_free<long long>(kg_cuda_sm86_device_strides == nullptr ? nullptr : kg_cuda_sm86_device_strides[kg_cuda_sm86_slot_index]);
    cuda_sm86::detail::device_free<float>(
        kg_cuda_sm86_device_data == nullptr ? nullptr : reinterpret_cast<float*>(kg_cuda_sm86_device_data[kg_cuda_sm86_slot_index]));
  }}
  delete[] kg_cuda_sm86_host_device_slots;
  delete[] kg_cuda_sm86_device_shapes;
  delete[] kg_cuda_sm86_device_strides;
  delete[] kg_cuda_sm86_device_data;
  delete[] kg_cuda_sm86_element_counts;
  if (launch_status != kOk || kg_cuda_sm86_host_status != 0) {{
    return -1;
  }}
  return 0;
}}
...
"""
                ...
                for block in region.blocks:
                    for arg_index, arg in enumerate(block.args):
                        ...
                        if isinstance(arg.type, NnMemoryType):
                            ...
                            lines.append(
                                f"  auto {name} = cuda_sm86::detail::memory_from_slot<{space_cpp}, float>(slots, count, {arg_index});"
                            )
                        else:
                            default_value = self.symbol_type_default_cpp(arg.type)
                            lines.append(
                                f"  S_INT {name} = cuda_sm86::detail::int_arg_or(slots, count, {arg_index}, {default_value});"
                            )
        return "\n".join(lines)
    ...
    def render_operation_statement(
        ...
        功能说明:
        - 根据真实 op name、operand 绑定、result 绑定、memory space 和 attrs 选择 CUDA SM86 wrapper call。
        ...
        if op_name == "symbol.for":
            if op_indices is None:
                raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: symbol.for")
            return self.render_symbol_for_statement(comment, op, names, op_indices)
        if op_name == "scf.if":
            if op_indices is None:
                raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: scf.if")
        ...
        if op_name == "arch.launch":
            ...
            helper_name = f"kg_cuda_sm86_device_func_{safe_callee_name}"
            self.rendered_launch_callees.add(callee_name)
            call_args = ", ".join(("ctx", "kg_cuda_sm86_status", *argument_names))
            return (
                f"{comment}\n"
                f"  {helper_name}({call_args});\n"
                f"  if (kg_cuda_sm86_status != nullptr && *kg_cuda_sm86_status != 0) {{\n"
                f"    return;\n"
                f"  }}"
            )
        ...
        if op_name == "dma.free" and op.operands:
            if self.memory_space_name(op.operands[0]) == "global":
                return f"{comment}\n  // cuda_sm86.dma.free: external global memory is owned by the caller"
        ...
        if op_name == "dma.alloc" and op.results:
            ...
            if not isinstance(result_type, NnMemoryType):
                raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: dma.alloc")
            ...
            return (
                f"{comment}\n"
                f"  auto {result_names[0]} = cuda_sm86::alloc<{self.memory_space_cpp(result)}, {element_cpp}>"
                f"(ctx, {shape}, {stride});\n"
                f"  if (!cuda_sm86::detail::memory_ready({result_names[0]})) {{\n"
                f"    if (kg_cuda_sm86_status != nullptr) {{ *kg_cuda_sm86_status = -1; }}\n"
                f"    return;\n"
                f"  }}"
            )
        if op_name == "dma.fill" and len(op.operands) >= 2:
            return self.render_status_checked_call(comment, f"cuda_sm86::fill(ctx, {operand_names[0]}, {operand_names[1]})")
        ...
        if op_name == "kernel.exp" and len(op.operands) >= 2:
            ...
            return self.render_status_checked_call(comment, f"cuda_sm86::exp<{space_cpp}, float, float>(ctx, {out_name}, {input_name})")
        if op_name == "kernel.reduce" and len(op.operands) >= 2:
            ...
            out_name, input_name = operand_names[0], operand_names[1]
            space_cpp = self.memory_space_cpp(op.operands[0])
            return self.render_status_checked_call(comment, f"cuda_sm86::{op_kind}<{space_cpp}, float, float>(ctx, {out_name}, {input_name}, {axis})")
        return comment
    ...
    def render_status_checked_call(self, comment: str, call: str) -> str:
        ...
        使用示例:
        - stmt = builder.render_status_checked_call(comment, "cuda_sm86::fill(ctx, tile, 0.0f)")
        """
    ...
    def render_scf_if_statement(self, comment: str, op: Operation, names: dict[int, str], op_indices: dict[int, int]) -> str:
        ...
        if isinstance(condition_owner, Operation) and condition_owner.name == "builtin.unregistered":
            if false_region is None or len(false_region.blocks) == 0:
                return f"{comment}\n  // cuda_sm86.scf.if: folded false builtin.unregistered condition with no else body"
            false_body = "\n".join(self.render_block_statements(block, names, op_indices) for block in false_region.blocks)
            return f"{comment}\n  // cuda_sm86.scf.if: folded false builtin.unregistered condition\n{false_body}"
    ...
    def render_load_store_statement(
        ...
        *,
        ...
        功能说明:
        - target/global write-back 使用 `cuda_sm86::store`，其它 materializing transfer 使用 `cuda_sm86::load`。
        ...
        call = (
            f"cuda_sm86::{wrapper}<{self.memory_space_cpp(target)}, {self.memory_space_cpp(source)}, float, float>"
    ...
    def render_slice_statement(
        ...
        stride = self.operation_vector_cpp(op.strides, names, self.unit_stride_vector(target))
        call = (
            f"cuda_sm86::{wrapper}<{self.memory_space_cpp(target)}, {self.memory_space_cpp(source)}, float>"
            f"(ctx, {operand_names[0]}, {operand_names[1]}, {offset}, {size}, {stride})"
        )
        return self.render_status_checked_call(comment, call)
    ...
    def render_transpose_statement(self, comment: str, op: Operation, operand_names: tuple[str, ...]) -> str:
        ...
        - perm 由 final IR attr 精确进入 public `cuda_sm86::transpose(...)` wrapper 实参。
        ...
        call = (
            f"cuda_sm86::transpose<{self.memory_space_cpp(target)}, {self.memory_space_cpp(source)}, float, float>"
    ...
    def render_alias_statement(
        ...
        - global/tsm 的一维 view 使用已确认 public `cuda_sm86::view(...)`；TLM1 alias 走 `detail` fragment descriptor glue，避免把 fragment 当普通 pointer view。
        ...
        if alias_kind == "reinterpret":
            return (
                f"{comment}\n"
                f"  auto {result_names[0]} = cuda_sm86::detail::fragment_alias<{result_space_cpp}, float>"
        ...
        if self.memory_rank(result) == 1:
            return (
                f"{comment}\n"
                f"  auto {result_names[0]} = cuda_sm86::view<{self.memory_space_cpp(source)}, float>"
    ...
    def render_make_ring_statement(
        ...
        - slot shape/stride 来自 result ring 或 backing descriptor，进入 public `cuda_sm86::make_ring(...)` 调用。
        ...
        if not isinstance(ring_type, DmaRingType):
            raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: dma.make_ring")
        ...
        return (
            f"{comment}\n"
            f"  auto {result_names[0]} = cuda_sm86::make_ring<{slot_cpp}, {self.memory_space_cpp(backing)}, {backing_cpp}>"
    ...
    def render_matmul_statement(self, comment: str, op: Operation, operand_names: tuple[str, ...], op_index: int) -> str:
        ...
        call = f"cuda_sm86::matmul<{lhs_space}, {rhs_space}, {out_space}, float, float, float>(ctx, {out_name}, {lhs_name}, {rhs_name}, false)"
        return (
            f"{comment}\n"
    ...
    def render_img2col2d_statement(self, comment: str, op: Operation, operand_names: tuple[str, ...]) -> str:
        ...
        call = (
            f"cuda_sm86::img2col2d<{input_space}, {out_space}, float, float>"
    ...
    def render_binary_elewise_statement(self, comment: str, op: Operation, operand_names: tuple[str, ...]) -> str:
        ...
        for candidate in ("sub", "mul", "truediv", "max"):
            if self.attr_contains(op, "kind", candidate):
                wrapper = candidate
        space_cpp = self.memory_space_cpp(op.operands[0])
        call = f"cuda_sm86::{wrapper}<{space_cpp}, float, float>(ctx, {operand_names[0]}, {operand_names[1]}, {operand_names[2]})"
        return self.render_status_checked_call(comment, call)
    ...
    def memory_space_name(self, value: SSAValue) -> str:
        """读取 memory value 的 CUDA SM86 space 名称。
    ...
    def memory_space_cpp(self, value: SSAValue) -> str:
        ...
        if space not in CUDA_SM86_SPACE_TO_CPP:
            raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: memory_space")
        return CUDA_SM86_SPACE_TO_CPP[space]
    ...
    def value_vector_cpp(self, values: tuple[SSAValue, ...] | list[SSAValue], names: dict[int, str]) -> str:
        ...
        if not values:
            raise self.ctx.emit_error("cuda_sm86", "unsupported cuda_sm86 final IR op: vector")
    ...
    def render_kernel_source(self, trace: CudaSm86IrTrace) -> str:
        ...
        - C ABI entry 调用 hash 专属 generated entry；generated entry 再通过 `cuda_sm86::launch` 进入 generated `__global__` kernel。
...
def build_cuda_sm86_source_bundle(module_op: ModuleOp, ctx: EmitCContext) -> dict[str, str]:
    ...
    - 输入必须是 `cuda-sm86-lowering` 后的 `ModuleOp`；unsupported op 使用既有 CUDA SM86 fail-fast 文本。
