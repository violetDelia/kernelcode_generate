...
from xdsl.dialects.builtin import FloatAttr, IndexType, IntAttr, IntegerAttr, IntegerType, StringAttr
...
from ..register import emit_c_attr_impl, emit_c_name_impl
...
def _format_literal(op: arith.ConstantOp, ctx: EmitCContext) -> str:
    ...
    if isinstance(value, FloatAttr):
        return str(value.value.data)
    raise emit_c_error(ctx, op.name, "unsupported constant literal")
...
def _format_static_layout(values: Any, ctx: EmitCContext, subject: str) -> list[str]:
    ...
    for value in values:
        if not isinstance(value, IntAttr):
            raise emit_c_error(ctx, subject, "only static memory layout is supported")
...
def _allocate_temp_name(ctx: EmitCContext, prefix: str) -> str:
    counters = ctx.config.setdefault("temp_name_counters", {})
...
def _emit_backing_storage_decl(
    ...
    if static_numel is None:
        raise emit_c_error(ctx, f"memory {name}", "dynamic shape backing is unsupported")
...
def _emit_dma_load_stmt(op: DmaLoadOp, ctx: EmitCContext) -> str:
    ...
    if not isinstance(op.target.type, NnMemoryType):
        raise emit_c_error(ctx, op.name, "target must be nn.memory")
...
def _emit_dma_store_stmt(op: DmaStoreOp, ctx: EmitCContext) -> str:
    ...
    if not isinstance(source_type, NnMemoryType) or not _is_unit_tile(source_type):
        raise emit_c_error(ctx, op.name, "only unit-tile dma.store source is supported")
    return (
...
def _emit_c_value(value: SSAValue, ctx: EmitCContext) -> str:
    ...
    if isinstance(value.type, NnMemoryType):
        raise emit_c_error(ctx, owner.name, f"invalid dependency for value {value}")
    ...
    if isinstance(owner, arith.CmpiOp):
        ...
        if predicate not in _CMPI_SIGILS:
            raise emit_c_error(ctx, owner.name, "unsupported comparison predicate")
...
def _emit_c_op(op: Operation, ctx: EmitCContext) -> str:
    ...
    if isinstance(op, func.ReturnOp):
        ...
        if len(op.arguments) != 1:
            raise emit_c_error(ctx, op.name, "unsupported return arity")
