...
- `include/api/Dma.h` 不公开 `copy` helper，因此整块 copy 发射为公开 `slice(ctx, ...)` 调用。
...
def _emit_npu_demo_dma_copy(op: DmaCopyOp, ctx) -> str:
    ...
    功能说明:
    - 将 `dma.copy(target, source)` 映射为公开 `slice(ctx, target, source, zero, target.shape, one)`。
    ...
    if rank > 8:
        raise ctx.emit_error("dma.copy", "layout rank exceeds Vector brace-list capacity")
    ...
    return (
        f"{ctx.current_indent}slice(ctx, {target_expr} /*dst*/, {source_expr} /*source*/, "
