...
- compile-only 策略的 source/build 文本产物写出统一委托 `kernel_gen.core.tools.dump_dir.DumpDirWriter`。
...
def _compile_root(function: str) -> DumpDirWriter:
    ...
    if writer is not None:
        return writer.child("compile").child(_TARGET).child(safe_function, fallback="kernel")
    return DumpDirWriter(Path(tempfile.mkdtemp(prefix="kg_dummy_compile_")))
...
class _DummyCompileStrategy:
    ...
    def compile(self, request: CompileRequest) -> CompiledKernel:
        ...
        source_writer = root.child("source")
        build_writer = root.child("build")
        source_writer.write("source.cpp", request.source)
        artifacts = _parse_source_bundle(request.source)
        if artifacts is not None:
            for artifact_path, content in artifacts.items():
                try:
                    source_writer.write(artifact_path, content)
                except ValueError as exc:
                    raise _dummy_error("source_bundle_path_escape") from exc
        artifact = build_writer.write("libdummy.so", "")
        return CompiledKernel(
            target=request.target,
            soname_path=str(artifact),
            function=request.function,
            entry_point=request.entry_point,
            compile_stdout=f"dummy compile strategy: source={source_writer.root} build={build_writer.root}",
            compile_stderr="",
        )
