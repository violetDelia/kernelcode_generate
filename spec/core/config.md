...
- 定义项目级公共配置底座，统一承载显式公开的 target、dump_dir、runtime trance 与 codegen mode 配置及稳定读写接口。
...
- `codegen_mode` 是源码生成模式的公开开关；`"norm"` 生成真实运行 host，`"cost"` 生成 npu_demo cost summary host。
...
- `reset_config() -> None`
- `class CoreConfigSnapshot(target: str | None, dump_dir: Path | None, trance_enabled: bool, codegen_mode: Literal["norm", "cost"] = "norm")`
...
- 注意事项：该接口会把 `target` 与 `dump_dir` 恢复为 `None`，把 `trance_enabled` 恢复为 `False`，并把 `codegen_mode` 恢复为 `"norm"`；测试和工具入口应在跨 case 修改配置后调用该接口或使用 `snapshot_config/restore_config` 恢复现场。

### `set_codegen_mode(value: Literal["norm", "cost"]) -> None`

- api：`set_codegen_mode(value: Literal["norm", "cost"]) -> None`
- 参数：
  - `value`：源码生成模式；类型 `Literal["norm", "cost"]`；无默认值，调用方必须显式提供；`"norm"` 表示普通真实运行源码，`"cost"` 表示 npu_demo cost summary 源码。
- 返回值：无返回值；调用成功表示模式已更新。
- 使用示例：

  ```python
  from kernel_gen.core.config import get_codegen_mode, set_codegen_mode

  set_codegen_mode("cost")
  assert get_codegen_mode() == "cost"
  ```
- 功能说明：设置源码生成模式。
- 注意事项：仅接受 `"norm"` 或 `"cost"`；非法值必须失败，错误文本为 `codegen_mode must be 'norm' or 'cost'`；不得通过 `gen_kernel(...)`、`dsl_run(...)` 或 `dsl_cost_run(...)` 新增平行 mode 入参。

### `get_codegen_mode() -> Literal["norm", "cost"]`

- api：`get_codegen_mode() -> Literal["norm", "cost"]`
- 参数：无。
- 返回值：`"norm"` 或 `"cost"`。
- 使用示例：

  ```python
  from kernel_gen.core.config import get_codegen_mode

  mode = get_codegen_mode()
  ```
- 功能说明：读取当前源码生成模式。
- 注意事项：默认值为 `"norm"`；调用方不得读取本模块内部变量替代该接口。
...
- api：`class CoreConfigSnapshot(target: str | None, dump_dir: Path | None, trance_enabled: bool, codegen_mode: Literal["norm", "cost"] = "norm")`
- 参数：
  ...
  - `codegen_mode`：源码生成模式；类型 `Literal["norm", "cost"]`；默认值 `"norm"`；旧三字段构造必须继续可用并得到 `"norm"`。
...
- 注意事项：该类型只保存公开 `target`、`dump_dir`、`trance_enabled` 与 `codegen_mode` 快照；不得塞入任意 dict、硬件配置、emit 上下文状态或临时生成状态。
...
- 注意事项：只接受 `CoreConfigSnapshot`，不接受任意 dict；该接口会恢复快照中的 `target`、`dump_dir`、`trance_enabled` 与 `codegen_mode`，不会恢复未列入本文件 API 的内部状态。
...
- `codegen_mode` 只控制 `gen_kernel(...)` 的源码形态；`dsl_run(...)` 必须临时使用 `"norm"` 并恢复调用前快照，`dsl_cost_run(...)` 必须临时使用 `"cost"` 并恢复调用前快照。
...
- 验证 `set_target/get_target/set_dump_dir/get_dump_dir/set_trance_enabled/get_trance_enabled/set_codegen_mode/get_codegen_mode/reset/snapshot/restore` 公开接口的稳定行为，并验证非法类型输入会稳定失败。
...
| TC-CORE-CONFIG-007 | 公开入口 | codegen mode round trip | 设置、读取、快照和恢复源码生成模式。 | 运行 `test_codegen_mode_round_trip`。 | `set_codegen_mode("cost")` 后读取为 `"cost"`，`reset_config()` 后恢复为 `"norm"`，非法值被拒绝。 | `test_codegen_mode_round_trip` |
