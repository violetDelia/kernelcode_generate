...
- 为后续实现恢复明确最小支持范围：只读 `Memory` 输入、`Memory` 结果降为显式输出参数、标量参数顺序保持、`emit_c` 错误向上抛出。
...
- 当前规范恢复范围仅覆盖 [`test/dsl/test_gen_kernel.py`](../../test/dsl/test_gen_kernel.py) 中已有用例映射，不在本阶段扩展到其他返回形式或更宽的后端语义。
...
- GK-008：不支持的返回形式明确报错。（`test_gen_signature_rejects_unsupported_return_form`）
- GK-009：生成源码保留函数名与参数名。（`test_gen_kernel_preserves_function_and_arg_names`）
