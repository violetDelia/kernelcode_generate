...
- `target="cpu"` 与 `target="npu_demo"` 的 `dma.reinterpret` 发射必须构造共享 source data 的 `Memory<SPACE, T>`，不生成数据搬运；source 是一维 `i8` byte pool 时按 byte offset 偏移后 `reinterpret_cast<T*>`，typed source 按 source element offset 偏移。
...
| TC-DSL-GEN-KERNEL-EMIT-026 | 边界/异常 | emit c op lowers kernel family and rejects unsupported reduce kind | 准备触发该错误路径的公开输入或非法参数组合。 | 运行 `test_emit_c_op_lowers_kernel_family_and_rejects_unsupported_reduce_kind`。 | `kernel.binary_elewise` 覆盖 CPU `min/max` 与 npu_demo `max` 发射；不支持 kind 按公开错误语义失败。 | `test_emit_c_op_lowers_kernel_family_and_rejects_unsupported_reduce_kind` |
...
| TC-DSL-GEN-KERNEL-EMIT-038 | pass 改写 | emit c op lowers DMA alloc, view and reinterpret | 准备包含目标 op、pass 名称或 pipeline 的公开 IR 输入。 | 运行 `test_emit_c_op_lowers_dma_alloc_and_view`。 | CPU / npu_demo 下 `dma.reinterpret` 构造共享 source data 的 Memory，byte-pool source 使用 byte offset + reinterpret cast；既有 alloc/view 行为保持。 | `test_emit_c_op_lowers_dma_alloc_and_view` |
...
| TC-DSL-GEN-KERNEL-EMIT-052 | pass 改写 | emit c lowers npu demo kernel context queries | 准备包含 `arch.get_block_id`、`arch.get_thread_id`、`arch.get_thread_num` 的 npu_demo emit 输入。 | 运行 `test_emit_c_lowers_npu_demo_kernel_context_queries`。 | block/thread query value 与语句生成对应 `npu_demo::block_id()` / `npu_demo::thread_id()` / `npu_demo::thread_num()` free helper 源码。 | `test_emit_c_lowers_npu_demo_kernel_context_queries` |
