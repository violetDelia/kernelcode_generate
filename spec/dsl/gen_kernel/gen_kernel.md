...
- 测试目标：
  ...
  - `gen_kernel(...)` 入口稳定
  - `gen_kernel(...)` 在公开 `dump_dir` 非空时写出 `source.cpp`
  ...
  - 测试只通过 `dsl_gen_kernel(...)`、`gen_kernel(...)`、package-root 导入和 `KernelCodeError` 验收，不直连 `KernelEmitter`、`kernel_emitter.py` helper、`mlir_gen` 私有 helper 或其他跨文件非公开 API
