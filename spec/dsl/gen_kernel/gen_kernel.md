...
- 对 `target="npu_demo"`，generated source layout 参数必须消费 include/npu_demo public initializer-list overload；源码中不得生成 `Vector(...)`、`Vector{...}`、`long long *_shape[]` 或 `long long *_stride[]` layout buffer。
