...
- 对 `target="npu_demo"`，普通节点级 `tuner.select` fallback 仍发射为 `S_INT <name> = 0;`，表示第一版固定选择 pattern0。
- 对 `target="npu_demo"`，若 module 含唯一 `entry_point` host dispatcher 且该 dispatcher 使用 `tuner.select` 的 `patterns` attr 表达 pattern 候选，`gen_kernel(...)` 必须生成 entry 级 `enum class`，enumerator 按输入 patterns 顺序写为 `pattern0 = 0`、`pattern1 = 1`、...、`pattern{N-1} = N-1`；同时生成返回该 enum 的 selector function，selector 参数只来自 `tuner_args`，首版 body 固定 `return <Enum>::pattern0;`。
- 对 `target="npu_demo"` 的 entry dispatcher enum/default selector 路径，`tuner.select args(...)` 的 `args` 只服务真实 pattern launch，不自动传给 selector function；host dispatcher 必须把 selector 返回值绑定为 enum 变量，并用 `pattern == <Enum>::patternN` 形式驱动互斥 N 路真实 `KernelContext` launch 分支。
- 对 `target="npu_demo"` 的 entry dispatcher enum/default selector 路径，本轮不得生成 runtime `CostContext` 评估、`candidate_cost`、`best_cost`、`data_movement_cost` 或 `compute_cost` 选择逻辑。
...
| TC-DSL-GEN-KERNEL-GEN-KERNEL-014 | 生成/编译 | gen kernel rewritten deslice target return uses front out param | 准备公开 DSL/IR 输入、目标配置与源码生成入口。 | 运行 `test_gen_kernel_rewritten_deslice_memory_result_uses_front_out_param`。 | 生成源码、IR 文本或编译结果体现“gen kernel rewritten deslice target return uses front out param”场景。 | `test_gen_kernel_rewritten_deslice_memory_result_uses_front_out_param` |
...
| TC-DSL-GEN-KERNEL-GEN-KERNEL-071 | 生成/编译 | gen kernel emits npu demo entry dispatcher enum default selector | 准备含 `entry_point` host、`tuner.select patterns` 与三路 outlined `arch.launch` 分支的 module。 | 运行 `test_gen_kernel_emits_npu_demo_entry_dispatcher_enum_default_selector`。 | 源码包含 pattern enum、默认返回 `pattern0` 的 selector function、enum comparison 驱动的三路互斥真实 `KernelContext` launch，且不包含 runtime cost selector 文本。 | `test_gen_kernel_emits_npu_demo_entry_dispatcher_enum_default_selector` |
