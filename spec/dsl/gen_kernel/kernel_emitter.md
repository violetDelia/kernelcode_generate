...
- `NnMemoryType` 参数携带兼容 `template_name` property 时，`KernelEmitter` 必须按函数签名首次出现顺序生成 `template <typename T1, ...>`，并在 wrapper、device body 与 body-level kernel 签名中输出 `Memory<space, Tn>&`。
- launch wrapper 与 body 签名比较只允许忽略 `external_attrs["template_name"]`；其它 `external_attrs` 差异属于真实 ABI 差异，必须触发 launch signature mismatch。
...
- `npu_demo` final host / entry dispatcher 中允许发射 xDSL `builtin.unregistered` + `op_name__` 为 `"symbol.const"` / `"symbol.eq"` 的 generic symbol op：`"symbol.const"` 必须绑定 `S_INT` 局部值，`"symbol.eq"` 必须消费已绑定值并生成布尔局部值；该支持仅限这两个 generic op，不能扩展为通用 `builtin.unregistered` 发射。
...
| TC-DSL-GEN-KERNEL-KERNEL-EMITTER-014 | 生成/编译 | gen kernel rewritten deslice target return uses front out param | 准备公开 DSL/IR 输入、目标配置与源码生成入口。 | 运行 `test_gen_kernel_rewritten_deslice_memory_result_uses_front_out_param`。 | 生成源码、IR 文本或编译结果体现“gen kernel rewritten deslice target return uses front out param”场景。 | `test_gen_kernel_rewritten_deslice_memory_result_uses_front_out_param` |
...
| TC-DSL-GEN-KERNEL-KERNEL-EMITTER-064 | 生成/编译 | gen kernel emits npu demo entry dispatcher generic symbol guard | 准备含 `entry_point` dispatcher、device 函数、generic `"symbol.const"` / `"symbol.eq"` selector guard 与 `arch.launch` 的公开 IR module。 | 运行 `test_gen_kernel_emits_npu_demo_entry_dispatcher_generic_symbol_guard`。 | 公开 `gen_kernel(...)` 生成 device 函数和 entry dispatcher；generic const 发射为 `S_INT`，generic eq 发射为 bool 条件，两个 `arch.launch` 均生成，且不出现 `builtin.unregistered` unsupported。 | `test_gen_kernel_emits_npu_demo_entry_dispatcher_generic_symbol_guard` |
| TC-DSL-GEN-KERNEL-KERNEL-EMITTER-065 | 边界/异常 | gen kernel rejects other builtin unregistered symbol ops | 准备含未注册 generic `"symbol.fake"` 的 `npu_demo` module。 | 运行 `test_gen_kernel_rejects_other_builtin_unregistered_symbol_ops`。 | 除 `"symbol.const"` / `"symbol.eq"` 之外的 generic `builtin.unregistered` op 继续按公开 unsupported 路径失败。 | `test_gen_kernel_rejects_other_builtin_unregistered_symbol_ops` |
