...
- `broadcast` 的字段顺序必须保持 target-first，且只接受 `target/source` 两个位置参数。
...
- 验证 `dma.broadcast(target, source)` 解析为 `DmaBroadcastAST`，不被误当作返回值式 helper。
...
| TC-DSL-AST-PLUGIN-DMA-005 | 公开入口 | DMA public statement helpers parse parameterized nodes | 准备 free/store/deslice/broadcast 语句型公开调用。 | 运行 `test_dma_public_statement_helpers_parse_parameterized_nodes`。 | 语句型 DMA helper 均解析为对应公开 AST 节点。 | `test_dma_public_statement_helpers_parse_parameterized_nodes` |
