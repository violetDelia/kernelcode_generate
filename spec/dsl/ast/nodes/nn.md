...
- 注意事项：构造参数必须是公开 AST 节点、公开 symbol/memory 类型或签名声明的 Python 基础值；不得传入内部 visitor/helper 状态；当输入来自 `dma.reshape` 且 shape operand 携带匿名动态维度时，transpose 结果连续 stride 必须保持匿名 `?`，不得使用 operand 名称或 SSA `name_hint` 伪造稳定维度。
...
- 注意事项：构造参数必须是公开 AST 节点、公开 symbol/memory 类型或签名声明的 Python 基础值；不得传入内部 visitor/helper 状态；matmul 两侧 contracting 维度必须可证明相等，静态、命名符号与结构化符号表达式均要求完全一致；两侧均为匿名 `?` 时按 unknown contracting 维度承接，单侧匿名 `?` 必须拒绝。
