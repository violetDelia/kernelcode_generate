...
- 当函数体仅包含 `deslice` 等写回语句且无显式 `return` 时，输出 `func.func` 允许零返回值，`ReturnOp` 必须无返回参数。
...
- DSL 函数体内允许出现 `alloc`、`copy`、`cast`、`view`、`reshape`、`flatten`、`free`、`load`、`store`、`slice`、`deslice` 这组 DMA helper 调用；其公开语义由 `emit_mlir` 负责落实到具体 lowering，其中 `deslice` 必须 lowering 为 `dma.deslice` 且不得生成 `dma.store`。
...
- 测试目标：
  ...
  - 验证 deslice helper 在 runtime_args offsets/sizes 场景仍可 lowering 为 `dma.deslice`，且不生成 `dma.store`。
...
- 功能与用例清单：
  ...
  - MGEN-026：DMA helper 调用在 `build_func_op(...)` 链路中按公开语义分流：`alloc/copy/cast/view/reshape/flatten` 生成对应 memory 结果，`free` 作为无返回值语句参与 lowering，`load/store/slice/deslice` 保持既有 memory 读写行为；`deslice` 不得生成 `dma.store`。（`test_build_func_op_supports_dma_helper_calls`、`test_build_func_op_supports_dma_free_statement`、`test_build_func_op_supports_dma_load_helper`、`test_build_func_op_supports_dma_store_helper`、`test_build_func_op_supports_dma_slice_helper`、`test_build_func_op_supports_dma_deslice_helper`、`test_build_func_op_supports_dma_deslice_helper_with_runtime_args`）
  ...
  - MGEN-026F：`build_func_op` 遇到 `deslice` helper 的非法 arity/target/space 时必须抛 `AstVisitorError`，报错文本与解析诊断一致。（`test_build_func_op_rejects_invalid_deslice_helper_variants`）
