...
- `view(...)` lowering 必须将 DSL `offset/size/stride` 参数分别透传为 `dma.view` 的 `offsets/shape/stride` operand，且 `result_type.stride` 必须来自 DSL `stride` 参数，不继承 `source.stride`。
- `view(...)` 的 `numel` 一致性判定归属 `dma.view` verifier（参见 [`spec/dialect/dma.md`](../../spec/dialect/dma.md) 的 `TC-DMA-015`）；`emit_mlir` 仅负责参数透传与类型组装，不额外改写该约束。
...
- DMA helper 的公开 lowering 约束如下：
  ...
  - `view(...)`：lowering 为 `dma.view`，`offset/size/stride` 分别映射到 `offsets/shape/stride` operand，`result_type.stride` 取 DSL `stride`；`numel` 合法性由方言 verifier 负责。
...
- 拆分归属：EMIT-001~EMIT-028 归属 `test_emit_mlir.py`；EMIT-029 归属 `test_mlir_gen.py`。
- 测试目标：
  - 覆盖常见表达式与语句节点的发射结果。
  ...
  - 覆盖 `view(...)` 的 `offset/size/stride` operand 透传与 `result_type.stride` 来源规则（来自 DSL `stride`，不继承 `source.stride`）；`numel` 判定留给 dialect verifier。
...
- 功能与用例清单：
  ...
  - EMIT-007A：`LoadAST` 携带 `sizes`（slice）且非 unit stride 时抛出可定位错误。（`test_slice_ast_lowering_raises_lowering_error`）
  ...
  - EMIT-009A：`StoreAST` 非 unit stride 抛出可定位错误。（`test_store_ast_lowering_rejects_non_unit_stride`）
  - EMIT-009B：`StoreAST` 携带 `sizes`（deslice）且非 unit stride 时抛出可定位错误。（`test_deslice_ast_lowering_rejects_non_unit_stride`）
  ...
  - EMIT-018：`view(...)` lowering 为 `dma.view` 并返回视图 memory 结果；`offset/size/stride` operand 与 DSL 参数一致，且结果 `stride` 来源于 DSL `stride`。（`test_emit_mlir_dma_view_lowering`）
  - EMIT-018A：operation `view` 的“合法 subview 但不属于当前 dialect 可验证子集”边界不在 emit 阶段吞掉；`emit_mlir` 仅透传参数，`numel` 约束仍由 `dma.view` verifier 判定。（`test_emit_mlir_dma_view_mapping_boundary_keeps_numel_check_in_verifier`）
