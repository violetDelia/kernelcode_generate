...
- DSL 解析入口当前仅将无参 `get_block_id()` / `get_block_num()` / `get_subthread_id()` 识别为 `arch` 查询 builtin，并解析为专用 `ArchQueryAST` 节点。
...
- `query_name` (`str`)：查询名，当前仅允许 `get_block_id` / `get_block_num` / `get_subthread_id`。
- `location` (`SourceLocation|None`)：可选源码位置。
...
- 当前仅允许 `get_block_id` / `get_block_num` / `get_subthread_id`；其他 `arch` 调用不属于本节点职责。
...
- 测试目标：
  ...
  - 覆盖 `get_subthread_id()` 解析为 `ArchQueryAST` 的最小 arch 查询入口。
  - 覆盖 `get_subthread_id()` 的非法参数在 AST 解析阶段被拒绝。
- 功能与用例清单：
