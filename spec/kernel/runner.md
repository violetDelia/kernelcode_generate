...
- [`spec/tools/dsl_run.md`](../tools/dsl_run.md)：真实执行入口；`run_numpy_demo(...)` 通过该入口继承 runtime trance 执行日志语义。
...
- api：`class KernelNumpyDemoResult(case_name: str, dsl_result: DslRunResult, max_abs_diff: float, atol: float, rtol: float)`
- 参数：
...
- 返回值：`KernelNumpyDemoResult` 实例。
- 使用示例：

  ```python
  result = KernelNumpyDemoResult(
...
- 注意事项：该入口不编译或执行源码；需要真实运行时使用 `run_numpy_demo(...)`；若需要表达 runtime stride/dilation/padding/tile，应优先传入 `SymbolDim` 作为编译期形参，并由 `dsl_run(...)` / `run_numpy_demo(...)` 用整数 `real_args` 绑定真实值。
