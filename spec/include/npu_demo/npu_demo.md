...
- `npu_demo::add/sub/mul/truediv/min/max/...`、`npu_demo::launch(...)`、`npu_demo::block_id()`、`npu_demo::build_contiguous_stride(...)`、`npu_demo::view(...)`、`npu_demo::alloc(...)`、`npu_demo::fill(...)`、`npu_demo::slice(...)`、`npu_demo::deslice(...)`、`npu_demo::transpose(...)`、`npu_demo::store(...)`、`npu_demo::load(...)`、`npu_demo::broadcast(...)` 以及 `npu_demo::cost::add/min/max/copy/slice/deslice/...` 是 public function 的唯一成功消费方向；`detail` 只服务实现内部。
...
- `gen_kernel(target="npu_demo")` 生成的 layout 参数必须使用 brace-list public overload，形如 `{...} /*shape*/`、`{...} /*stride*/`、`{...} /*offset*/`、`{...} /*size*/`；完整 generated source 中不得泄漏 `Vector(...)`、`Vector{...}` 或局部 `long long *_shape[]` / `*_stride[]` layout buffer。
