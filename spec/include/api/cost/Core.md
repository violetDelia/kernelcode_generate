...
- 当前公共层收口 `npu_demo::cost::CostKind`、`npu_demo::{DMA, DMA1, DMA2, DMA3, DMA4, MAC, VECTOR1, VECTOR2}` 模板实参别名，以及“全部 cost helper 返回 `S_INT`”这三类基础合同。
...
- 固定生成源码使用的 kind 别名：`npu_demo::{DMA, DMA1, DMA2, DMA3, DMA4, MAC, VECTOR1, VECTOR2}`。
...
- 本规范只接受 `DMA/DMA1/DMA2/DMA3/DMA4/MAC/VECTOR1/VECTOR2` 公开 kind 别名；不再公开 `Compute`、`Memory`、`Kind2`、`Kind3`、`All` 或 target 配置驱动的额外 kind。
...
- 注意事项：稳定枚举成员仅包含 `DMA1`、`DMA2`、`DMA3`、`DMA4`、`MAC`、`VECTOR1`、`VECTOR2`；`npu_demo::DMA` 是 namespace 兼容别名，不是枚举成员。生成源码不得继续出现 `Compute`、`Memory`、`Kind2`、`Kind3` 或其他旧枚举值。
...
```
- 功能说明：保留既有 `DMA` 兼容常量。
- 注意事项：`DMA` 等价于 `npu_demo::cost::CostKind::DMA1`，用于兼容既有 include 消费者；新增生成逻辑优先使用 `DMA1/DMA2/DMA3/DMA4` 精确 kind。
...
- 验证 `CostKind::{DMA1, DMA2, DMA3, DMA4, MAC, VECTOR1, VECTOR2}`、`DMA/DMA1/DMA2/DMA3/DMA4/MAC/VECTOR1/VECTOR2` 公开别名与 `S_INT` 返回合同可独立 include、可模板实例化。
...
| --- | --- | --- | --- | --- | --- | --- |
| TC-COST-CORE-001 | 公开入口 | 独立 include `include/api/cost/Core.h` | 按 spec 声明的导入路径、CLI 参数、注册名或命名空间访问公开入口。 | 运行 `test_include_api_cost_core_exports_npu_demo_cost_kinds`。 | 公开入口暴露七值 kind，保留 `npu_demo::DMA` 兼容别名，且旧 `compute/memory` 别名不可达。 | `test_include_api_cost_core_exports_npu_demo_cost_kinds` |
