...
定义 include/api 层统一对外 DMA 操作 API 头文件规范（`include/api/Dma.h`），当前公共层只收口 `slice` 与 `deslice` 两个公开接口，面向后端无关的 `Memory<Space, T>` 与 `Vector` 抽象。

- `slice(target, source, offset, size, stride)`：定义把源区域切片读取到预分配 `target` 的公开接口。
- `deslice(target, source, offset, size, stride)`：定义将源块写回目标区域的公开接口。
- `view` 与 `reshape` 已移动到 `Memory` 的成员接口，不再保留 `view(source, ...)` 这种公共层自由函数。
...
- 明确删旧边界：`view` / `reshape` 不再属于 DMA 公共层；`deslice(source, target, ...)`、`store(...)` 等旧公开形态退出本轮稳定口径。
...
- 本规范不定义 `alloc/free/copy/cast/load/store/broadcast/transpose` 的稳定公开接口；这些语义若实现暂存，也不属于 S1 稳定公共层。
- `offset/size/stride` 的公开参数类型统一为 `Vector`；不得把 `std::vector<long long>`、`std::array<long long, N>`、裸 `long long[N]` 直接暴露成稳定公开签名。
- `slice/deslice` 的接口面向 `Memory<Space, T>`；不使用 `memory<rank, type>`、`memory<float>` 之类模板占位语言作为公开契约描述。
- `view` 与 `reshape` 已在 `Memory` 公共层收口；`Dma` 不再公开 `view(source, ...)` 这种自由函数。
- `deslice` 固定为 `target-first`；不得继续保留 `deslice(source, target, ...)` 这种旧顺序公共口径。
...
Status status = slice(target, source, offset, size, stride);
```
...
- 高层 `operation.slice(source, offset, size, stride, space)` 若采用表达式式返回值，必须在 lowering 时先显式补出 `alloc(target)`，再桥接到本接口。
...
Status status = deslice(target, source, offset, size, stride);
```
...
- 执行命令：`pytest -q test/include/api/test_dma.py`
- 测试目标：
  - 验证 `slice(target, source, ...)` 是当前公共层稳定读切片接口。
  - 验证 `deslice(target, source, ...)` 是当前公共层稳定写回接口。
  - 验证 `view(source, ...)` 与 `deslice(source, target, ...)` 不再作为公共层稳定口径出现。
- 功能与用例清单：
  - `API-DMA-001`：`slice(target, source, ...)` 的最小目标式语义可工作。
  - `API-DMA-002`：`deslice(target, source, ...)` 的最小目标式语义可工作。
