...
- 支持 `json` 与 `txt` 两类来源；其中 `npu_demo` 的公开真源是 `kernel_gen/target/targets/npu_demo.txt`，`cuda_sm89` 的公开真源是 `kernel_gen/target/targets/cuda_sm89.txt`，不再以 Python 内置模板形式对外承诺。
...
- 注意事项：返回值是本次加载集合；target 名称必须与文件名不含扩展名部分一致；`npu_demo` 的公开真源是目录中的 `npu_demo.txt`，`cuda_sm89` 的公开真源是目录中的 `cuda_sm89.txt`；加载会注册解析出的 target，重复加载默认目录必须保持幂等。
...
hw.sm_memory_size=0
hw.lm_memory_size=0
hw.tsm_memory_size=2097152
hw.tlm1_memory_size=524288
hw.tlm2_memory_size=1048576
hw.tlm3_memory_size=1048576
```
...
### `cuda_sm89.txt` 目录 target

- `cuda_sm89` 的标准注册入口是 `kernel_gen/target/targets/cuda_sm89.txt`；调用方通过 `load_targets(...)` 从目录加载，registry 不再把 CUDA target 当作 Python 内置模板对外承诺。
- `cuda_sm89.txt` 的字段使用普通 TXT 语法，字段与 `npu_demo.txt` 同类：`name`、`arch.supported_ops`、`arch.unsupported_ops` 与 `hw.*`。

`kernel_gen/target/targets/cuda_sm89.txt` 的推荐内容语义等价于：
...
- `cuda_sm89` 使用显式白名单；除上述 `arch.get_*` 查询、`arch.get_dynamic_memory`、`arch.barrier` 与 `arch.launch` 外，其他能力查询固定判定为未启用。
- `thread_num=256` 表示首版 block 级 CUDA launch 的线程能力上限；`subthread_num=32` 表示 CUDA warp 分组能力上限。
- `sm_memory_size=49152` 与 `tsm_memory_size=49152` 表示 SM89 首版 shared-memory staging 容量；`tlm1/tlm2/tlm3_memory_size=0` 表示 TLM fragment 不通过 registry 暴露动态 byte-pool 容量。
- `cuda_sm89` 不新增 `warp_num`、`compute_capability`、`shared_memory_size`、`mma_m`、`mma_n` 或 `mma_k` 硬件字段；这些能力由 CUDA include / emit / pipeline 合同承接。
- `cuda_sm89` 的标准查询入口是 `is_arch_op_supported(...)`、`get_target_hardware(...)` 与 `get_current_target_hardware(...)`。
...
- 当 `target="cuda_sm89"` 时，`thread_num=256` 与 `subthread_num=32` 必须作为 CUDA block / warp 能力上限读取；`TSM` 对应 shared-memory staging，`TLM1/TLM2/TLM3` 不得被解释为动态 byte-pool 容量。
- 当 `target="cuda_sm89"` 时，`arch.launch` 与 `arch.barrier` 通过 `is_arch_op_supported(...)` 查询必须返回已启用；旧名 `arch.launch_kernel` 必须保持未启用。
...
- 测试文件：`test/target/test_cuda_sm89_registry.py`
- 执行命令：`PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. pytest -q test/target/test_registry.py test/target/test_cuda_sm89_registry.py`
