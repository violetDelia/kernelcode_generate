...
- 只有当某条 spec 显式声明 `exact` 或“内部表达式结构完全一致”口径时，调用方才可要求比较底层表达式结构；未显式声明时，默认不得把结构级比较视为 `Memory` 长期公开合同。
...
- 若某条 spec 必须比较“内部表达式结构完全一致”，必须在该条合同中显式写明 `exact`；未声明时不得从本文件推导出结构级强约束。
