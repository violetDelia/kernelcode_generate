...
- 明确 `+`、`-`、`*`、`/`、`//`、`min(lhs, rhs)` 与 `max(lhs, rhs)` 在静态值、动态值、可约表达式、链式除法与链式整除下的公开行为，为实现与测试同步收口提供直接依据。
...
- 字符串表达式仅允许完整语法的整数算术、`floor(arg)`、`min(lhs, rhs)` 与 `max(lhs, rhs)`；含独立匿名未知 token `?` 的合法表达式可保守传播为 `?`，但 `"? +"`、`"? bad"`、`"? **"`、`"min(?, )"`、`"max(?, )"`、`"? < 2"`、`"? and 2"`、`"[?]"` 等语法不完整、arity 不合法或超出公开整数算术范围的输入必须先抛出 `ValueError`，不得吞成匿名未知维度。
- 浮点构造输入与浮点算术操作数均不受支持：
