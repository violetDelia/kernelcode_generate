...
- `tuner.cost` 的 `cost_kind` 接受任意非空字符串 attr，`op_name` 必须为非空字符串。
...
- `cost_kind(str attr)`：当前 cost function 的统计视角，接受任意非空字符串；枚举或排序由外层 pass 决定。
- `op_name(str attr)`：原 op 名称，例如 `"dma.alloc"`、`"kernel.matmul"`、`"arch.barrier"`。

使用示例：

```text
%cost0 = tuner.cost(%tile_m, %k) {cost_kind = "latency", op_name = "dma.copy"} : (!symbol.int<"TILE_M">, !symbol.int<"K">) -> !symbol.int<"LOCAL">
%cost1 = tuner.cost() {cost_kind = "bandwidth", op_name = "arch.barrier", scope = #arch.scope<global>} : () -> !symbol.int<"BARRIER_COST">
```
...
- 验证 `tuner.cost` 的 parse/print round-trip、operand 透传、`!symbol.int<"...">` 结果类型、metadata attr 校验与错误路径。
...
| TC-TUNER-005 | `tuner.cost.cost_kind` 必须为非空字符串 attr，并拒绝旧 `kind / device_func` attrs | `test_tuner_cost_rejects_invalid_kind_attrs` |
