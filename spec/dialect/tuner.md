...
- 超参数标量统一返回 `!symbol.dim<"name">`，与 `SymbolDim` 的符号整数语义保持一致。
...
- [`spec/dialect/symbol.md`](../../spec/dialect/symbol.md)：提供符号整数标量语义；`!symbol.dim<"name">` 作为符号维度标量类型由 `symbol dialect` 维护。
...
- 结果类型必须为 `!symbol.dim<"name">`；不得使用 builtin `index`、普通整数或其他类型替代。
