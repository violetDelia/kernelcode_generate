...
- 明确 `NnLoweringPass` 面向 `kernel.binary_elewise`、`kernel.exp`、`kernel.reduce(kind=...)`、`kernel.matmul`、`kernel.img2col1d`、`kernel.img2col2d` 的目标 op 名字与输出消费链路；`kernel.reduce_sum/min/max` 仍保留为具名 op，但 NnLoweringPass 产出以 `kernel.reduce(kind=...)` 为准。
...
- 上述旧具名逐元素公开名已不再属于 `kernel` dialect 的公开合同。
...
- 验证 `kernel.binary_elewise / kernel.exp / kernel.reduce / kernel.reduce_* / kernel.matmul / kernel.img2col*` 的 op 名字、关键 attrs 与 `out` 消费链路合同。
...
| TC-KRN-004 | `kernel.binary_elewise(kind="add")` 正常路径 | `test_kernel_binary_elewise_add_success` |
| TC-KRN-005 | `kernel.binary_elewise(kind="add")` layout 或 dtype 不一致报错 | `test_kernel_binary_elewise_add_layout_mismatch` |
| TC-KRN-006 | `kernel.binary_elewise(kind="eq")` 输出类型为 `i1` | `test_kernel_binary_elewise_compare_output_type` |
| TC-KRN-007 | `kernel.binary_elewise` 比较输出类型或 kind 非法 | `test_kernel_binary_elewise_compare_output_type_error` |
