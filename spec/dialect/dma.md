...
用于定义 `dma dialect` 的稳定方言语义，描述 `dma.alloc`、`dma.fill`、`dma.free`、`dma.copy`、`dma.make_ring`、`dma.current_ring`、`dma.advance_ring`、`dma.load`、`dma.store`、`dma.slice`、`dma.deslice`、`dma.subview`、`dma.view`、`dma.reshape`、`dma.reinterpret`、`dma.cast` 以及作为 lowering 目标面的 `dma.broadcast`、`dma.transpose`，用于表示内存对象之间的数据搬运、标量物化、ring stage 轮转与布局转换，包括整块搬运、切片读取、切片回写、跨空间迁移、标量写入临时 memory、显式广播物化、显式转置物化、一维 byte backing 切分、视图变换、统一 alias 重解释与显式数据转换，并支持动态 shape 的表达。该方言不单独定义 memory type / memory space，而是统一复用 `nn dialect` 中的 `NnMemoryType` 与 `NnMemorySpaceAttr`。
...
- 为 builtin 数值标量 / `!symbol.int<#symbol.expr<expr>>` 到临时 memory 的真实物化提供稳定方言层原语，避免只生成空 `dma.alloc` 占位。
...
- 当前方言范围包含 `dma.alloc`、`dma.fill`、`dma.free`、`dma.copy`、`dma.make_ring`、`dma.current_ring`、`dma.advance_ring`、`dma.broadcast`、`dma.transpose`、`dma.load`、`dma.store`、`dma.slice`、`dma.deslice`、`dma.subview`、`dma.view`、`dma.reshape`、`dma.reinterpret`、`dma.cast`。
- 当前方言公开 xDSL `MemoryEffect` 语义，供 pass 通过 `xdsl.traits.get_effects(op)` 机械判定 memory 生命周期：
  ...
  - `dma.view/subview/reshape/reinterpret` 仅生成 memory view/result，不读写数据，必须通过 `NoMemoryEffect` 暴露无副作用语义。
...
- `dma.fill.value` 不属于 index-like 布局 operand。当前公开口径接受 builtin 非 bool 整数、builtin 浮点或 `!symbol.int<#symbol.expr<expr>>`；`target.element_type` 必须是非 bool 数值 dtype，builtin 整数只匹配整数 memory，builtin 浮点只匹配浮点 memory，`!symbol.int` 可填充非 bool 数值 memory。
...
- `dma.reinterpret` 接受单个线性 `offset` operand 与 variadic `shape/stride` operand，均以 `!symbol.int<#symbol.expr<expr>>` 表达；`offset` 额外允许 `!symbol.iter<start = #symbol.expr<0>, end = #symbol.expr<N>, step = #symbol.expr<1>>`。`shape/stride` 必须与 result type exact 匹配。非 byte pool source 的 `offset` 单位是 source element；一维 `i8` byte pool source 的 `offset` 单位是 byte。
- `memory-plan{auto-pad=true}` 可生成 typed padded backing 上的 `dma.reinterpret(offset=0, logical_shape, logical_stride)`；该场景仍完全服从本条 `dma.reinterpret` source/result space、dtype、shape/stride exact match 与 bounds 规则，不新增 `dma` 方言 API 或特殊 verifier 例外。
...
- 对 mixed add 等需要 `scalar -> memory` 合法化的链路，当前唯一公开合法原语是 `dma.alloc + dma.fill(target, value)`：`dma.alloc` 负责生成 temporary memory，`dma.fill` 负责把 builtin 数值标量 / `!symbol.int<#symbol.expr<expr>>` 真实写入该 memory 的每个逻辑元素。仅生成空 `dma.alloc` 占位，或生成 `dma.fill` 后其 `target` 在下游 IR 中 `users=[]`，都不属于当前链路的通过口径。
...
| `fill(target, value)` / pass 侧合法化原语 | `dma.fill` | 将 builtin 数值标量 / `!symbol.int<#symbol.expr<expr>>` 真实写入 `target` 的每个逻辑元素；用于公开 fill helper 与 mixed add 的 `scalar -> memory` 合法化。 |
...
| `（无直接 DSL helper；pass 侧合法化原语）` | `dma.subview` | 从一维 `i8` backing memory 按目标 dtype 元素单位切出一维 contiguous typed memory；当前保留供 pass / 测试显式使用，主 pipeline 由 `dma-alias-to-reinterpret` 归一到 `dma.reinterpret`。 |
| `view(source, offset, size, stride)` | `dma.view` | 视图重解释，`offset/size/stride` 分别映射为 `dma.view` 的 `offsets/shape/stride` operand；`result_type.shape == size`；非 byte pool 场景下 `result_type.stride == source.stride * stride`，静态可判定时要求 `source/result` `numel` 一致；byte pool 场景按 target dtype 元素单位解释 `offset/size/stride` 并校验 typed 子区间字节边界；若该结果直接返回，则 `func.return` 类型必须与同一 `result_type` 一致。 |
| `reshape(source, shape)` | `dma.reshape` | 连续布局 reshape。 |
| `flatten(source)` | `dma.reshape` | 视为 `reshape` 到一维形状。 |
| `（无直接 DSL helper；pass 侧归一化原语）` | `dma.reinterpret` | 在同一 backing memory 上按线性 offset、result shape 与 result physical stride 构造新的 memory 描述；由 `dma-alias-to-reinterpret` 与 `memory-pool` rewrite 生成。 |
...
- `dma.fill` 是 dialect / pass 层公开原语，也是公开 DSL `fill(target, value)` helper 的下沉目标。对 `nn.add(memory, const(i32) / symbol.int)` 这类 mixed add 路径，当前最小合法 lower 片段必须显式包含 `dma.alloc + dma.fill`，且被填充的 temporary memory 必须在后续 IR 中被实际消费。
- operation 层允许“静态可判定的缩小 subview”（`size` 的 `numel` 小于 `source.shape`），但当前非 byte pool `dma.view` 在静态可判定时要求 `source/result` `numel` 一致；该场景不属于当前非 byte pool `dma.view` 的可验证映射子集。byte pool `dma.view` 允许 typed 子区间小于 source byte pool。
...
- `dma.fill` 中 `target` 不是 `!nn.memory<...>`、`target.element_type` 不是非 bool 数值 dtype、`value` 不是 builtin 非 bool 整数 / builtin 浮点 / `!symbol.int<#symbol.expr<expr>>`，或 builtin value dtype 与 target dtype family 不匹配时必须报错。
...
- `dma.view` 中 `source/result` 的 `space` 不一致必须报错；`element_type` 不一致仅在 source 为一维 `i8` byte pool 时允许；非 byte pool 可判定的 `numel` 不一致必须报错；byte pool 场景不要求 source/result 字节数相等，只按 typed 子区间字节边界判断。
- `dma.reshape` 中 `source/result` 的 `space` 或 `element_type` 不一致必须报错；`source/result` rank 可以不同，但静态可判定的 `numel` 不一致必须报错。
- `dma.reinterpret` 中 `source/result` 的 `space` 不一致必须报错；非 byte-pool source 的 `element_type` 不一致必须报错；一维 `i8` byte-pool source 允许 result 使用其它公开支持 dtype。非 byte-pool typed `dma.reinterpret` 允许 rank-changing 与 partial alias，不要求 `source/result` rank 或 `numel` 相等，仅按 `shape/stride` exact match 与静态 bounds 判断合法性；byte pool 场景同样不要求 source/result 字节数相等，只按 typed 子区间字节边界判断。
...
- `dma.reinterpret` 的 `offset` 必须是非负 `!symbol.int<#symbol.expr<expr>>` 或 `!symbol.iter<start = #symbol.expr<0>, end = #symbol.expr<N>, step = #symbol.expr<1>>`；`shape/stride` operand 数量必须等于 result rank，且必须与 result type 的 shape/stride exact 匹配；静态可判定时，非 byte pool source 需满足 `offset + max_index(shape, stride) < source_numel`，byte pool source 需满足 `offset + (max_index(shape, stride) + 1) * sizeof(result.element_type) <= source_bytes`。
...
- `dma` 的布局/索引类标量输入以 `!symbol.int<#symbol.expr<expr>>` 为主，`offsets` 额外允许 `!symbol.iter<start = #symbol.expr<0>, end = #symbol.expr<N>, step = #symbol.expr<1>>`；parse/print 不得再使用 builtin `index` 作为这些 operand 的公开文本语义。`dma.fill.value` 是当前唯一例外，公开口径允许 builtin 非 bool 整数、builtin 浮点与 `!symbol.int<#symbol.expr<expr>>`。
## API详细说明
...
- 表示将单个数值标量真实写入 `target` 的每个逻辑元素，完成 `scalar -> memory` 物化。
- 当前用于公开 DSL `fill` helper 与 mixed add 等链路，把 builtin 数值标量 / `!symbol.int<#symbol.expr<expr>>` 合法化为可被下游 IR 消费的 temporary memory。

- 参数：

- `target`：被写入的目标内存，类型为 `!nn.memory<...>`。
- `value`：待物化的数值标量，接受 builtin 非 bool 整数、builtin 浮点或 `!symbol.int<#symbol.expr<expr>>`。
...
- `target.element_type` 必须是非 bool 数值 dtype；bool memory 不属于 `dma.fill` 目标。
- 当 `value` 为 builtin 整数时，target 必须是整数 memory；当 `value` 为 builtin 浮点时，target 必须是浮点 memory；当 `value` 为 `!symbol.int<#symbol.expr<expr>>` 时，target 可以是任意非 bool 数值 memory。
- 上层 DSL `fill(...)` helper 的字符串字面量只允许表示正无穷 / 负无穷两种规范语义；文面固定为 `"inf"` 与 `"-inf"`，并在 MLIR 层下沉为对应浮点 constant + `dma.fill`，不得放宽到其他大小写、别名或任意字符串。
...
- `dma.fill` canonicalization 可在 `kernel-decompose` 已经把 `kernel.matmul_fusion` 显式化为 dynamic acc `kernel.matmul` 后删除 acc initial zero fill。该规则只识别 `dma.fill(%acc, 0) -> symbol.for -> kernel.matmul(..., %is_acc)`，其中 `%is_acc` 必须按 SSA 身份来自 `symbol.ne(iter_arg, loop.start)`，fill target 必须是 matmul out 或 target/root 的一跳 `dma.view` / `dma.subview` / `dma.reshape` / `dma.reinterpret` alias，fill 到 loop 之间和 loop body 起点到 matmul 之间不得读取、写入、逃逸 target/alias，也不得出现 unknown effect 或 nested region。
- dynamic acc matmul dead-fill 的 loop 至少执行一次证明仅支持两类：`start/end/step` 全静态且 `end > start`、`step > 0`；或 `start/end` 静态且 `end > start`，`step` 为用户已确认必须为正数的 runtime tile 参数符号（如 `TILE_K`）。不假设 dynamic end（如 `K > 0`），dynamic/dynamic matmul 的 acc fill 不作为必删项。
- `dma.fill` canonicalization 可删除 local `dma.alloc` target 上的 safe-if dead fill：后续 `scf.if` 必须无 results，then/else 均为单 block（无 else 按空分支处理），分支内 target 或一跳 alias 的读取必须由同分支内先出现的 full `dma.deslice(target, ...)` 支配；无 full overwrite 的分支不得读取 target/alias。if 后必须继续同 block 扫描，不能把 `scf.if` 本身当成全路径 full overwrite。
- `dma.fill` canonicalization 遇到 target 在覆盖前被读取、partial writer、region op、unknown side effect、self-copy、target-derived alias read、memory-source broadcast、`kernel.matmul_fusion`、非零 acc fill、非 canonical acc、dynamic end、多跳 alias、target/alias escape 或无法证明完整覆盖的 op 时必须保留前序 `dma.fill`。

- 返回值：

- 返回类型为无返回值；当前 op result 数量固定为 `0`。
- 当前公开子集覆盖非 bool 整数、浮点与 `!symbol.int<#symbol.expr<expr>>` 的标量物化；若后续需要 bool memory fill 或隐式 dtype promotion，必须新增独立 spec 收口。
...
- api：`class DmaRingType(memory_type: NnMemoryType)`
- 功能说明：描述 DMA ring 的单个 slot memory type。
- 参数：
...
- api：`class DmaCurrentRingOp(ring: SSAValue | Operation, result_type: NnMemoryType | None = None)`
- 功能说明：读取当前 DMA ring stage 对应的 memory slot。
- 参数：
  - `ring`：`DmaRingType` 结果。
  - `result_type`：可选；省略时使用 ring slot memory type。
- 返回值：当前 stage 的 `!nn.memory<...>`。
- 注意事项：显式 `result_type` 必须与 ring slot memory type 完全一致。

### `class DmaAdvanceRingOp(ring: SSAValue | Operation, result_type: NnMemoryType | None = None)`

- api：`class DmaAdvanceRingOp(ring: SSAValue | Operation, result_type: NnMemoryType | None = None)`
- 功能说明：推进 DMA ring 到下一 stage，并返回推进后的 slot 类型占位。
- 参数：
  - `ring`：`DmaRingType` 结果。
  - `result_type`：可选；省略时使用 ring slot memory type。
- 返回值：推进后 stage 的 `!nn.memory<...>`。
- 注意事项：该 op 具有副作用，不属于 Pure；显式 `result_type` 必须与 ring slot memory type 完全一致。
...
- 若 `source.shape` 与 `result.shape` 的元素总数可判定不一致，非 byte pool 场景必须报错；byte pool 场景不要求字节数相等，`offset/size/stride` 按 target dtype 元素单位解释。
...
- api：`class DmaReinterpretOp(source: SSAValue | Operation, offset: SSAValue | Operation, shape: Sequence[SSAValue], stride: Sequence[SSAValue], result_type: NnMemoryType)`

- 功能说明：

- 表示同一 backing memory 上的统一 alias 重解释。
- 不搬运数据，不读写 memory，只用线性 offset、结果 shape 与结果 physical stride 构造新的 `!nn.memory<...>` 描述。

- 参数：

- `source`：源内存，类型为 `!nn.memory<...>`。
- `offset`：单个 `!symbol.int<#symbol.expr<expr>>` 或 `!symbol.iter<start = #symbol.expr<0>, end = #symbol.expr<N>, step = #symbol.expr<1>>` operand。
- `shape`：variadic `!symbol.int<#symbol.expr<expr>>` operand，按 result rank 顺序提供结果 shape。
- `stride`：variadic `!symbol.int<#symbol.expr<expr>>` operand，按 result rank 顺序提供结果 physical stride。
- `result_type`：结果类型，必须为 `!nn.memory<...>`。

- 使用示例：

```python
op = DmaReinterpretOp(source, offset, shape, stride, result_type)
```

- 注意事项：

- `source.space` 必须与 `result_type.space` 一致。
- 非 byte pool source 要求 `source.element_type == result_type.element_type`。
- 当 `source` 是一维 `i8` byte pool 时，`result_type.element_type` 可以是其它公开支持 dtype，且 `offset` 按 byte 解释；其它 source 的 `offset` 按 source element 解释。
- 非 byte pool typed reinterpret 允许 rank-changing 与 partial alias，不要求 `source/result` rank 或 `numel` 相等；合法性由 same element_type、same space、shape/stride exact match 与静态 bounds 共同约束。
- `shape` operand 数量必须等于 result rank，且 operand 类型表达必须与 `result_type.shape` exact 匹配。
- `stride` operand 数量必须等于 result rank，且 operand 类型表达必须与 `result_type.stride` exact 匹配。
- 静态可判定时，非 byte pool source 必须满足 `offset + max_index(shape, stride) < source_numel`；byte pool source 必须满足 `offset + (max_index(shape, stride) + 1) * sizeof(result.element_type) <= source_bytes`。
- `dma.reinterpret` 由 `dma-alias-to-reinterpret` pass 和 `memory-pool` rewrite 生成；高层 DSL helper 仍可继续产生 `dma.view` / `dma.reshape`，由 pass 归一化。

- 返回值：

- 返回新的 `!nn.memory<...>` 结果块；当前 op result 数量固定为 `1`。
- 仅表示 alias 重解释，不改变数据内容。
...
- 验证 `dma.fill` 能将 builtin 数值标量 / `!symbol.int<#symbol.expr<expr>>` 真实写入非 bool 数值 memory，并锁定它不是“空 `dma.alloc` 占位”的替代说法。
- 验证 `dma.free` 的内存类型约束与无返回值语义。
- 验证 `dma.ring` / `dma.make_ring` / `dma.current_ring` / `dma.advance_ring` 的 parse、verifier、side-effect 保留与 result type 边界。
...
- 验证 `dma.fill` dynamic acc matmul dead-fill canonicalization 删除静态正 trip count 与 positive runtime tile step 下的 acc zero fill，并保留 `kernel.matmul_fusion`、非零 fill、非 canonical acc、dynamic end、loop 前 read 与 body 内 matmul 前 read 场景。
- 验证 local alloc safe-if dead-fill canonicalization 删除 full `dma.deslice` 支配 alias read 且 absent path 不读的 fill，并保留 else 未覆盖读取、partial deslice、nested region 和 target 非 local alloc 的场景。
...
| TC-DMA-014C3 | pass 改写 | `dma.fill` dynamic acc matmul dead-fill | 准备 `fill -> symbol.for -> kernel.matmul(dynamic_acc)` 的静态正 trip count 与 `TILE_K` runtime step IR。 | 运行 `test_dma_fill_canonicalization_removes_zero_fill_before_static_trip_dynamic_acc_matmul` 与 `test_dma_fill_canonicalization_removes_zero_fill_with_positive_symbol_step`。 | 前序 acc zero fill 被 `CanonicalizePass` 删除，`kernel.matmul` 保留。 | `test_dma_fill_canonicalization_removes_zero_fill_before_static_trip_dynamic_acc_matmul`, `test_dma_fill_canonicalization_removes_zero_fill_with_positive_symbol_step` |
| TC-DMA-014C4 | pass 改写 | `dma.fill` dynamic acc matmul 反例 | 准备 `kernel.matmul_fusion`、非零 fill、dynamic end、loop 前 target read 与 body 内 read 的公开 IR。 | 运行 `test_dma_fill_canonicalization_keeps_dynamic_acc_matmul_boundaries`。 | 前序 fill 保留，不假设 fusion 语义或 dynamic shape 正数。 | `test_dma_fill_canonicalization_keeps_dynamic_acc_matmul_boundaries` |
| TC-DMA-014C5 | pass 改写 | `dma.fill` safe-if dead-fill 正例 | 准备 local `dma.alloc` target、then 分支 full `dma.deslice` 后读取 alias、else 为空的公开 IR。 | 运行 `test_dma_fill_canonicalization_removes_fill_for_if_full_deslice_before_alias_read`。 | 前序 fill 被删除，if 后继续同 block 扫描。 | `test_dma_fill_canonicalization_removes_fill_for_if_full_deslice_before_alias_read` |
| TC-DMA-014C6 | pass 改写 | `dma.fill` safe-if dead-fill 反例 | 准备 else 未覆盖读取 alias、partial deslice、nested region 与 target 非 local alloc 的公开 IR。 | 运行 `test_dma_fill_canonicalization_keeps_unsafe_if_dead_fill_boundaries`。 | 前序 fill 保留，规则不扩展为通用 CFG DSE。 | `test_dma_fill_canonicalization_keeps_unsafe_if_dead_fill_boundaries` |
...
| TC-DMA-019G | 内存/DMA | `dma.reinterpret` typed 与 byte-pool alias | 准备同 dtype/space typed source、byte-pool source、动态 shape/stride operand、space/dtype/bounds 反例。 | 运行 `test/dialect/dma/test_reinterpret.py`。 | `DmaReinterpretOp` 公开导入、NoMemoryEffect、typed / byte-pool verifier 正例与错误文本边界均符合合同。 | `test/dialect/dma/test_reinterpret.py` |
...
| TC-DMA-062 | 内存/DMA | `dma.view/subview/reshape/reinterpret` 暴露 no-memory-effect | 准备合法 alias op。 | 运行 `test_dma_alias_ops_have_no_memory_effect`。 | `get_effects(op)` 返回空 effect 集合，确认 alias op 不读写数据。 | `test_dma_alias_ops_have_no_memory_effect` |
