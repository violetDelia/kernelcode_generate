...
- `nn.softmax` 仍然是合法输入 op；默认 lowering 链通过 `DecompassPass`（见 [`spec/pass/lowering/decompass.md`](../../spec/pass/lowering/decompass.md)）把 `nn.softmax` 分解成可继续 lowering 的 `nn` 链路，方言层不负责自动分解。
