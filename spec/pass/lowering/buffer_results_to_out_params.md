...
- `BROTP-005`：`python3 -m expectation.pass.buffer_results_to_out_params` 锁死“callee 的 memory result -> 前置 arg0；caller 补 out 实参；`func.call` 零 memory result”的公开口径。（`expectation/pass/buffer_results_to_out_params/__main__.py`）
