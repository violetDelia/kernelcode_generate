...
- `nn.matmul` 的 contracting 维度在静态维度、命名符号和 DSL 匿名 `?` 维度生成的 `runtime_dim_*` type-level 符号场景下都必须精确相等；不得把任意两个 `runtime_dim_*` 互相匹配。
- `nn.img2col1d/2d` 的结果类型若包含匿名 `?` 维度生成的 `runtime_dim_*` shape，结果 `dma.alloc` 使用 full-rank dynamic shape operands；普通命名符号结果继续使用符号子集形态。
...
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-009 | pass 改写 | `test_nn_lowering_matmul_accepts_runtime_contract_dims` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造 contracting 轴为同名 runtime type-level 维度的 matmul 输入。 | 运行 `test_nn_lowering_matmul_accepts_runtime_contract_dims`。 | lowering 接受可证明同名的 `runtime_dim_*` contracting 维度并输出 `kernel.matmul`，静态和命名符号错误边界不放宽。 | `test_nn_lowering_matmul_accepts_runtime_contract_dims` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-010 | pass 改写 | `test_nn_lowering_img2col2d_runtime_dim_result_uses_full_rank_alloc_shape` | 通过公开 `NnImg2col2dOp` 与 `NnLoweringPass` 构造结果含 `runtime_dim_*` 的 img2col2d 输入。 | 运行 `test_nn_lowering_img2col2d_runtime_dim_result_uses_full_rank_alloc_shape`。 | lowering 生成 full-rank `dma.alloc` dynamic shape，并输出 `kernel.img2col2d`。 | `test_nn_lowering_img2col2d_runtime_dim_result_uses_full_rank_alloc_shape` |
...
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-012 | 边界/异常 | `test_nn_lowering_matmul_rejects_unrelated_runtime_contract_dims` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造 contracting 轴为不同 `runtime_dim_*` 的 matmul 输入。 | 运行 `test_nn_lowering_matmul_rejects_unrelated_runtime_contract_dims`。 | lowering 拒绝不可证明相等的 runtime contracting 维度。 | `test_nn_lowering_matmul_rejects_unrelated_runtime_contract_dims` |
