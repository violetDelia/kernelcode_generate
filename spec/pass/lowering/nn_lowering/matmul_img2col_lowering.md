...
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-009 | pass 改写 | `test_nn_lowering_matmul_accepts_named_symbol_contract_dims` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造 contracting 轴为同名命名符号维度的 matmul 输入。 | 运行 `test_nn_lowering_matmul_accepts_named_symbol_contract_dims`。 | lowering 接受可证明同名的命名符号 contracting 维度并输出 `kernel.matmul`，静态错误边界不放宽。 | `test_nn_lowering_matmul_accepts_named_symbol_contract_dims` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-009A | pass 改写 | `test_nn_lowering_matmul_accepts_unknown_contract_dims` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造 contracting 轴双侧均为匿名 `?` 的 matmul 输入。 | 运行 `test_nn_lowering_matmul_accepts_unknown_contract_dims`。 | lowering 接受双侧 `?` contracting 维度并输出 `kernel.matmul`，不伪造具名 K。 | `test_nn_lowering_matmul_accepts_unknown_contract_dims` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-010 | pass 改写 | `test_nn_lowering_img2col2d_anonymous_result_uses_full_rank_alloc_shape` | 通过公开 `NnImg2col2dOp` 与 `NnLoweringPass` 构造结果含匿名 `?` 的 img2col2d 输入。 | 运行 `test_nn_lowering_img2col2d_anonymous_result_uses_full_rank_alloc_shape`。 | lowering 生成 full-rank `dma.alloc` dynamic shape，并输出 `kernel.img2col2d`。 | `test_nn_lowering_img2col2d_anonymous_result_uses_full_rank_alloc_shape` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-011 | 边界/异常 | `test_nn_lowering_matmul_public_error_matrix` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造非 memory operand、rank、contracting dim、输出 shape 与 stride 错误。 | 运行 `test_nn_lowering_matmul_public_error_matrix`。 | 非法 matmul 输入按公开 `KernelCodeError` 语义失败。 | `test_nn_lowering_matmul_public_error_matrix` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-012 | 边界/异常 | `test_nn_lowering_matmul_rejects_unrelated_symbol_contract_dims` | 通过公开 `NnMatmulOp` 与 `NnLoweringPass` 构造 contracting 轴为不同命名符号的 matmul 输入。 | 运行 `test_nn_lowering_matmul_rejects_unrelated_symbol_contract_dims`。 | lowering 拒绝不可证明相等的 contracting 维度。 | `test_nn_lowering_matmul_rejects_unrelated_symbol_contract_dims` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-013 | 边界/异常 | `test_nn_lowering_img2col1d_public_error_matrix` | 通过公开 `NnImg2col1dOp` 与 `NnLoweringPass` 构造非法参数类型、静态源轴动态输出和非法结果 rank。 | 运行 `test_nn_lowering_img2col1d_public_error_matrix`。 | 非法 img2col1d 输入按公开 `KernelCodeError` 语义失败。 | `test_nn_lowering_img2col1d_public_error_matrix` |
| TC-PASS-LOWERING-NN-LOWERING-MATMUL-IMG2COL-LOWERING-014 | 边界/异常 | `test_nn_lowering_img2col2d_public_error_matrix` | 通过公开 `NnImg2col2dOp` 与 `NnLoweringPass` 构造非法参数类型、静态源轴动态输出和非法结果 rank。 | 运行 `test_nn_lowering_img2col2d_public_error_matrix`。 | 非法 img2col2d 输入按公开 `KernelCodeError` 语义失败。 | `test_nn_lowering_img2col2d_public_error_matrix` |
