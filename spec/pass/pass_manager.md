...
- 对 softmax lowering 链固定公开一个顺序边界：`DecomposeNnSoftmaxPass` 必须运行在 `LowerNnToKernelPass` 之前，确保默认 lowering 链路不让 residual `nn.softmax` 落入 `nn_to_kernel`。
...
- 验证默认 lowering pipeline 会注册 `DecomposeNnSoftmaxPass -> LowerNnToKernelPass -> BufferResultsToOutParamsPass -> LowerDmaMemoryHierarchyPass`。
