...
- 对 tile family 固定公开一个顺序边界：`TilePass` / `TileElewisePass` / `TileReducePass` 必须运行在 `BufferResultsToOutParamsPass` 之后，并且位于 `LowerDmaMemoryHierarchyPass` 之前。
- 对 `symbol-loop-hoist` 固定公开一个 materialize 边界：当 pipeline 中存在 `tile` / `tile-elewise` / `tile-reduce` 时，`symbol-loop-hoist` 必须运行在这些 tile family materializer 之后，且位于 `lower-dma-memory-hierarchy` 之前。
...
- 当 lowering 链包含 `TilePass` / `TileElewisePass` / `TileReducePass` 时，manager 只冻结其排序边界；是否注册具体 tile family pass 仍由调用方决定。
...
- 当需要显式插入 tile family lowering 时，顺序固定扩展为：
