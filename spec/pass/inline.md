...
| TC-PASS-INLINE-003 | no-op | empty module 与 external declaration no-op 边界 | 准备空 `builtin.module` 与只含 external declaration 的公开 IR module。 | 运行 `test_inline_noops_for_empty_module_and_external_declaration`。 | 空 module 不改写；未被引用的 private declaration 可按 helper 清理规则移除，公开入口保留。 | `test_inline_noops_for_empty_module_and_external_declaration` |
| TC-PASS-INLINE-004 | no-op | public helper 与仍被引用的非 inlineable private helper 保留 | 准备公开 helper、被 caller 引用但不满足 inlineable 条件的 private helper。 | 运行 `test_inline_keeps_public_helpers_and_referenced_private_non_inlineable`。 | 公开 helper 不因未引用被删除；仍被 caller 引用的 private helper 不被清理。 | `test_inline_keeps_public_helpers_and_referenced_private_non_inlineable` |
| TC-PASS-INLINE-005 | 边界/异常 | self-recursive inlineable call 拒绝 | 准备单 block self-call `func.func`。 | 运行 `test_inline_rejects_self_recursive_inlineable_call`。 | pass 不内联自递归调用，最终以 `InlineError: unresolved func.call remains after inline` 失败。 | `test_inline_rejects_self_recursive_inlineable_call` |
| TC-PASS-INLINE-006 | 边界/异常 | func.call 参数数量与 callee 形参数量不一致 | 准备 helper 形参和 caller `func.call` 实参数量不一致的公开 IR。 | 运行 `test_inline_rejects_call_argument_arity_mismatch`。 | pass 以 `InlineError: func.call arity mismatch for 'helper'` 失败。 | `test_inline_rejects_call_argument_arity_mismatch` |
| TC-PASS-INLINE-007 | 边界/异常 | func.call 结果数量与 callee return 数量不一致 | 准备 helper 返回值和 caller `func.call` 结果数量不一致的公开 IR。 | 运行 `test_inline_rejects_call_result_arity_mismatch`。 | pass 以 `InlineError: func.call result arity mismatch for 'helper'` 失败。 | `test_inline_rejects_call_result_arity_mismatch` |
