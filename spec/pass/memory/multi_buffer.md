...
- `multi-buffer-analysis` 只分析可证明的 loop staging / scratch `dma.alloc/free` 生命周期，并在原 `dma.alloc` 上写三项临时属性。
...
- `MultiBufferAnalysisPass`：
  - `memory_stage` 必须为正整数；`target is None` 时写入固定 `multi_buffer.num = str(memory_stage)`。
  - `target` 为 `None` 或非空字符串；`target` 非空时 same-loop staging 写入 `multi_buffer.num = "auto"`。
...
- analysis 遍历可证明的 `dma.alloc/free` lifecycle，记录 update point、use point 和 num。
- analysis 不删除、不插入、不替换业务 IR；输出只允许在候选 `dma.alloc` 上新增三项属性：

```text
multi_buffer.update_point = "loopN"
multi_buffer.use_point = "loopN"
multi_buffer.num = "auto" | "<positive-int>"
```

- same-loop staging：
  - `target` 非空时写 `"auto"`。
  - `target is None` 时写 `str(memory_stage)`。
...
- apply 重新校验：
  ...
  - update/use point 与当前可证明 loop region 一致；
...
- loop staging / scratch direct alias 与 direct use：
  ...
  - 所有 access op 必须位于同一个最近 `symbol.for` 的直接 body，不得跨 nested/sibling region 或 loop 外 escape。
