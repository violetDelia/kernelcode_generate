...
- `target` 路径中按 slot memory space 分组，同一 space 使用 `target_space_bytes // sum(slot_span_bytes in this space)` 的共享 num，不同 space 各自使用本 space capacity；backing bytes 仍为每个 slot 的 `num * offset`。
