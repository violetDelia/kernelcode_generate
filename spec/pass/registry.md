...
- `multi-buffer` 接受 pass 专属 `options={"memory-stage": "<positive-int>", "target": "<target-name>"}`；`fold` 仍由 registry 通用 option 处理。`target` 非空时优先于 `memory-stage` 计算 ring `num`。`memory-stage` 非整数、`<= 0`、`target` 为空或未知 option 必须由 `MultiBufferPass.from_options(...)` 失败并由 registry 保留为 `PassRegistryError: pass 'multi-buffer' option error: <原因>`；直接调用 `MultiBufferPass.from_options({"fold": "false"})` 必须失败，不能把通用 `fold` 兼容进 pass 专属 options。
...
- `kernel-decompose` 第一版不接受 pass 专属 option；`fold` 仍由 registry 通用 option 处理。未知 option 必须由 `KernelDecomposePass.from_options(...)` 失败并由 registry 保留为 `PassRegistryError: pass 'kernel-decompose' option error: <原因>`，内层原因包含 `kernel-decompose options unknown: <name>`。
...
| TC-PASS-REGISTRY-023D | 公开入口 | multi-buffer memory-stage/target/fold options | 加载内置 pass，验证默认 `memory_stage=2`，并提供 `memory-stage=4`、`target=npu_demo` 和 registry 通用 `fold=false` option。 | 运行 `test_build_registered_multi_buffer_options`。 | registry 构造默认 `MultiBufferPass(memory_stage=2)`；显式 option 构造 `memory_stage=4`、`target=npu_demo`、`fold=False`。 | `test_build_registered_multi_buffer_options` |
| TC-PASS-REGISTRY-023E | 边界/异常 | multi-buffer options 非法值 | 准备未知 option、非整数/非正 `memory-stage`、空 `target` 或 direct `from_options({"fold": "false"})`。 | 运行 `test_build_registered_multi_buffer_rejects_invalid_options`。 | direct from_options 与 registry 包装错误均按公开错误语义失败。 | `test_build_registered_multi_buffer_rejects_invalid_options` |
