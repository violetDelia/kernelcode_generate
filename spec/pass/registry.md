...
| TC-PASS-REGISTRY-023D | 公开入口 | multi-buffer memory-stage/target/fold options | 加载内置 pass 并提供 `memory-stage=3`、`target=npu_demo` 和 registry 通用 `fold=false` option。 | 运行 `test_build_registered_multi_buffer_options`。 | registry 构造 `MultiBufferPass`，`memory_stage=3`、`target=npu_demo`、`fold=False`。 | `test_build_registered_multi_buffer_options` |
