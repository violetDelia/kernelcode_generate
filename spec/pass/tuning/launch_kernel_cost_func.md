...
- 若仓库后续需要重新引入历史四 kind companion 资产，或扩展当前目录入口覆盖范围，必须拆到单独的合同资产处理；保留合同以 open-kind 产品 spec/pytest/实现 与 tracked `compute / memory` expectation 入口分层定义。
...
%total = symbol.for %i = %start to %end step %step iter_args(%acc = %zero) {iter = #symbol.iter<start = "0", end = "M", step = "TILE_M">} -> !symbol.int<"0"> {
  %local = tuner.cost(value) {cost_kind = "compute", op_name = "kernel.add"} : (value) -> !symbol.int<"LOCAL">
...
- api：`class LaunchKernelCostFuncPass(cost_kind: str = "DMA|MAC", fold: bool = True)`
- 参数：
  - `cost_kind`：`cost_kind` 输入值，参与 `LaunchKernelCostFuncPass` 的公开处理流程；类型 `str`；默认值 `"DMA|MAC"`；不允许 `None` 或空值作为稳定输入，除非本接口 `注意事项` 另有明确说明；按值或只读语义消费，调用方不得依赖输入对象被修改；非法值按该 API 的公开错误语义处理。
...
- 使用示例：
  ...
  ```python
  launch_kernel_cost_func_pass = LaunchKernelCostFuncPass(cost_kind="DMA|MAC", fold=True)
  ```
...
- 使用示例：
  ...
  pass_obj = LaunchKernelCostFuncPass(cost_kind="DMA|MAC")
...
- api：`__init__(cost_kind: str = "DMA|MAC", fold: bool = True) -> None`
- 参数：
  - `cost_kind`：`cost_kind` 输入值，参与 `__init__` 的公开处理流程；类型 `str`；默认值 `"DMA|MAC"`；不允许 `None` 或空值作为稳定输入，除非本接口 `注意事项` 另有明确说明；按值或只读语义消费，调用方不得依赖输入对象被修改；非法值按该 API 的公开错误语义处理。
...
- 使用示例：
  ...
  ```python
  __init__(cost_kind="DMA|MAC", fold=True)
  ```
- 功能说明：执行 `__init__`。
...
- 锁定 cost function 命名/签名、共享 callee 去重、open-kind 多值语义、helper 保留与失败路径、`!symbol.int` 汇总返回。
- 锁定 registry 名称 `launch-kernel-cost-func` 与 `cost_kind` 选项透传。
- 锁定 `tuner.cost` 的 open-kind verifier 边界与错误路径。

### 功能与用例清单

| 用例 ID | 功能 | 场景 | 前置条件 | 操作 | 预期结果 | 建议测试 |
| --- | --- | --- | --- | --- | --- | --- |
| TC-LKCF-001A | 内存/DMA | 默认 `DMA | 准备公开 Memory/DMA 参数，包括 shape、stride、dtype、space 或切片元信息。 | 运行 `test_launch_kernel_cost_func_default_kind_is_dma_mac`。 | 内存类型、布局、搬运结果或 verifier 行为体现“默认 `DMA”场景。 | `test_launch_kernel_cost_func_default_kind_is_dma_mac` |
| TC-LKCF-001 | 成本/调优 | `cost_kind=compute` 基础成功路径 | 准备公开 cost kind、kernel/DMA 参数或 tuning IR 输入。 | 运行 `test_launch_kernel_cost_func_builds_cost_function_for_compute_kind`。 | 成本函数、tuning 属性或 cost IR 输出体现“`cost_kind=compute` 基础成功路径”场景。 | `test_launch_kernel_cost_func_builds_cost_function_for_compute_kind` |
| TC-LKCF-002 | 内存/DMA | `cost_kind=memory` 基础成功路径 | 准备公开 Memory/DMA 参数，包括 shape、stride、dtype、space 或切片元信息。 | 运行 `test_launch_kernel_cost_func_memory_keeps_compute_nodes`。 | 内存类型、布局、搬运结果或 verifier 行为体现“`cost_kind=memory` 基础成功路径”场景。 | `test_launch_kernel_cost_func_memory_keeps_compute_nodes` |
| TC-LKCF-003 | 成本/调优 | open-kind 列表成功路径 | 准备公开 cost kind、kernel/DMA 参数或 tuning IR 输入。 | 运行 `test_launch_kernel_cost_func_builds_cost_functions_for_multi_kind_order`。 | 成本函数、tuning 属性或 cost IR 输出体现“open-kind 列表成功路径”场景。 | `test_launch_kernel_cost_func_builds_cost_functions_for_multi_kind_order` |
