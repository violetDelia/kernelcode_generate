...
- `out` operand 命中非空 target 时，`kernel.matmul` 后必须追加 `dma.copy(original_out, staged_out)` 再释放 staging buffer，保证结果对后续 consumer 或最终 write-back 可见。
...
- `spec`：[`spec/pass/tuning/dma_memory_hierarchy.md`](dma_memory_hierarchy.md)
...
- Pass 管理器：[`spec/pass/pass_manager.md`](../pass_manager.md)
- pass registry：[`spec/pass/registry.md`](../registry.md)
- DMA dialect：[`spec/dialect/dma.md`](../../dialect/dma.md)
- Kernel dialect：[`spec/dialect/kernel.md`](../../dialect/kernel.md)
- NN dialect：[`spec/dialect/nn.md`](../../dialect/nn.md)
...
- 参数：
  ...
  - `apply_op`：规则文本；类型 `str | None`；默认 `None`。`None` 表示不启用规则；当前唯一合法非空形式为 `matmul{["", "tlm1", "tlm2"]}` 或 `matmul{["tlm1", "tlm1", "tlm1"]}` 这类单条 `matmul{[...]}`。
...
- 功能说明：
  ...
  - out operand 被替换为 staging buffer 时，kernel 后必须写回原 out descriptor。
  - 无 `apply_op` 且 `fold=True` 时，不改写 `module`。
  - 无 `apply_op` 且 `fold=False` 时，执行 legacy `GM -> SM -> LM` / `LM -> SM -> GM` 兼容路径。
- 注意事项：
  - `apply_op='matmul{["tlm1", "tlm1", "tlm1"]}'` 输入 `kernel.matmul(out@tsm, lhs@tsm, rhs@tsm)` 后，必须生成：

    ```text
    %out_buf = "dma.alloc"() : () -> !nn.memory<..., #nn.space<tlm1>>
    "dma.copy"(%out_buf, %out) : (...) -> ()
    %lhs_buf = "dma.alloc"() : () -> !nn.memory<..., #nn.space<tlm1>>
    "dma.copy"(%lhs_buf, %lhs) : (...) -> ()
    %rhs_buf = "dma.alloc"() : () -> !nn.memory<..., #nn.space<tlm1>>
    "dma.copy"(%rhs_buf, %rhs) : (...) -> ()
    "kernel.matmul"(%out_buf, %lhs_buf, %rhs_buf) ...
    "dma.copy"(%out, %out_buf) : (...) -> ()
    "dma.free"(%out_buf) : (!nn.memory<..., #nn.space<tlm1>>) -> ()
    "dma.free"(%lhs_buf) : (!nn.memory<..., #nn.space<tlm1>>) -> ()
    "dma.free"(%rhs_buf) : (!nn.memory<..., #nn.space<tlm1>>) -> ()
    ```
  - 规则中空字符串 operand 不得插入 `dma.alloc`、`dma.copy` 或替换 operand。
  - 规则命中 out operand 时，out 与 input 使用同一 staging 规则，但必须追加 out writeback。
...
- 执行命令：
  ...
  ```bash
  PYTHONDONTWRITEBYTECODE=1 PYTHONPATH=. pytest -q test/passes/tuning/test_dma_memory_hierarchy.py
...
- 验证 `apply_op` 合法规则、非法规则、空 target、out target 写回、C5 all-TLM1、显式符号 shape 与匿名动态 shape 流转。
...
| TC-DMH-004 | apply_op rewrite | matmul out copy/free/writeback | 构造 `kernel.matmul`。 | 运行 `apply_op='matmul{["tlm1", "", ""]}'`。 | out 替换到 `tlm1`，kernel 后生成 `dma.copy(original_out, staged_out)` 再释放 staging。 | `test_dma_memory_hierarchy_apply_op_can_copy_out` |
| TC-DMH-005 | apply_op rewrite | CUDA C5 all-TLM1 | 构造 `kernel.matmul`。 | 运行 `apply_op='matmul{["tlm1", "tlm1", "tlm1"]}'`。 | out/lhs/rhs 三个 operand 均替换到 `tlm1`，out 写回原 descriptor。 | `test_dma_memory_hierarchy_apply_op_all_tlm1_writes_back_out` |
| TC-DMH-006 | apply_op no-op | 空 target rule | 构造 `kernel.matmul`。 | 运行 `apply_op='matmul{["", "", ""]}'`。 | 不插入搬运，operand 保持不变。 | `test_dma_memory_hierarchy_apply_op_empty_rule_noop` |
| TC-DMH-007 | registry | options 构造 | 调用 `load_builtin_passes()`。 | `build_registered_pass("lower-dma-memory-hierarchy", {"fold": "false", "apply_op": ...})`。 | 返回 pass 并执行 copy rewrite。 | `test_dma_memory_hierarchy_registry_apply_op` |
| TC-DMH-008 | dynamic shape | 显式 symbol 维度 | 构造含 `M` 维度的 `kernel.matmul`。 | 运行 `apply_op` 改写 lhs。 | `dma.alloc.dynamic_shape` 使用 `symbol.get_dim` 结果。 | `test_dma_memory_hierarchy_apply_op_symbol_shape` |
| TC-DMH-009 | dynamic shape | 匿名 `?` 维度 | 构造含 `?` 维度的 `kernel.matmul`。 | 运行 `apply_op` 改写该 operand。 | `dma.alloc.dynamic_shape` 使用 `symbol.get_dim` 结果，result shape 保持 `?`。 | `test_dma_memory_hierarchy_apply_op_accepts_anonymous_dynamic_shape` |
| TC-DMH-010 | 错误语义 | 非法 apply_op | 使用非法 op、非法 arity、非法 space、非字符串元素。 | 构造 `LowerDmaMemoryHierarchyPass(apply_op=...)`。 | 稳定抛出 `KernelCodeError`。 | `test_dma_memory_hierarchy_rejects_invalid_apply_op_rules` |
