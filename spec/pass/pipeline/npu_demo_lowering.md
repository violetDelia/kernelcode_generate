...
- `multi-buffer-analysis`：`MultiBufferAnalysisPass` 的公开 pass 名称；本 pipeline 中位于第三段 `symbol-hoist-pipeline -> cse -> canonicalize` 之后，固定 `memory_stage=2` 且使用 pipeline `target` 写入 `"auto"` analysis 属性。
...
- 公开顺序必须固定为：
  ...
  22. `CanonicalizePass`
  23. `MultiBufferAnalysisPass(memory_stage=2, target=<pipeline target>)`
  24. `MultiBufferApplyPass(target=<pipeline target>, alignment=1024)`
  25. `LoopSoftPipelinePass`
  26. `ProducerConsumerAnalysisPass`
  27. `MemoryPoolPass(rewrite=True, alignment=1024)`
  28. `CommonSubexpressionElimination`
  29. `CanonicalizePass`
  30. `ArchParallelizePass(target=<pipeline target>, parallel_level="block")`
  31. `AttachArchInformationPass`
  32. `OutlineDeviceKernelPass`
  33. `TemplateNameInferPass`
...
- 注意事项：
  ...
  - `transform-apply` 后必须依次运行 `memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> kernel-aggregate -> kernel-decompose -> memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> multi-buffer-analysis -> multi-buffer-apply -> loop-soft-pipeline -> producer-consumer-analysis -> memory-pool`。
