...
- 该 pipeline 面向 `dsl_run` 的 `npu_demo` 正向链路，固定为最小可执行 lowering 组合。
...
- 提供一条不依赖 `tile` 的最小公开 pipeline，供 `dsl_run` 正向 expectation 与 pytest 直接复用。
...
- pass 顺序必须固定为 `decompass -> lower-nn -> symbol-loop-hoist`。
...
- 测试目标：
  ...
  - pass 顺序固定为 `decompass -> lower-nn -> symbol-loop-hoist`
