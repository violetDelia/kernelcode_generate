...
- `cse`：公共子表达式消除阶段；本 pipeline 中四次均紧跟 `canonicalize`，分别服务 inline 展平、第一段 `symbol-hoist-pipeline`、第一段 `symbol-buffer-hoist` 后 cleanup 和 transform 后 cleanup；memory-pool 后不再接入 CSE。
...
- `symbol-hoist-pipeline`：`SymbolHoistPipelinePass` 的公开 pass 名称；本 pipeline 中运行三次，组合 `dma-alias-to-reinterpret`、`symbol-loop-hoist` 与 `hoist-dma-alias-ops` pattern 能力，且 alias 归一 pattern 必须先于后续 hoist pattern。
...
- `producer-consumer-analysis`：`ProducerConsumerAnalysisPass` 的公开 pass 名称；本 pipeline 中位于第二段 `symbol-buffer-hoist` 之后、`memory-pool` 之前，只写普通或控制流分类分析 attr，不生成同步 op，并保留 typed `dma.alloc` 形态供分析读取。
- `kernel-aggregate`：`KernelAggregatePass` 的公开 pass 名称；本 pipeline 中位于第二段 `symbol-buffer-hoist` 后、`producer-consumer-analysis` 前，固定 `matmul_acc=True`。
- `kernel-matmul-fusion-decompose`：`KernelMatmulFusionDecomposePass` 的公开 pass 名称；本 pipeline 中紧跟 `kernel-aggregate`，在 source/emit 前清除所有 `kernel.matmul_fusion`。
- `tile-analysis`：`TileAnalysisPass` 的公开 pass 名称；本 pipeline 中紧跟第一段 `symbol-buffer-hoist -> symbol-hoist-pipeline -> cse -> canonicalize` cleanup，只补充 tile 分析属性。
...
- `symbol-buffer-hoist`：`SymbolBufferHoistPass` 的公开 pass 名称；本 pipeline 中两次分别位于两段 `memory-plan` 之后，用于把安全 `dma.alloc + dma.free` 成对外提到 owner `symbol.for` 两侧。
- `memory-pool`：`MemoryPoolPass` 的公开 pass 名称；本 pipeline 中固定为 `rewrite=True` 且 `alignment=0`，将片上 `dma.alloc` 改写为 `arch.get_dynamic_memory + dma.reinterpret`，后续不再接入额外 `cse`。
...
- 明确第一段 `symbol-hoist-pipeline` 后必须紧跟 `cse -> canonicalize`，在 memory-plan / symbol-buffer-hoist 前先收口 alias 与 symbol 外提。
- 明确第一段 `memory-plan` 位于第二段 `canonicalize` 之后、第一段 `symbol-buffer-hoist` 之前，并固定 `MemoryPlanPass(insert_free=True, fold=False)`。
- 明确第一个 `symbol-buffer-hoist` 位于第一段 `memory-plan` 之后，对 `symbol.for` 内安全 `dma.alloc + dma.free` 做成对外提；无可外提 buffer 时保持 no-op。
- 明确第一个 `symbol-buffer-hoist` 后必须运行 `symbol-hoist-pipeline -> cse -> canonicalize` cleanup。
- 明确 `tile-analysis` 位于第一段 post-buffer cleanup 之后、`kernel-pattern-attach` 之前，只记录 tile 分析结果，不生成 tile 循环。
...
- 明确 `transform-apply` 后再次运行 `symbol-hoist-pipeline -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> kernel-aggregate -> kernel-matmul-fusion-decompose -> producer-consumer-analysis`，再进入 `MemoryPoolPass(rewrite=True, alignment=0)`。
- 明确 `kernel-aggregate` 固定 `matmul_acc=True`，只在第二段 `symbol-buffer-hoist` 后聚合 matmul tmp+add 中间形态。
- 明确 `kernel-matmul-fusion-decompose` 紧跟 `kernel-aggregate`，在 `producer-consumer-analysis` 前把 `kernel.matmul_fusion` 拆回已有可 emit IR。
- 明确 `producer-consumer-analysis` 位于 `kernel-matmul-fusion-decompose` 之后、`memory-pool` 之前，并在 memory-pool 改写前读取 typed `dma.alloc` 形态。
- 明确 `memory-pool` 位于 `producer-consumer-analysis` 之后，并固定 `MemoryPoolPass(rewrite=True, alignment=0)`，本 pipeline 默认执行 dynamic backing 改写；该改写直接生成 `dma.reinterpret`，不再依赖后置第二次 alias 归一化 pass。
...
- 公开顺序必须固定为：
  ...
  4. `DecompassPass`
  5. `NnLoweringPass`
  6. `SymbolHoistPipelinePass`
  7. `CommonSubexpressionElimination`
  8. `CanonicalizePass`
  9. `MemoryPlanPass(insert_free=True, fold=False)`
  10. `SymbolBufferHoistPass`
  11. `SymbolHoistPipelinePass`
  12. `CommonSubexpressionElimination`
  13. `CanonicalizePass`
  14. `TileAnalysisPass`
  15. `KernelPatternAttachPass`
  16. `TransformApplyPass`
  17. `SymbolHoistPipelinePass`
  18. `CommonSubexpressionElimination`
  19. `CanonicalizePass`
  20. `MemoryPlanPass(insert_free=True, fold=False)`
  21. `SymbolBufferHoistPass`
  22. `KernelAggregatePass(matmul_acc=True)`
  23. `KernelMatmulFusionDecomposePass`
  24. `ProducerConsumerAnalysisPass`
  25. `MemoryPoolPass(rewrite=True, alignment=0)`
  26. `CanonicalizePass`
...
- 注意事项：
  ...
  - `SymbolHoistPipelinePass` 必须紧跟 `NnLoweringPass`，且该 pass 内必须先纳入 alias 归一 pattern。
  - memory-pool 前的每个 `cse` 后必须紧跟 xDSL `CanonicalizePass`；memory-pool 后不得再插入 `cse`。
  - 第一段 `symbol-buffer-hoist` 后必须紧跟 `symbol-hoist-pipeline -> cse -> canonicalize`。
  - 第一段和 transform 后的 `symbol-hoist-pipeline` 后必须紧跟 `cse -> canonicalize`。
  - `memory-plan` 固定 `insert_free=True, fold=False`，并在本 pipeline 中执行两次，均位于对应 `symbol-buffer-hoist` 前。
  - 第一个 `symbol-buffer-hoist` 必须在第一段 `memory-plan` 后把安全 `dma.alloc + dma.free` 成对外提，并在 `tile-analysis` 前完成 post-buffer cleanup。
  ...
  - `transform-apply` 后必须依次运行 `symbol-hoist-pipeline -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> kernel-aggregate -> kernel-matmul-fusion-decompose -> producer-consumer-analysis -> memory-pool`。
  - `kernel-aggregate` 必须固定 `matmul_acc=True`，不得由 pipeline option 改写。
  - `kernel-matmul-fusion-decompose` 必须在 `producer-consumer-analysis` 前移除所有 `kernel.matmul_fusion`。
  - `producer-consumer-analysis` 位于 `kernel-matmul-fusion-decompose` 之后、`memory-pool` 之前，只写普通或控制流分类分析 attr，不生成 `arch.wait` / `arch.sign`，且该阶段必须仍可观察 typed `dma.alloc` 形态。
  - `memory-pool` 固定 `rewrite=True` 与 `alignment=0`，将片上 `dma.alloc` 改写为 `arch.get_dynamic_memory + dma.reinterpret`。
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-002 | pass 改写 | npu demo lowering pipeline pass order | 准备包含目标 op、pass 名称或 pipeline 的公开 IR 输入。 | 运行 `test_npu_demo_lowering_pipeline_pass_order`。 | 固定顺序包含 `lower-nn -> symbol-hoist-pipeline -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> symbol-hoist-pipeline -> cse -> canonicalize -> tile-analysis`、`kernel-pattern-attach -> transform-apply -> symbol-hoist-pipeline -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> kernel-aggregate -> kernel-matmul-fusion-decompose -> producer-consumer-analysis -> memory-pool`、`memory-pool -> canonicalize -> arch-parallelize -> attach-arch-information -> outline-device-kernel -> template-name-infer`，且不包含顶层 `dma-alias-to-reinterpret`、`symbol-loop-hoist`、`hoist-dma-alias-ops`、`lower-dma-memory-hierarchy` 或 `multi-buffer`。 | `test_npu_demo_lowering_pipeline_pass_order` |
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-004 | pass 改写 | npu demo lowering pipeline memory plan dump shows lifecycle and pool | 通过公开 dump 配置运行 npu-demo-lowering。 | 运行 `test_npu_demo_lowering_pipeline_memory_plan_dump_shows_lifecycle_and_pool`。 | 按 dump marker 定位两段 `memory-plan`、两段 `symbol-buffer-hoist`、`kernel-aggregate`、`kernel-matmul-fusion-decompose`、`producer-consumer-analysis`、`memory-pool`、memory-pool 后 `canonicalize`、`arch-parallelize`、唯一 `attach-arch-information` 与 `outline-device-kernel`；memory-plan 含 `dma.free`，producer-consumer-analysis 位于 decompose 与 memory-pool 之间且保留 typed `dma.alloc`，late attach 位于 arch-parallelize 后且 outline 前，并特化 memory-pool 后 `arch.get_dynamic_memory`。 | `test_npu_demo_lowering_pipeline_memory_plan_dump_shows_lifecycle_and_pool` |
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-007 | pass 改写 | npu demo lowering static dump uses memory pool | 通过公开 dump 配置运行静态 tile matmul。 | 运行 `test_npu_demo_lowering_pipeline_static_dump_uses_pool_without_multi_buffer`。 | 静态 tile matmul 在 `lower-nn -> symbol-hoist-pipeline` 后先归一 lower-nn alias，并在第一段 `symbol-buffer-hoist -> symbol-hoist-pipeline -> cse -> canonicalize -> tile-analysis` 后进入 pattern attach；`transform-apply -> symbol-hoist-pipeline -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> kernel-aggregate -> kernel-matmul-fusion-decompose -> producer-consumer-analysis -> memory-pool` 后由 `arch.get_dynamic_memory + dma.reinterpret` 承接，且 producer-consumer-analysis 保留 typed `dma.alloc`、顶层不接入 `dma-alias-to-reinterpret` / `symbol-loop-hoist` / `hoist-dma-alias-ops` / `lower-dma-memory-hierarchy` / `multi-buffer`、最终不残留 `kernel.matmul_fusion` 或 `dma.alloc/dma.free`。 | `test_npu_demo_lowering_pipeline_static_dump_uses_pool_without_multi_buffer` |
