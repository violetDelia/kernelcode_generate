...
- `loop-soft-pipeline`：[`spec/pass/loop_soft_pipeline.md`](../../../spec/pass/loop_soft_pipeline.md)
...
- `cse`：公共子表达式消除阶段；本 pipeline 顶层保留两次，分别服务 inline 展平和 `memory-pool` 后动态 backing 清理；三段 `symbol-hoist-pipeline` 内嵌 cleanup 不生成独立顶层 dump marker。
- `canonicalize`：xDSL 内置 canonicalization 阶段；只在本 pipeline 内直接实例化 `CanonicalizePass`，不得新增仓库 registry pass 名称；本 pipeline 顶层共运行两次。
...
- `multi-buffer-analysis`：`MultiBufferAnalysisPass` 的公开 pass 名称；本 pipeline 中位于第三段 `symbol-hoist-pipeline` 默认内嵌 cleanup 后，固定 `memory_stage=2` 且使用 pipeline `target` 写入 `"auto"` analysis 属性。
- `multi-buffer-apply`：`MultiBufferApplyPass` 的公开 pass 名称；本 pipeline 中紧跟 `multi-buffer-analysis`，固定 `target=<pipeline target>` 与 `alignment=1024`，按 target registry 容量物化 ring。
- `loop-soft-pipeline`：`LoopSoftPipelinePass` 的公开 pass 名称；本 pipeline 中位于 `multi-buffer-apply` 之后、`producer-consumer-analysis` 之前，把可证明 ring-backed matmul preload loop 改写为 prologue / steady loop / epilogue 形态。
- `producer-consumer-analysis`：`ProducerConsumerAnalysisPass` 的公开 pass 名称；本 pipeline 中位于 `loop-soft-pipeline` 之后、`memory-pool` 之前，只写普通、控制流分类或 ring-aware 分析 attr，不生成同步 op，并保留 typed `dma.alloc` 或 ring 化后的 typed current slot 形态供分析读取。
- `kernel-aggregate`：`KernelAggregatePass` 的公开 pass 名称；本 pipeline 中位于第二段 `symbol-hoist-pipeline` 默认内嵌 cleanup 后，固定 `matmul_acc=True`。
...
- `tile-analysis`：`TileAnalysisPass` 的公开 pass 名称；本 pipeline 中紧跟第一段 `memory-plan -> symbol-hoist-pipeline` 默认内嵌 cleanup 后，只补充 tile 分析属性。
...
- 明确每段 `memory-plan` 均固定 `MemoryPlanPass(insert_free=True, reuse=True, fold=False, auto_pad=True)` 并紧跟默认 `symbol-hoist-pipeline{cse=true, canonicalize=true}`；本 pipeline 固定启用既有 `auto_pad` 能力，不新增 pipeline option。
- 明确 standalone `SymbolBufferHoistPass` 不再作为本 pipeline 顶层阶段出现；buffer 外提只通过 `symbol-hoist-pipeline` 内部 stage 完成。
- 明确 `tile-analysis` 位于第一段 `memory-plan -> symbol-hoist-pipeline` 默认内嵌 cleanup 之后、`kernel-pattern-attach` 之前，只记录 tile 分析结果，不生成 tile 循环。
...
- 明确当前 pipeline 在第三段 `symbol-hoist-pipeline` 默认内嵌 cleanup 后接入 `MultiBufferAnalysisPass(memory_stage=2, target=<pipeline target>) -> MultiBufferApplyPass(target=<pipeline target>, alignment=1024)`；不新增 pipeline option。
- 明确 `transform-apply` 后再次运行 `memory-plan -> symbol-hoist-pipeline -> kernel-aggregate -> kernel-decompose -> memory-plan -> symbol-hoist-pipeline -> multi-buffer-analysis -> multi-buffer-apply -> loop-soft-pipeline -> producer-consumer-analysis`，再进入 `MemoryPoolPass(rewrite=True, alignment=1024)`；两段 `symbol-hoist-pipeline` 默认内嵌 cleanup。
...
- 明确 `kernel-decompose` 后的第三段 `memory-plan -> symbol-hoist-pipeline` 默认内嵌 cleanup 负责让 `dma.fill` canonicalization 观察显式 dynamic acc `kernel.matmul`，并删除可证明 dead 的 acc/input/bias staging fill。
...
- 明确 `loop-soft-pipeline` 位于 `multi-buffer-apply` 之后、`producer-consumer-analysis` 之前，只对可证明 ring-backed preload loop 做 soft-pipeline 改写；unsupported 与 static zero-trip loop 保持 no-op。
- 明确 `producer-consumer-analysis` 位于 `loop-soft-pipeline` 之后、`memory-pool` 之前，并在 memory-pool 改写前读取 typed `dma.alloc` 或 ring current 形态。
...
- 公开顺序必须固定为：
  ...
  8. `TileAnalysisPass`
  9. `KernelPatternAttachPass`
  10. `TransformApplyPass`
  11. `MemoryPlanPass(insert_free=True, reuse=True, fold=False, auto_pad=True)`
  12. `SymbolHoistPipelinePass`
  13. `KernelAggregatePass(matmul_acc=True)`
  14. `KernelDecomposePass`
  15. `MemoryPlanPass(insert_free=True, reuse=True, fold=False, auto_pad=True)`
  16. `SymbolHoistPipelinePass`
  17. `MultiBufferAnalysisPass(memory_stage=2, target=<pipeline target>)`
  18. `MultiBufferApplyPass(target=<pipeline target>, alignment=1024)`
  19. `LoopSoftPipelinePass`
  20. `ProducerConsumerAnalysisPass`
  21. `MemoryPoolPass(rewrite=True, alignment=1024)`
  22. `CommonSubexpressionElimination`
  23. `CanonicalizePass`
  24. `ArchParallelizePass(target=<pipeline target>, parallel_level="block")`
  25. `AttachArchInformationPass`
  26. `OutlineDeviceKernelPass`
  27. `TemplateNameInferPass`
...
- 注意事项：
  ...
  - 每个顶层 `cse` 后必须紧跟 xDSL `CanonicalizePass`；顶层 cleanup 只保留 inline 后和 memory-pool 后两组。
  - 每段 `symbol-hoist-pipeline` 使用默认 `cse=true` / `canonicalize=true` 内嵌 cleanup，不再紧跟独立顶层 `cse -> canonicalize`。
  ...
  - 当前 pipeline 固定接入 `MultiBufferAnalysisPass(memory_stage=2, target=<pipeline target>) -> MultiBufferApplyPass(target=<pipeline target>, alignment=1024) -> LoopSoftPipelinePass`；不得为此新增 pipeline option。
  - `transform-apply` 后必须依次运行 `memory-plan -> symbol-hoist-pipeline -> kernel-aggregate -> kernel-decompose -> memory-plan -> symbol-hoist-pipeline -> multi-buffer-analysis -> multi-buffer-apply -> loop-soft-pipeline -> producer-consumer-analysis -> memory-pool`。
  ...
  - `kernel-decompose` 后的第三段 `symbol-hoist-pipeline` 默认内嵌 cleanup 必须按 `dma.fill` 合同删除 static/static 与 static/dynamic 中可证明 dead 的 acc/input/bias fill；dynamic/dynamic 不把 acc fill 删除列为通过条件。
  - `loop-soft-pipeline` 位于 `multi-buffer-apply` 之后、`producer-consumer-analysis` 之前，不支持结构与 static zero-trip loop 必须保持 no-op。
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-002 | pass 改写 | npu demo lowering pipeline pass order | 准备包含目标 op、pass 名称或 pipeline 的公开 IR 输入。 | 运行 `test_npu_demo_lowering_pipeline_pass_order`。 | 固定顺序包含 `lower-nn -> memory-plan(insert_free=true,reuse=true,fold=false,auto_pad=true) -> symbol-hoist-pipeline -> tile-analysis`、`kernel-pattern-attach -> transform-apply -> memory-plan(insert_free=true,reuse=true,fold=false,auto_pad=true) -> symbol-hoist-pipeline -> kernel-aggregate -> kernel-decompose -> memory-plan(insert_free=true,reuse=true,fold=false,auto_pad=true) -> symbol-hoist-pipeline -> multi-buffer-analysis(memory_stage=2,target=npu_demo) -> multi-buffer-apply(target=npu_demo,alignment=1024) -> loop-soft-pipeline -> producer-consumer-analysis -> memory-pool(rewrite=true,alignment=1024)`、`memory-pool -> cse -> canonicalize -> arch-parallelize -> attach-arch-information -> outline-device-kernel -> template-name-infer`，且不包含三段 symbol-hoist 后独立顶层 `cse/canonicalize`、顶层 standalone `symbol-buffer-hoist`、`dma-alias-to-reinterpret`、`symbol-loop-hoist`、`hoist-dma-alias-ops`、`lower-dma-memory-hierarchy` 或旧 `kernel-matmul-fusion-decompose`。 | `test_npu_demo_lowering_pipeline_pass_order` |
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-007 | pass 改写 | npu demo lowering static dump runs multi buffer before pool | 通过公开 dump 配置运行静态 tile matmul。 | 运行 `test_npu_demo_lowering_pipeline_static_dump_runs_multi_buffer_before_pool`。 | 静态 tile matmul 在 `lower-nn -> memory-plan(auto_pad=true) -> symbol-hoist-pipeline` 后先归一 lower-nn alias，并在第一段内嵌 cleanup 后进入 pattern attach；`transform-apply -> memory-plan(auto_pad=true) -> symbol-hoist-pipeline -> kernel-aggregate -> kernel-decompose -> memory-plan(auto_pad=true) -> symbol-hoist-pipeline -> multi-buffer-analysis -> multi-buffer-apply -> loop-soft-pipeline -> producer-consumer-analysis -> memory-pool -> cse -> canonicalize` 后由 `arch.get_dynamic_memory + dma.reinterpret` 承接；analysis dump 只写三项属性，apply dump 生成 ring 且不残留属性，最终不残留 `kernel.matmul_fusion` 或 `dma.alloc/dma.free`。 | `test_npu_demo_lowering_pipeline_static_dump_runs_multi_buffer_before_pool` |
