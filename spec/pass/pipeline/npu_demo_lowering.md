...
- `memory-pool`：`MemoryPoolPass` 的公开 pass 名称；本 pipeline 中固定为 `rewrite=True` 且 `alignment=0`，将片上 `dma.alloc` 改写为 `arch.get_dynamic_memory + dma.view + dma.reshape`，后续不再接入额外 `cse`。
...
- 明确 `memory-pool` 位于第二段 `symbol-buffer-hoist` 之后，并固定 `MemoryPoolPass(rewrite=True, alignment=0)`，本 pipeline 默认执行 dynamic backing 改写。
...
- 公开顺序必须固定为：
  ...
  4. `DecompassPass`
  5. `NnLoweringPass`
  6. `SymbolLoopHoistPass`
  7. `HoistDmaAliasOpsPass`
  8. `CommonSubexpressionElimination`
  9. `CanonicalizePass`
  10. `MemoryPlanPass(insert_free=True, fold=False)`
  11. `SymbolBufferHoistPass`
  12. `TileAnalysisPass`
  13. `KernelPatternAttachPass`
  14. `TransformApplyPass`
  15. `SymbolLoopHoistPass`
  16. `HoistDmaAliasOpsPass`
  17. `CommonSubexpressionElimination`
  18. `CanonicalizePass`
  19. `MemoryPlanPass(insert_free=True, fold=False)`
  20. `SymbolBufferHoistPass`
  21. `MemoryPoolPass(rewrite=True, alignment=0)`
  22. `CanonicalizePass`
  23. `ArchParallelizePass(target=<pipeline target>, parallel_level="block")`
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-002 | pass 改写 | npu demo lowering pipeline pass order | 准备包含目标 op、pass 名称或 pipeline 的公开 IR 输入。 | 运行 `test_npu_demo_lowering_pipeline_pass_order`。 | 固定顺序包含 memory-pool 前每个 `symbol-loop-hoist -> hoist-dma-alias-ops -> cse -> canonicalize`、第一段 `memory-plan -> symbol-buffer-hoist`、`kernel-pattern-attach -> transform-apply -> symbol-loop-hoist -> hoist-dma-alias-ops -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> memory-pool`、`memory-pool -> canonicalize -> arch-parallelize -> producer-consumer-analysis -> attach-arch-information -> outline-device-kernel -> template-name-infer`，且不包含顶层 `lower-dma-memory-hierarchy` 或 `multi-buffer`。 | `test_npu_demo_lowering_pipeline_pass_order` |
...
| TC-PASS-PIPELINE-NPU-DEMO-LOWERING-007 | pass 改写 | npu demo lowering static dump uses memory pool | 通过公开 dump 配置运行静态 tile matmul。 | 运行 `test_npu_demo_lowering_pipeline_static_dump_uses_pool_without_multi_buffer`。 | 静态 tile matmul 在 `transform-apply -> symbol-loop-hoist -> hoist-dma-alias-ops -> cse -> canonicalize -> memory-plan -> symbol-buffer-hoist -> memory-pool` 后由 `arch.get_dynamic_memory + dma.view + dma.reshape` 承接，且顶层不接入 `lower-dma-memory-hierarchy` / `multi-buffer`、不残留 `dma.alloc/dma.free`。 | `test_npu_demo_lowering_pipeline_static_dump_uses_pool_without_multi_buffer` |
