...
- `producer-consumer-analysis`：`ProducerConsumerAnalysisPass` 的公开 pass 名称；本 pipeline 中位于 `multi-buffer-apply` 之后、`memory-pool` 之前，只写普通或控制流分类分析 attr，不生成同步 op，并保留 typed `dma.alloc` 或 ring 化后的 typed current slot 形态供分析读取。
...
- 明确 `transform-apply` 后再次运行 `memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> kernel-aggregate -> kernel-decompose -> memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> multi-buffer-analysis -> multi-buffer-apply -> producer-consumer-analysis`，再进入 `MemoryPoolPass(rewrite=True, alignment=1024)`。
...
- 明确 `producer-consumer-analysis` 位于 `multi-buffer-apply` 之后、`memory-pool` 之前，并在 memory-pool 改写前读取 typed `dma.alloc` 或 ring current 形态。
...
- 公开顺序必须固定为：
  ...
  26. `MemoryPoolPass(rewrite=True, alignment=1024)`
  27. `CommonSubexpressionElimination`
  28. `CanonicalizePass`
  29. `ArchParallelizePass(target=<pipeline target>, parallel_level="block")`
  30. `AttachArchInformationPass`
  31. `OutlineDeviceKernelPass`
  32. `TemplateNameInferPass`
...
- 注意事项：
  ...
  - 当前 pipeline 固定接入 `MultiBufferAnalysisPass(memory_stage=2, target=<pipeline target>) -> MultiBufferApplyPass(target=<pipeline target>, alignment=1024)`；不得为此新增 pipeline option。
  - `transform-apply` 后必须依次运行 `memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> kernel-aggregate -> kernel-decompose -> memory-plan -> symbol-hoist-pipeline -> cse -> canonicalize -> multi-buffer-analysis -> multi-buffer-apply -> producer-consumer-analysis -> memory-pool`。
  ...
  - `producer-consumer-analysis` 位于 `multi-buffer-apply` 之后、`memory-pool` 之前，只写普通或控制流分类分析 attr，不生成 `arch.wait` / `arch.sign`，且该阶段必须仍可观察 typed `dma.alloc` 或 ring current 形态。
