...
- `kernel_gen.passes.arch_parallelize.ArchParallelizePass`（compat shim）
- `kernel_gen.passes.arch_parallelize.arch_parallelize.ArchParallelizePass`（compat shim）
...
- `test`：`test/passes/arch/test_arch_parallelize.py`、`test/passes/test_registry.py`、`test/passes/test_pattern_public_api_docs.py`
...
- `kernel_gen.dialect.dma`：公开 `DmaViewOp`、`DmaReshapeOp`、`DmaReinterpretOp`、`DmaMakeRingOp` 与 `DmaRingType` 类型，用于识别 memory-pool / multi-buffer 生成的 loop 前 setup 前缀，并在 block-strided loop body clone 时同步重写 ring slot type 中的 symbol 表达式。
...
- `kernel_gen.passes.arch.arch_parallelize` 是唯一 canonical public module path；旧 `kernel_gen.passes.arch_parallelize` 与 `kernel_gen.passes.arch_parallelize.arch_parallelize` 仅作为 compat shim。
- 内部 `FuncOp` root pattern 是当前实现细节，不从 `kernel_gen.passes.arch` package root、旧 compat shim 或 `kernel_gen.passes` 顶层 re-export。
...
- after:
  ...
  func.func @kernel() {
    ...
    %offset = symbol.mul %block, %c1 : !symbol.int<#symbol.expr<block_id * 1>>
    %new_start = symbol.add %c0, %offset : !symbol.int<#symbol.expr<0 + block_id * 1>>
    %new_step = symbol.mul %c1, %two : !symbol.int<#symbol.expr<1 * 2>>
    symbol.for %i = %new_start to %c8 step %new_step {iter = #symbol.iter<start = #symbol.expr<0 + block_id * 1>, end = #symbol.expr<8>, step = #symbol.expr<1 * 2>>} {
    }
    func.return
  }
  ```
...
- 测试文件：`test/passes/arch/test_arch_parallelize.py`、`test/passes/test_registry.py`
- 执行命令：`pytest -q test/passes/arch/test_arch_parallelize.py test/passes/test_registry.py`
