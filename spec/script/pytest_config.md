...
- 保证 `cuda` 标记稳定可用，避免 CUDA SM89 本机运行测试产生未知标记 warning。
...
| TC-SCRIPT-PYTEST-CONFIG-005 | 公开入口 | 配置块存在且包含 `cuda` 标记说明。 | 按 spec 声明的导入路径、CLI 参数、注册名或命名空间访问公开入口。 | 运行 `test_pytest_ini_options_present`。 | CUDA SM89 本机运行测试可使用 `-m cuda`，不会产生 unknown mark warning。 | `test_pytest_ini_options_present` |
