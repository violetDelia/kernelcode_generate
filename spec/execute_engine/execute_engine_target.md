...
- CUDA SM89 include runtime：[`spec/include/cuda_sm89/cuda_sm89.md`](../include/cuda_sm89/cuda_sm89.md)
...
- 冻结 `target=cpu`、`target=npu_demo` 与 `target=cuda_sm89` 的 include 注入结果。
...
- `P0` 内置 include 注入与真实执行仅支持 `target in {"cpu", "npu_demo", "cuda_sm89"}`；第三方 target 必须通过 [`spec/execute_engine/strategy.md`](strategy.md) 的 compile strategy 扩展，不得复用 CPU include 注入作为 fallback。
- `target` 选择只由 `CompileRequest.target` 驱动；不得因为源码内容自动切换到另一 target。
- `target=npu_demo` 不允许以注入 `cpu` include 或回退调用 `cpu::*` 的方式完成编译。
- `target=cuda_sm89` 不允许以注入 `cpu` / `npu_demo` include、回退调用 `cpu::*` / `npu_demo::*` 或普通 C++ compiler 替代 `nvcc` 的方式完成默认编译。
...
- `ExecutionEngine.compile(...)` 根据 `CompileRequest.target` 选择 compile strategy；内置 `cpu` / `npu_demo` / `cuda_sm89` strategy 由 `builtin_strategy/` package 选择 target include set 并生成编译产物。
...
- `target="cuda_sm89"` 时只能注入 `#include "include/cuda_sm89/cuda_sm89.cuh"`。
- 当 `compiler is None` 时，`cpu` / `npu_demo` 编译命令使用 `g++`，`cuda_sm89` 编译命令使用 `nvcc`。
- 编译 flags 必须保留 `-std=c++17` 基线，并按调用方顺序追加 `CompileRequest.compiler_flags`。
- `cuda_sm89` 在调用方未显式提供 CUDA 必需 flags 时必须附加 `-arch=sm_89`、`-shared`、`-Xcompiler`、`-fPIC`，并输出 `.so` shared object。
- `cuda_sm89` 编译策略必须消费公开 `SourceBundle` aggregate string，把 bundle marker 展开为真实 `.cu/.cuh` artifact 后编译主 `.cu`；不得把 aggregate marker 原文直接交给 `nvcc` 作为唯一源码。
...
- 内置 target 编译产物结构、include 注入、entry shim 与真实编译 helper 均为 `builtin_strategy/` package 内实现，不进入 `kernel_gen.execute_engine` 包根公开 API；调用方不得依赖这些 helper 或 `cpu` / `npu_demo` / `cuda_sm89` 子模块作为第三方扩展点。
...
- `cuda_sm89` 使用既有 `kg_execute_entry(slots, count)` C ABI；`slots` 布局由 `include/cuda_sm89/cuda_sm89.cuh` 的 `cuda_sm89::ArgSlot` 承接。
- `cuda_sm89` 当前不支持非 `None` stream；`CompiledKernel.execute(..., stream=<non-none>)` 必须以 `stream_not_supported` 稳定失败，文本包含 `cuda_sm89 does not support non-None stream`。
...
- 测试文件：
  ...
  - `test/execute_engine/test_cuda_sm89_strategy.py`
  - `test/cuda/test_cuda_sm89_kernel_demos_runtime.py`
- 执行命令：
...
| TC-EXECUTE-ENGINE-EXECUTE-ENGINE-TARGET-013 | 生成/编译 | `target=cuda_sm89` 编译 SourceBundle artifact。 | 使用 `ExecutionEngine(target="cuda_sm89")` 与包含 `kernel.cu` 的公开 SourceBundle aggregate。 | 运行 `test_cuda_sm89_builtin_strategy_compiles_source_bundle_with_nvcc`。 | 编译策略写出 `.cu/.cuh` artifact，默认命令包含 `nvcc`、`-arch=sm_89`、`-shared`、`-Xcompiler -fPIC` 并返回 `.so`。 | `test_cuda_sm89_builtin_strategy_compiles_source_bundle_with_nvcc` |
| TC-EXECUTE-ENGINE-EXECUTE-ENGINE-TARGET-014 | 边界/异常 | `target=cuda_sm89` 拒绝 npu_demo / cpu header 混用。 | 准备混入 `include/npu_demo/npu_demo.h` 的 CUDA 源码。 | 运行 `test_cuda_sm89_builtin_strategy_rejects_header_mismatch`。 | 失败短语为 `target_header_mismatch`。 | `test_cuda_sm89_builtin_strategy_rejects_header_mismatch` |
| TC-EXECUTE-ENGINE-EXECUTE-ENGINE-TARGET-015 | 执行结果 | `target=cuda_sm89` 非空 stream 失败。 | 构造公开 `CompiledKernel(target="cuda_sm89", ...)`。 | 运行 `test_cuda_sm89_execute_rejects_non_none_stream`。 | 失败短语为 `stream_not_supported`，文本包含 `cuda_sm89 does not support non-None stream`。 | `test_cuda_sm89_execute_rejects_non_none_stream` |
| TC-EXECUTE-ENGINE-EXECUTE-ENGINE-TARGET-016 | CUDA runtime | SM89 GPU 上运行 9 个现有 kernel demo 形态。 | 本机存在 `nvcc` 与 SM89 CUDA device。 | 运行 `pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda`。 | CUDA `.so` 编译并通过 generated `kg_execute_entry` 执行 matmul / conv2d / flash_attention demo 形态，输出与 NumPy baseline 一致；缺少 SM89 环境必须记录阻塞，不能记作通过。 | `test_cuda_sm89_runtime_preflight_accepts_sm89`, `test_cuda_sm89_runtime_preflight_rejects_non_sm89`, `test_cuda_sm89_matmul_demo_runtime_cases`, `test_cuda_sm89_conv2d_demo_runtime_cases`, `test_cuda_sm89_flash_attention_demo_runtime_cases` |
