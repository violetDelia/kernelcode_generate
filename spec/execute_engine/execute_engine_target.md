...
- 本文档覆盖 target include、entry shim 与 compiler 三类职责；公开入口和 `CompiledKernel` 装配由 `kernel_gen/execute_engine/compiler.py` 承接，内置 target 支持实现由 `kernel_gen/execute_engine/builtin_strategy.py` 承接。
...
- `ExecutionEngine.compile(...)` 根据 `CompileRequest.target` 选择 compile strategy；内置 `cpu` / `npu_demo` strategy 由 `builtin_strategy.py` 选择 target include set 并生成编译产物。
...
- 内置 target 编译产物结构、include 注入、entry shim 与真实编译 helper 均为 `builtin_strategy.py` 包内实现，不进入 `kernel_gen.execute_engine` 包根公开 API；调用方不得依赖这些 helper 或内部模块作为第三方扩展点。
