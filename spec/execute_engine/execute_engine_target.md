...
| TC-EXECUTE-ENGINE-EXECUTE-ENGINE-TARGET-016 | CUDA runtime | SM89 GPU 上运行 9 个现有 kernel demo 形态。 | 本机存在 `nvcc` 与 SM89 CUDA device。 | 运行 `pytest -q test/cuda/test_cuda_sm86_kernel_demos_runtime.py -m cuda`。 | CUDA `.so` 编译并通过 generated `kg_execute_entry` 执行 matmul / conv2d / flash_attention demo 形态，输出与 NumPy baseline 一致；缺少 SM89 环境必须记录阻塞，不能记作通过。 | `test_cuda_sm86_runtime_preflight_accepts_sm89`, `test_cuda_sm86_runtime_preflight_rejects_non_sm89`, `test_cuda_sm86_matmul_demo_runtime_cases`, `test_cuda_sm86_conv2d_demo_runtime_cases`, `test_cuda_sm86_flash_attention_demo_runtime_cases` |
