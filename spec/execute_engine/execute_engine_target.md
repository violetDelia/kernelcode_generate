...
- 本文档覆盖 target include、entry shim 与 compiler 三类职责；公开入口、内置 target 支持实现和 `CompiledKernel` 装配均由 `kernel_gen/execute_engine/compiler.py` 承接。
...
- `ExecutionEngine.compile(...)` 根据 `CompileRequest.target` 选择 compile strategy；内置 `cpu` / `npu_demo` strategy 在 `compiler.py` 内选择 target include set 并生成编译产物。
...
- 内置 target 编译产物结构、include 注入、entry shim 与真实编译 helper 均为 `compiler.py` 私有实现，不进入 `kernel_gen.execute_engine` 包根公开 API；调用方不得依赖这些下划线 helper。
