...
定义 [`skills/codex-multi-agents/scripts/codex-multi-agents-task.sh`](../../../skills/codex-multi-agents/scripts/codex-multi-agents-task.sh) 的任务流转合同。该脚本负责维护 `TODO.md`、`DONE.md`、`agents-lists.md` 三类文件中的任务状态、计划统计、角色忙闲状态，以及分发后的会话通知。
...
- 在 `-dispatch` 与 `-next -auto` 成功后，发送固定格式的任务或摘要消息。
...
- `-next -auto` 的候选扫描顺序固定为：`merge > review > build > spec`；同类型内按 `任务列表` 出现顺序选择。
- `-next -auto` 仅选择依赖已满足的任务；接手顺序固定为：
  - 任务记录中的默认指派，且该角色当前空闲、职责匹配
  - 其他当前空闲、职责匹配的角色
  - 当前执行 `-next -auto` 的角色本人；仅当其在本次续接后已变为空闲，且职责匹配时成立
- `-next -auto` 若自动接续到其他角色，会先执行一次 `list -init`，再向接手人发送任务消息，并向管理员发送摘要。
- `-next -auto` 若自动接续到当前执行者本人，不向本人发消息，只向管理员发送摘要。
- `-next -auto` 若没有可继续推进任务、没有合适角色、或当前并发人数已达上限，则任务保留在 `任务列表`，并向管理员发送摘要。
- `-auto` 只能与 `-next` 组合使用。
...
- 将当前运行中任务退回 `任务列表`，改写下一阶段的描述与任务类型；带 `-auto` 时，继续尝试自动接续下一条可推进任务。
...
- `-next -auto` 自动接续成功时，会把选中的候选任务移入 `正在执行的任务`，并把接手角色状态改为 `busy`。
...
- 测试目标：
  ...
  - 验证 `-next -auto` 的自动续接、优先级顺序、当前执行者自动接续与会话消息语义。
...
- 功能与用例清单：
  ...
  - `TC-035` `test_new_restricted_for_non_privileged_operator`：普通执行人调用 `-new`，返回 `3`
  - `TC-036` `test_done_restricted_for_non_privileged_operator`：非管理员调用 `-done`，返回 `3`
  - `TC-037` `test_new_requires_worktree`：新建缺少 `-worktree`，返回 `1`
  - `TC-038` `test_dispatch_blocked_by_unresolved_dependency`：分发时依赖未清空，返回 `3`
  - `TC-039` `test_dispatch_rejects_busy_agent`：目标角色为 `busy`，返回 `3`
  - `TC-040` `test_next_requires_agents_list`：续接缺少角色列表，返回 `1`
  - `TC-041` `test_new_requires_depends_and_plan`：新建缺少 `-depends/-plan`，返回 `1`
  - `TC-042` `test_status_plan_list_outputs_plan_table`：查询计划统计表
  - `TC-043` `test_new_requires_existing_dependencies`：新建任务依赖不存在，返回 `3`
  - `TC-044` `test_new_updates_plan_progress_table`：新建任务更新计划统计
  - `TC-045` `test_done_last_plan_task_marks_plan_waiting_review`：完成最后一个计划任务，计划状态改为 `完成待检查`
  - `TC-046` `test_done_plan_removes_review_ready_plan`：归档完成待检查计划
  - `TC-047` `test_done_plan_requires_review_ready_status`：归档非 `完成待检查` 计划，返回 `3`
  - `TC-048` `test_done_plan_restricted_for_non_privileged_operator`：非管理员调用 `-done-plan`，返回 `3`
  - `TC-049` `test_dispatch_rejects_when_parallel_limit_reached`：并发人数达到上限，返回 `3`
  - `TC-050` `test_dispatch_restricted_for_non_admin_operator`：非管理员调用 `-dispatch`，返回 `3`
  - `TC-051` `test_next_auto_dispatches_task_and_sends_messages`：自动续接到其他角色
  - `TC-052` `test_next_auto_keeps_task_in_list_when_no_free_agent`：没有合适空闲角色，任务保留在列表
  - `TC-053` `test_next_auto_prefers_merge_task`：自动续接优先处理 `merge`
  - `TC-054` `test_next_auto_assigns_operator_when_no_free_agent_matches`：当前执行者自动接续
