...
- `-next -auto` 会在完成同样的退回动作后，继续按当前 `任务列表` 顺序自动启动所有依赖已清空且存在合适候选人的任务，直到没有 ready 任务或达到并发上限。
...
- 普通 `-next` 只会自动启动首个可启动任务；`-next -auto` 会持续启动所有当前可启动任务。
- 自动续接的候选人集合规则：
...
- 权限限制：
  ...
  - `-done` 管理员可执行；合并角色仅可对“指派给自身且任务类型为 `merge` 的运行中任务”执行。流程要求：`merge` 任务合并完成后由合并角色自行执行 `-done`，管理员不代执行该动作。
- 锁顺序固定：
