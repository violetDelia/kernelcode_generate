...
- 输入 batch/head 使用 `B/H` 符号维度，并由真实 torch tensor 运行时 shape 绑定；`SL=8/D=4` 保持静态。
...
API 列表:
- `flash_attention_inputs_dynamic_tile_dynamic_kernel(out: Tensor[f32, B, H, 8, 4], q: Tensor[f32, B, H, 8, 4], k: Tensor[f32, B, H, 8, 4], v: Tensor[f32, B, H, 8, 4], br: SymbolDim, bc: SymbolDim) -> None`
- `main() -> None`
...
def flash_attention_inputs_dynamic_tile_dynamic_kernel(
    out: "Tensor[f32, B, H, 8, 4]",
    q: "Tensor[f32, B, H, 8, 4]",
    k: "Tensor[f32, B, H, 8, 4]",
    v: "Tensor[f32, B, H, 8, 4]",
    br: SymbolDim,
    bc: SymbolDim,
) -> None:
    ...
    使用示例:
    - `flash_attention_inputs_dynamic_tile_dynamic_kernel(out, q, k, v, 4, 8)`
    """
...
def main() -> None:
    ...
    q = torch.linspace(-0.3, 0.2, steps=4 * 4 * 8 * 4, dtype=torch.float32).reshape(4, 4, 8, 4)
    k = torch.linspace(0.1, -0.3, steps=4 * 4 * 8 * 4, dtype=torch.float32).reshape(4, 4, 8, 4)
    v = torch.linspace(-0.6, 0.4, steps=4 * 4 * 8 * 4, dtype=torch.float32).reshape(4, 4, 8, 4)
