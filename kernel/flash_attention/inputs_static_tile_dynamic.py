...
API 列表:
- `flash_attention_inputs_static_tile_dynamic_kernel(out: Tensor[f32, 1, 2, 16, 8], q: Tensor[f32, 1, 2, 16, 8], k: Tensor[f32, 1, 2, 16, 8], v: Tensor[f32, 1, 2, 16, 8]) -> None`
- `main() -> None`
...
def flash_attention_inputs_static_tile_dynamic_kernel(
    ...
    使用示例:
    - `flash_attention_inputs_static_tile_dynamic_kernel(out, q, k, v)`
    """
...
def main() -> None:
    ...
    q = torch.linspace(-0.2, 0.3, steps=1 * 2 * 16 * 8, dtype=torch.float32).reshape(1, 2, 16, 8)
    k = torch.linspace(0.3, -0.1, steps=1 * 2 * 16 * 8, dtype=torch.float32).reshape(1, 2, 16, 8)
    v = torch.linspace(-0.4, 0.6, steps=1 * 2 * 16 * 8, dtype=torch.float32).reshape(1, 2, 16, 8)
