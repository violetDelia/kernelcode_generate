...
- 输入 batch 使用 `N` 符号维度，并由真实 torch tensor 运行时 shape 绑定；卷积结构维度保持静态，避免当前 emit 路径生成未绑定 stride 符号。
- tile 在 DSL 函数体内固定为当前 demo 运行值，满足 `dsl_run` 只接收 tensor 运行时参数的公开入口约束。
- 通过 `dsl_run` 真实执行，并和 `torch.nn.functional.conv2d` 参考结果对齐。

API 列表:
- `conv2d_inputs_dynamic_tile_dynamic_kernel(out: Tensor[f32, N, 4, 6, 6], input_tensor: Tensor[f32, N, 3, 8, 8], weight: Tensor[f32, 4, 3, 3, 3]) -> None`
- `main() -> None`
...
def conv2d_inputs_dynamic_tile_dynamic_kernel(
    out: "Tensor[f32, N, 4, 6, 6]",
    input_tensor: "Tensor[f32, N, 3, 8, 8]",
    ...
    功能说明:
    - 输入 batch 维度来自 `Tensor[...]` 符号维度。
    - 卷积结构维度保持静态，避免当前 emit 路径生成未绑定 stride 符号。
    - tile shape 使用当前 demo 的运行值。
    - 固定 stride=1、dilation=1、padding=0。

    使用示例:
    - `conv2d_inputs_dynamic_tile_dynamic_kernel(out, input_tensor, weight)`
    """
...
def main() -> None:
    ...
    功能说明:
    - 构造真实 torch tensor 输入，shape 绑定到 `N`。
    ...
    input_tensor = torch.arange(1 * 3 * 8 * 8, dtype=torch.float32).reshape(1, 3, 8, 8) / 137.0
    weight = torch.arange(4 * 3 * 3 * 3, dtype=torch.float32).reshape(4, 3, 3, 3) / 139.0
