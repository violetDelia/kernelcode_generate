...
API 列表:
- `conv2d_inputs_static_tile_dynamic_kernel(out: Tensor[f32, 5, 20, 35, 33], input_tensor: Tensor[f32, 5, 65, 281, 262], weight: Tensor[f32, 20, 65, 3, 3], bias: Tensor[f32, 20], tile_f: SymbolDim, tile_c: SymbolDim, tile_n: SymbolDim, tile_ho: SymbolDim, tile_wo: SymbolDim) -> None`
- `main() -> None`
...
def main() -> None:
    ...
    for bias_case in _BIAS_CASE_ORDER:
        if bias_case == "absent":
            results[bias_case] = run_numpy_demo(
                "conv2d/inputs_static_tile_dynamic_absent_bias",
                conv2d_inputs_static_tile_dynamic_kernel,
                (absent_out, input_tensor, weight, None, *tile_args),
                absent_out,
                absent_expected,
            )
            continue
        results[bias_case] = run_numpy_demo(
            "conv2d/inputs_static_tile_dynamic_present_bias",
            conv2d_inputs_static_tile_dynamic_kernel,
            (present_out, input_tensor, weight, bias, *tile_args),
            present_out,
            present_expected,
        )
    ...
    print(
        "[ARGS] "
        f"seed=2026051612 input={input_tensor.shape} weight={weight.shape} "
