...
def truediv_func2(a: int, b: int) -> int:
    return nn.truediv(a, b)

TARGET_FUNCS: tuple[Callable[[int, int], int], ...] = (truediv_func, truediv_func2)


def check_const_div_const() -> None:
    for target_func in TARGET_FUNCS:
        ...
        assert_static_symbol_int(div_ops[0].result.type, expected_expr)
...
def check_dynamic_div_dynamic() -> None:
    for target_func in TARGET_FUNCS:
...
def check_const_div_dynamic() -> None:
    for target_func in TARGET_FUNCS:
...
def check_dynamic_div_const() -> None:
    for target_func in TARGET_FUNCS:
...
check_const_div_const()
check_dynamic_div_dynamic()
check_const_div_dynamic()
check_dynamic_div_const()
