...
- 验证 `SymbolSubOp` 结果类型与 `ReturnOp` 返回值类型都与期望值一致。
...
def check_const_minus_const(func):
...
def check_dynamic_minus_dynamic(func):
...
def check_const_minus_dynamic(func):
...
def check_dynamic_minus_const(func):
