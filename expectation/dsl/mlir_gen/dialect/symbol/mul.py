...
- 验证 `SymbolMulOp` 结果类型与 `ReturnOp` 返回值类型都与期望值一致。
...
def check_const_mul_const(func):
...
def check_dynamic_mul_dynamic(func):
...
def check_const_mul_dynamic(func):
...
def check_dynamic_mul_const(func):
