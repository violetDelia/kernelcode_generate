...
def alloc_symbol_kernel(
    rank1: int | SymbolDim,
    rank2: int | SymbolDim,
) -> f"Tensor[f32, {SYMBOL_LHS_NAME}, {SYMBOL_RHS_NAME}]":
    return alloc([rank1, rank2], NumericType.Float32, MemorySpace.SM)
...
def alloc_invalid_dtype_kernel(
    rank1: int | SymbolDim,
    rank2: int | SymbolDim,
) -> f"Tensor[f32, {ALLOC_ROWS}, {ALLOC_COLS}]":
...
def alloc_invalid_space_kernel(
    rank1: int | SymbolDim,
    rank2: int | SymbolDim,
) -> f"Tensor[f32, {ALLOC_ROWS}, {ALLOC_COLS}]":
...
# case 7: invalid runtime_args type should raise
try:
    build_func_op(alloc_kernel, 1.5, ALLOC_COLS)
    assert False, "build_func_op should fail when alloc runtime args type is invalid"
except AstVisitorError as exc:
    assert "Unsupported scalar argument type" in str(exc)
