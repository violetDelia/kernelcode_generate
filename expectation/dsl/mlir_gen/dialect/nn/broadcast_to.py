...
- 锁定 `broadcast_to(source, target_shape, space)` 的目标公开合同：应生成显式 `nn.broadcast`。
...
# Case 列表:
# - CASE-1: 静态正向例子：`broadcast_to(source, target_shape, space)` 的静态目标 shape 应生成显式 `nn.broadcast`。
# - CASE-2: symbol 正向例子：`broadcast_to(source, target_shape, space)` 的 symbol 目标 shape 应生成显式 `nn.broadcast`。
# - CASE-3: 失败例子：`broadcast_to(source, target_shape, space)` 的目标 rank 小于输入 rank 时应在构造阶段拒绝。
...
from kernel_gen.operation.nn import broadcast_to
from kernel_gen.symbol_variable.memory import Memory, MemorySpace
from kernel_gen.symbol_variable.symbol_dim import SymbolDim
from kernel_gen.symbol_variable.type import Farmat, NumericType
from kernel_gen.tools.mlir_gen_compare import mlir_gen_compare_text

# CASE-1 IR：静态正向例子：`broadcast_to(source, target_shape, space)` 的静态目标 shape 应生成显式 `nn.broadcast`。
CASE_1_IR = """builtin.module {
  func.func @broadcast_to_kernel_case_1(%0 : !nn.memory<[1, 4], [4, 1], f32, #nn.space<shared>>) -> !nn.memory<[3, 4], [4, 1], f32, #nn.space<local>> {
    %1 = "nn.broadcast"(%0) {space = #nn.space<local>} : (!nn.memory<[1, 4], [4, 1], f32, #nn.space<shared>>) -> !nn.memory<[3, 4], [4, 1], f32, #nn.space<local>>
    func.return %1 : !nn.memory<[3, 4], [4, 1], f32, #nn.space<local>>
  }
}"""
...
# CASE-2 IR：symbol 正向例子：`broadcast_to(source, target_shape, space)` 的 symbol 目标 shape 应生成显式 `nn.broadcast`。
CASE_2_IR = """builtin.module {
  func.func @broadcast_to_kernel_case_2(%0 : !nn.memory<[1, K], [K, 1], f32, #nn.space<shared>>, %1 : !symbol.int<"M">, %2 : !symbol.int<"K">) -> !nn.memory<[M, K], [K, 1], f32, #nn.space<local>> {
    %3 = "nn.broadcast"(%0) {space = #nn.space<local>} : (!nn.memory<[1, K], [K, 1], f32, #nn.space<shared>>) -> !nn.memory<[M, K], [K, 1], f32, #nn.space<local>>
    func.return %3 : !nn.memory<[M, K], [K, 1], f32, #nn.space<local>>
  }
}"""
...
# CASE-3 描述：失败例子：`broadcast_to(source, target_shape, space)` 的目标 rank 小于输入 rank 时应在构造阶段拒绝。
CASE_3_RUNTIME_ARGS = (
    Memory([1, 4], NumericType.Float32, space=MemorySpace.SM, format=Farmat.CLast),
)
...
def _case_3_reject_target_rank_too_small():
    ...
    try:
        ...
        assert "broadcast_to target rank must be >= input rank" in str(exc)
    else:
        raise AssertionError("broadcast_to with target rank smaller than input rank should be rejected before MLIR compare")
...
def main():
    ...
    run_case(failures, "CASE-3", _case_3_reject_target_rank_too_small)
    raise_if_failures("dsl mlir_gen nn broadcast_to expectation", failures)
