"""arch.get_thread_id expectation.

创建者: 榕
最后一次更改: 金铲铲大作战

功能说明:
- 使用统一 expectation 结构描述 `arch.get_thread_id` 的 DSL 调用方式。
- 验证 `build_func_op` 可将无参函数 lowering 为 `arch.get_thread_id`。
...
func_op = build_func_op(get_thread_id_kernel)
assert isinstance(func_op, FuncOp)

query_ops = [op for op in func_op.body.block.ops if isinstance(op, ArchGetThreadIdOp)]
return_ops = [op for op in func_op.body.block.ops if isinstance(op, ReturnOp)]

assert len(query_ops) == 1
assert_dynamic_symbol_int(query_ops[0].result.type, "thread_id")
assert len(return_ops) == 1
assert_dynamic_symbol_int(return_ops[0].arguments[0].type, "thread_id")
