...
- 验证 `build_func_op` 可将无参函数 lowering 为 `arch.get_block_id`。
...
query_ops = [op for op in func_op.body.block.ops if isinstance(op, ArchGetBlockIdOp)]
return_ops = [op for op in func_op.body.block.ops if isinstance(op, ReturnOp)]

assert len(query_ops) == 1
assert_dynamic_symbol_int(query_ops[0].result.type, "block_id")
assert len(return_ops) == 1
assert_dynamic_symbol_int(return_ops[0].arguments[0].type, "block_id")
