"""symbol.lt expectation.

创建者: 榕
最后一次更改: 榕

功能说明:
- 使用函数 + `build_func_op` 组织 `symbol.lt` 的目标态 expectation。
- 验证静态整数、动态符号以及混合输入都可以成功 lowering 为 `symbol.lt`。

使用示例:
- python expectation/temp/symbol/lt.py
...
def check_case(lhs_value: int | SymbolDim, rhs_value: int | SymbolDim) -> None:
    func_op = build_func_op(lt_func, lhs_value, rhs_value)
    ...
    lt_ops = [op for op in func_op.body.block.ops if isinstance(op, SymbolLtOp)]
    return_ops = [op for op in func_op.body.block.ops if isinstance(op, ReturnOp)]

    assert len(lt_ops) == 1
    assert lt_ops[0].result.type == i1
