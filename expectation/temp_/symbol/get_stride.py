"""symbol.get_stride expectation.

创建者: 榕
最后一次更改: 榕

功能说明:
- 使用函数 + `build_func_op` 组织 `symbol.get_stride` 的目标态 expectation。
- 验证从静态 stride 和符号 stride 中读取步幅都可以成功 lowering 为 `symbol.get_stride`。

使用示例:
- python expectation/temp/symbol/get_stride.py
...
def get_stride_static(value: f"Tensor[f32, {STATIC_DIM0}, {STATIC_DIM1}]") -> int:
    return value.get_stride()[0]


def get_stride_dynamic(value: f"Tensor[f32, {SYMBOL_DIM0_NAME}, {SYMBOL_DIM1_NAME}]") -> int:
    return value.get_stride()[0]


def check_static_case() -> None:
    func_op = build_func_op(get_stride_static, STATIC_MEMORY)
    assert isinstance(func_op, FuncOp)

    stride_ops = [op for op in func_op.body.block.ops if isinstance(op, SymbolGetStrideOp)]
    return_ops = [op for op in func_op.body.block.ops if isinstance(op, ReturnOp)]

    assert len(stride_ops) == 1
    assert_static_symbol_int(stride_ops[0].result.type, STATIC_DIM1)
    assert len(return_ops) == 1
    assert_static_symbol_int(return_ops[0].arguments[0].type, STATIC_DIM1)


def check_dynamic_case() -> None:
    func_op = build_func_op(get_stride_dynamic, DYNAMIC_MEMORY)
    assert isinstance(func_op, FuncOp)
    ...
    assert len(stride_ops) == 1
    assert_dynamic_symbol_int(stride_ops[0].result.type, SYMBOL_DIM1)
    assert len(return_ops) == 1
