"""symbol.le expectation.

创建者: 榕
最后一次更改: 榕

功能说明:
- 使用函数 + `build_func_op` 组织 `symbol.le` 的目标态 expectation。
- 验证静态整数、动态符号以及混合输入都可以成功 lowering 为 `symbol.le`。

使用示例:
- python expectation/temp/symbol/le.py
...
def check_case(lhs_value: int | SymbolDim, rhs_value: int | SymbolDim) -> None:
    func_op = build_func_op(le_func, lhs_value, rhs_value)
    ...
    le_ops = [op for op in func_op.body.block.ops if isinstance(op, SymbolLeOp)]
    return_ops = [op for op in func_op.body.block.ops if isinstance(op, ReturnOp)]

    assert len(le_ops) == 1
    assert le_ops[0].result.type == i1
