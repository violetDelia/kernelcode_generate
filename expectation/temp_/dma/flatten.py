...
功能说明:
- 使用统一 expectation 结构描述 `flatten(...)` 的 DSL 调用方式。
- 参数与 Memory shape 使用随机整数，便于覆盖不同输入规模。

使用示例:
- python expectation/temp/dma/flatten.py
...
FLAT_SIZE = SOURCE_ROWS * SOURCE_COLS
SOURCE_MEMORY = Memory([SOURCE_ROWS, SOURCE_COLS], NumericType.Float32, space=MemorySpace.GM)


def flatten_kernel(src: f"Tensor[f32, {SOURCE_ROWS}, {SOURCE_COLS}]") -> f"Tensor[f32, {FLAT_SIZE}]":
    return flatten(src)


func_op = build_func_op(flatten_kernel, SOURCE_MEMORY)
