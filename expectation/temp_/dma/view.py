...
功能说明:
- 使用统一 expectation 结构描述 `view(...)` 的 DSL 调用方式。
- 参数与 Memory shape 使用随机整数，便于覆盖不同输入规模。

使用示例:
- python expectation/temp/dma/view.py
...
VIEW_ROWS = get_random_non_zero_int(1, SOURCE_ROWS)
VIEW_COLS = get_random_non_zero_int(1, SOURCE_COLS)
OFFSET_ROW = get_random_int(0, SOURCE_ROWS - VIEW_ROWS)
OFFSET_COL = get_random_int(0, SOURCE_COLS - VIEW_COLS)
SOURCE_MEMORY = Memory([SOURCE_ROWS, SOURCE_COLS], NumericType.Float32, space=MemorySpace.GM)


def view_kernel(src: f"Tensor[f32, {SOURCE_ROWS}, {SOURCE_COLS}]") -> f"Tensor[f32, {VIEW_ROWS}, {VIEW_COLS}]":
    return view(src, [OFFSET_ROW, OFFSET_COL], [VIEW_ROWS, VIEW_COLS], [1, 1])


func_op = build_func_op(view_kernel, SOURCE_MEMORY)
