...
功能说明:
- 使用统一 expectation 结构描述 `slice(...)` 的 DSL 调用方式。
- 参数与 Memory shape 使用随机整数，便于覆盖不同输入规模。

使用示例:
- python expectation/temp/dma/slice.py
...
def slice_kernel(src: f"Tensor[f32, {SOURCE_ROWS}, {SOURCE_COLS}]") -> f"Tensor[f32, {TILE_ROWS}, {TILE_COLS}]":
    return slice(src, [OFFSET_ROW, OFFSET_COL], [TILE_ROWS, TILE_COLS], [1, 1], MemorySpace.LM)


func_op = build_func_op(slice_kernel, SOURCE_MEMORY)
