...
功能说明:
- 使用统一 expectation 结构描述 `copy(...)` 的 DSL 调用方式。
- 参数与 Memory shape 使用随机整数，便于覆盖不同输入规模。

使用示例:
- python expectation/temp/dma/copy.py
...
def copy_kernel(src: f"Tensor[f32, {SOURCE_ROWS}, {SOURCE_COLS}]") -> f"Tensor[f32, {SOURCE_ROWS}, {SOURCE_COLS}]":
    return copy(src, MemorySpace.SM)


func_op = build_func_op(copy_kernel, SOURCE_MEMORY)
assert isinstance(func_op, FuncOp)

copy_ops = [op for op in func_op.body.block.ops if isinstance(op, DmaCopyOp)]
