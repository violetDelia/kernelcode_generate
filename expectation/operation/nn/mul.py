...
功能说明:
- 验证 nn.mul 支持 Memory/Memory 逐元素相乘。
- 验证 lhs/rhs dtype 不一致时，结果返回较低优先级的 dtype。

使用示例:
- python expectation/operation/nn/mul.py
...
from kernel_gen.operation.nn import mul
...
layout_result = mul(lhs_layout, rhs_layout)
...
with pytest.raises(ValueError):
    mul(same_lhs, Memory([invalid_dim], NumericType.Int32))
