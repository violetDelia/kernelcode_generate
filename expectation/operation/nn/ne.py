...
功能说明:
- 验证 nn.ne 返回 predicate dtype 的 Memory。

使用示例:
- python expectation/operation/nn/ne.py
...
from kernel_gen.operation.nn import ne
...
broadcast_result = ne(broadcast_lhs, rhs)

assert broadcast_result.get_shape() == [s1, s2]
assert broadcast_result.get_type() is NumericType.Bool

# 标量与 Memory 的比较应保持 Memory 的 shape。
scalar_result = ne(0, rhs)
...
with pytest.raises(ValueError):
    ne(rhs, Memory([invalid_dim], NumericType.Float32))
