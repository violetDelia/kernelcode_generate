"""NN floordiv expectation.
[immutable-file]
创建者: 榕
最后一次更改: 榕

功能说明:
- 验证 nn.floordiv 支持 Memory/Memory 逐元素整除。
- 验证 lhs/rhs dtype 不一致时，结果返回较低优先级的 dtype。

使用示例:
- python expectation/operation/nn/floordiv.py
...
from kernel_gen.operation.nn import floordiv
...
layout_result = floordiv(lhs_layout, rhs_layout)
...
with pytest.raises(ValueError):
    floordiv(same_lhs, Memory([invalid_dim], NumericType.Int32))
