"""NN truediv expectation.

创建者: 榕
最后一次更改: 榕

功能说明:
- 验证 nn.truediv 支持 Memory/Memory 逐元素相除。
- 验证 lhs/rhs dtype 不一致时，结果返回较低优先级的 dtype。

使用示例:
- python expectation/operation/nn/truediv.py
...
from kernel_gen.operation.nn import truediv
...
# 相同 shape 的除法应保持原有的 Memory 描述。
same_lhs = Memory([s1, s2], NumericType.Int32)
same_rhs = Memory([s1, s2], NumericType.Int32)
same_result = truediv(same_lhs, same_rhs)

assert isinstance(same_result, Memory)
assert same_result == same_lhs

# singleton 维度应按隐式 broadcast 规则扩张到目标 shape。
broadcast_rhs = Memory([1, s2], NumericType.Int32)
broadcast_result = truediv(same_lhs, broadcast_rhs)

assert broadcast_result.get_shape() == [s1, s2]
assert broadcast_result.get_type() is NumericType.Int32

# 与标量相除时，应保持原 shape 与 dtype。
scalar_result = truediv(same_lhs, 3)

assert scalar_result.get_shape() == [s1, s2]
assert scalar_result.get_type() is NumericType.Int32

# 不同 dtype 的除法应遵循约定的类型提升规则。
lhs = Memory([s1, s2], NumericType.Int32)
rhs = Memory([s1, s2], NumericType.Float32)
result = truediv(lhs, rhs)
...
lhs_layout = Memory([s1, s2], NumericType.Int32, stride=[1, 1], format=Farmat.CLast)
rhs_layout = Memory([s1, s2], NumericType.Int32, stride=[2, 1], format=Farmat.Norm)
layout_result = truediv(lhs_layout, rhs_layout)
...
with pytest.raises(ValueError):
    truediv(same_lhs, Memory([invalid_dim], NumericType.Int32))
