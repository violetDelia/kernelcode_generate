...
功能说明:
- 验证 `Memory != Memory` 可通过 `build_func_op` 生成 `func op`，且函数体包含 `nn.ne`。
- 验证对该 `func op` 运行 `LowerNnToKernelPass` 后，`nn.ne` 被 lower 为 `kernel.ne`，并插入 `dma.alloc`。

使用示例:
- python expectation/pass/lowing/nn_to_kernel/ne.py
...
"""
# Case 列表:
# - Case-1: 参数合法：两个同 shape Memory 先生成 nn.ne，再执行 lowering，验证替换为 kernel.ne 并插入 dma.alloc。
...
from kernel_gen.dialect.nn import NnNeOp
...
print("[CASE-1] 参数合法：两个同 shape Memory 先生成 nn.ne，再执行 lowering，验证替换为 kernel.ne 并插入 dma.alloc。")
func_op = build_func_op(ne_memory, lhs_memory, rhs_memory)
print(func_op)

before_ops = list(func_op.body.block.ops)
assert any(isinstance(op, NnNeOp) for op in before_ops)

module = ModuleOp([func_op])
LowerNnToKernelPass().run(module)

after_ops = list(func_op.body.block.ops)
assert any(op.name == "kernel.ne" for op in after_ops)
