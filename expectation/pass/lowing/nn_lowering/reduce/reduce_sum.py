"""pass nn_lowering reduce expectation：reduce_sum。
[immutable-file]
...
功能说明:
- 使用完整 `CASE_TEXT` 运行 `ircheck` 版 `reduce_sum` expectation。

使用示例:
- `PYTHONPATH=. python expectation/pass/lowing/nn_lowering/reduce/reduce_sum.py`
...
# Case 列表:
# - Case-1: 正例：静态 nn.reduce_sum 输入应 lower 为 dma.alloc + kernel.reduce(kind=sum) + func.return。
# - Case-2: 正例：符号维度 nn.reduce_sum 输入应 lower 为 dma.alloc + kernel.reduce(kind=sum) + func.return。
...
// CHECK-NEXT: func.func @reduce_sum_kernel(%[[ARG0:{{reg}}]] : {STATIC_SRC}) -> {STATIC_DST} {{
// CHECK-NEXT: %[[V0:{{reg}}]] = "dma.alloc"() <{{operandSegmentSizes = array<i32: 0>}}> : () -> {STATIC_DST}
// CHECK-NEXT: "kernel.reduce"(%[[V0]], %[[ARG0]]) {{axis = 1 : i64, keepdim = true, kind = "sum", space = {SPACE_ATTR}}} : ({STATIC_DST}, {STATIC_SRC}) -> ()
...
builtin.module {{
  func.func @reduce_sum_kernel(%arg0: {STATIC_SRC}) -> {STATIC_DST} {{
    %0 = "nn.reduce_sum"(%arg0) {{axes = [#builtin.int<1>], keepdim = #builtin.int<1>, space = {SPACE_ATTR}}} : ({STATIC_SRC}) -> {STATIC_DST}
...
// CHECK: builtin.module {{
// CHECK-NEXT: func.func @reduce_sum_kernel(%[[ARG0:{{reg}}]] : {DYNAMIC_SRC}) -> {DYNAMIC_DST} {{
...
// CHECK-NEXT: "kernel.reduce"(%[[V1]], %[[ARG0]]) {{axis = 1 : i64, keepdim = true, kind = "sum", space = {SPACE_ATTR}}} : ({DYNAMIC_DST}, {DYNAMIC_SRC}) -> ()
...
builtin.module {{
  func.func @reduce_sum_kernel(%arg0: {DYNAMIC_SRC}) -> {DYNAMIC_DST} {{
    %0 = "nn.reduce_sum"(%arg0) {{axes = [#builtin.int<1>], keepdim = #builtin.int<1>, space = {SPACE_ATTR}}} : ({DYNAMIC_SRC}) -> {DYNAMIC_DST}
...
def main() -> None:
    """运行 `reduce_sum` ircheck expectation。"""

    run_single("reduce_sum", CASE_TEXT_STATIC, CASE_TEXT_DYNAMIC)
