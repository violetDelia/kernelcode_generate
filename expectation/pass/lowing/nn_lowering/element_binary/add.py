"""pass nn_lowering element_binary expectation：add。
[immutable-file]
...
功能说明:
- 使用完整 `CASE_TEXT` 运行 `ircheck` 版 `add` expectation。
...
# Case 列表:
# - Case-1: 正例：静态 nn.add memory + memory 输入应 lower 为 dma.alloc + kernel.binary_elewise(kind=add) + func.return。
# - Case-2: 正例：静态 nn.add memory + const 输入应 lower 为 dma.alloc + dma.fill + kernel.binary_elewise(kind=add) + func.return。
# - Case-3: 正例：符号维度 nn.add memory + memory 输入应 lower 为 symbol.get_dim + dma.alloc + kernel.binary_elewise(kind=add) + func.return。
# - Case-4: 正例：符号维度 nn.add memory + symbol 输入应 lower 为 symbol.get_dim + dma.alloc + dma.fill + kernel.binary_elewise(kind=add) + func.return。
# - Case-5: 正例：符号维度 nn.add memory + (const + symbol) 输入应 lower 为 symbol.get_dim + dma.alloc + dma.fill + kernel.binary_elewise(kind=add) + func.return。
