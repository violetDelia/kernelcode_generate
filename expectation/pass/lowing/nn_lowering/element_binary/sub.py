"""pass nn_lowering element_binary expectation：sub。
[immutable-file]
...
功能说明:
- 使用完整 `CASE_TEXT` 运行 `ircheck` 版 `sub` expectation。

使用示例:
- `PYTHONPATH=. python expectation/pass/lowing/nn_lowering/element_binary/sub.py`
...
# Case 列表:
# - Case-1: 正例：静态 nn.sub memory + memory 输入应 lower 为 dma.alloc + kernel.binary_elewise(kind=sub) + func.return。
# - Case-2: 正例：静态 nn.sub memory + const 输入应 lower 为 dma.alloc + dma.fill + kernel.binary_elewise(kind=sub) + func.return。
# - Case-3: 正例：符号维度 nn.sub memory + memory 输入应 lower 为 symbol.get_dim + dma.alloc + kernel.binary_elewise(kind=sub) + func.return。
# - Case-4: 正例：符号维度 nn.sub memory + symbol 输入应 lower 为 symbol.get_dim + dma.alloc + dma.fill + kernel.binary_elewise(kind=sub) + func.return。
# - Case-5: 正例：符号维度 nn.sub memory + (const + symbol) 输入应 lower 为 symbol.get_dim + dma.alloc + dma.fill + kernel.binary_elewise(kind=sub) + func.return。
