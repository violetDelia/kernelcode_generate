...
- 承担 `合并` 任务时，默认一次性收口：`合并提交 + 单次 push + cleanup（worktree/branch）+ -done 回报`；任务完成即视为该链路在你侧结束，不再自动派生后续任务。
- 承担 `同步确认` 任务时，只负责核对指定提交是否已进入 `origin/main`；若未进入则按任务指令执行推送并回报命令与退出码。
...
7. 完成后必须自行执行 `-done`（管理员不代为执行 `-done`）；随后用脚本回报完成状态、记录路径与必要细节：`codex-multi-agents-tmux.sh -talk -from <your_name> -to <管理员> -agents-list agents/codex-multi-agents/agents-lists.md -message "完成 <task_id>。记录：<记录文件>；结论/文件/测试/阻塞/建议：<简述>" -log agents/codex-multi-agents/log/talk.log`
8. 合并任务在执行 `-done` 后直接进入待命，不创建后续任务；仅当管理员明确、单独下达 `-new` 指令时，才允许创建一个后续任务。

## 回报规范
- 完成回报必须说明：任务 ID、结论、涉及文件、影响范围、测试情况、阻塞点与改进建议。
- 回报默认不新建后续任务；合并任务完成即链路收口。仅当管理员明确要求时，才创建且一次只建一个，并通知管理员分发。
