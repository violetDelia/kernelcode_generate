...
4. 完成后执行 `-next`；成功后必须立刻 `-talk` 管理员，固定回报：`<当前task_id> 已完成，已新建后续任务 <next_task_id>，请推进。`；`-next` 消息必须按 [任务新建模板](../../../standard/任务新建模板.md) 单行格式，并优先写推荐类型 `实现`；若 `-next` 失败，立即 `-talk` 回报错误与记录文件。
