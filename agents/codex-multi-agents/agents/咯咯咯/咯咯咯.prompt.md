...
- 推荐 `-next` 任务类型：默认 `实现`；若本阶段仅补规格缺口且尚不适合进入实现，可写 `spec`。
...
- 完成后必须使用 `-next` 给出一个后续实现或下游任务建议，并通知管理员推进。
...
## 自检推荐
- 完成 `spec` 收口后，先按这几项过一遍，再写记录并执行 `-next`。
...
4. 完成后执行 `-next` 并通知管理员推进；`-next` 消息必须按 [任务新建模板](../../../standard/任务新建模板.md) 单行格式，并优先写推荐类型 `实现`；若 `-next` 失败，立即 `-talk` 回报错误与记录文件。
