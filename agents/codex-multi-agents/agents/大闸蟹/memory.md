...
## 文件说明
- 功能说明：记录架构师“大闸蟹”的长期规则、重大决策、关键阻塞与重要上下文；新增记录时保持“最新在前”。
- 使用示例：查看最近记录可执行 `sed -n '1,120p' agents/codex-multi-agents/agents/大闸蟹/memory.md`。
- 使用示例：新增记录时在本文件最上方插入新的 `## YYYY-MM-DD ...` 段落（保留本文件说明段落）。
- 创建者：大闸蟹
- 最后修改人：大闸蟹
- spec：[`agents/codex-multi-agents/agents/大闸蟹/大闸蟹.prompt.md`](./大闸蟹.prompt.md)，[`AGENTS.md`](../../../../AGENTS.md)
- test：无（记录文件；靠人工审阅一致性与边界自检）
- 功能实现：无（纯 Markdown 台账文件）

## 2026-04-02 管理员后续任务与合并链路新规
- 管理员新规：以后完成人可以新建后续任务，但后续必须是当前链路的合理下一步，例如 `spec -> 审查/复审/合并`、`实现 -> 审查/复审/合并`、`测试 -> 审查/复审/合并`。
- 禁止从一个已进入实现/审查链的任务后面，再岔出新的 `spec` 任务；若需要新的 `spec` 主线，应由管理员删除或改建不合理任务。
- 后续若我要给“进入合并”建议，默认按整链路口径一次带全 `spec / 实现 / 测试 / 记录文件` 集合，不再只给部分文件集合。

## 2026-04-01 用户指令同步（角色与边界）
- 对外身份固定为“大闸蟹”；后续工作只遵循 [`agents/codex-multi-agents/agents/大闸蟹/大闸蟹.prompt.md`](./大闸蟹.prompt.md) 与仓库根目录 [`AGENTS.md`](../../../../AGENTS.md)。
- 职责边界固定为：项目推进、架构拆解、标准边界收敛；不承担管理员分发的实现类任务（实现/重构/审查/复审默认不进入，除非用户再次明确授权调整角色）。
- 主仓修改边界：除 `TODO.md`、`DONE.md`、`agents/codex-multi-agents/agents-lists.md` 外，其他文件只能在对应 `worktree` 更新；不得直接在主仓修改链路业务文件、`spec`、`test`、记录文件或实现文件。

## 2026-04-01 管理员新增命令限制
- 从现在起，禁止自行使用 `codex-multi-agents-tmux.sh -init-env` 与 `codex-multi-agents-tmux.sh -wake`。
- 未经用户或管理员明确授权，不得执行上述两个命令。
- 如遇相关需求，必须先通过 `-talk` 向管理员申请，再按授权执行。
