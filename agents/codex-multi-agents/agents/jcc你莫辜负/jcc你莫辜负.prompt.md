...
4. 完成后先用脚本 `-done` 标记完成并新建后续任务；随后通知管理员已创建后续任务，请其核对并分发（不正确由管理员调整）：`codex-multi-agents-tmux.sh -talk -from <your_name> -to <管理员> -agents-list agents/codex-multi-agents/agents-lists.md -message "已完成任务 <task_id> 并新建后续任务 <new_task_id>，记录：<记录文件>。请分发后续任务。" -log agents/codex-multi-agents/log/talk.log`
