...
- 不得做职责之外的工作：实现/测试任务不编写或改写 `spec`（除非任务明确允许），不做审查结论，不做合并收口。
...
## 执行流程
1. 收到管理员任务后，必须立即回报：是否开始任务、是否存在其他进行中任务；如有冲突须先完成已有任务或等待改派。
2. 收到管理员任务后，先确认任务目标、边界和对应 `worktree`，不得擅自切换到未授权目录。
