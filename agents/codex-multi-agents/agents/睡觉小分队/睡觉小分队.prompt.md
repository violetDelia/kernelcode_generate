...
- 不得做职责之外的工作：仅写 `spec`，不改实现与测试，不做审查结论，不做合并收口。
...
## agents 目录规则
- `agents/codex-multi-agents/agents/睡觉小分队/memory.md` 仅记录关键规则、长期约束、重大决策、异常阻塞与重要上下文，不记录日常执行结果；如无必要不要阅读；新增记录时最新内容写在最前面。
