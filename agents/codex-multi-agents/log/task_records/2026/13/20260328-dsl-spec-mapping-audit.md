...
- 验证：未执行测试（本次为标注/映射核对）。
- 时间：`2026-03-29 13:47:05 +0800`
- 任务：`T-20260329-4c03fb00`
- 任务目标：将 EMIT 标注映射修复（`spec/dsl/emit_mlir.md`、`spec/dsl/mlir_gen.md`）合并到 `origin/main`，并执行 cleanup。
- 改动：
  - 在 `origin/main` 基础新建合并分支 `li-bai-merge-T-20260329-4c03fb00`，导入提交 `655e725` 的两个 spec 文件改动。
  - 纳入文件：`spec/dsl/emit_mlir.md`、`spec/dsl/mlir_gen.md`、本任务记录文件。
  - 合并范围外文件未纳入（符合合并规范）。
  - 测试：未执行（本次为文档映射合并任务）。
- 结论：`已完成合并内容准备`，下一步执行提交、push、`-done` 与 worktree/branch cleanup。
