...
- 问题列表：
  - 文件范围：worktree 中存在 `spec/pass/lowering/nn_lowering/spec.md`，主仓无该文件，且不在 S1 可改文件与记录“改动”中；若合入会引入未审查的额外 spec（优先级：P1）。建议：移出本任务或拆分独立任务并补记录。
- 漏洞排查结果：
...
验证：
- `diff -qr spec/pass wt-20260411-pass-pipeline-s1/spec/pass` -> README/registry 变更；新增 `spec/pass/pipeline` 目录；且仅 worktree 内存在 `spec/pass/lowering/nn_lowering/spec.md`。
...
结论：需修改。原因：文件范围不一致（`spec/pass/lowering/nn_lowering/spec.md` 未在 S1 允许范围与记录中）；请先处理范围问题后再继续 review。
...
改动：
- 复核范围：worktree 仅变更 `spec/pass/README.md`、`spec/pass/registry.md` 与 `spec/pass/pipeline/*`，`spec/pass/lowering/nn_lowering/spec.md` 已移出。
...
改动：删除 worktree 中的 spec/pass/lowering/nn_lowering/spec.md，避免引入额外 spec；保留本任务范围内的 README/registry/pipeline 变更
...
- 已恢复 `spec/pass/lowering/nn_lowering/spec.md` 为主线内容，避免引入额外删除。
验证：
