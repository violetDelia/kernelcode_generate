...
任务目标：在 `spec/pass/lowering/dma_memory_hierarchy.md` 补 dynamic shape / symbol 透传与显式失败边界，并在允许范围内同步实现与测试，锁定匿名 `?` 与目标缺失 `SM/LM` 的失败口径。
改动：
- 更新 `spec/pass/lowering/dma_memory_hierarchy.md`：新增 staging `dma.alloc(dynamic_shape=...)` 必须来自显式 symbol 来源的合同；明确匿名 `?` 且无可恢复 shape 来源时必须以包含 `dynamic_shape` 的错误短语失败；补充 `COV-DMH-009` / `COV-DMH-010` 测试映射，并把窗口链路 wording 同步到当前 unit stride 合同。
...
- 引用文件：`ARCHITECTURE/plan/dma_memory_hierarchy_lowering_green_plan.md`、`spec/pass/lowering/dma_memory_hierarchy.md`、`kernel_gen/passes/lowering/dma_memory_hierarchy.py`、`test/pass/test_dma_memory_hierarchy.py`。
结论：
...
- 下一步建议：派生唯一审查任务，对本次 `spec/pass/lowering/dma_memory_hierarchy.md`、`kernel_gen/passes/lowering/dma_memory_hierarchy.py`、`test/pass/test_dma_memory_hierarchy.py` 的一致性进行复核。
...
改动：
- 核对文件范围与一致性：`spec/pass/lowering/dma_memory_hierarchy.md`、`kernel_gen/passes/lowering/dma_memory_hierarchy.py`、`test/pass/test_dma_memory_hierarchy.py`。
