...
请推进“正在执行的任务”并分发“任务列表”中可分发任务。如果又完成的任务一个一个询问架构师验收,验收通过后归档。
EOF

# busy 执行人提醒消息（后发送；同一轮会逐一发送）。
read -r -d '' BUSY_MESSAGE <<'EOF' || true
查看TODO.md, 继续你的任务，完成后使用 -next 并回报管理员,如果发现之前执行过相同的任务,说明任务又流转到你身上,查看日志,继续按照任务要求执行。,有任何阻塞/疑问再次回报管理/架构师，请求对方回复。
EOF
