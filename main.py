...
def _is_npu_demo_wrapper(func_op: func.FuncOp) -> bool:
    ...
    - body kernel 则不含 `arch.launch`，但有 `npu_demo::KernelContext& ctx` 签名。
...
def _select_npu_demo_source_functions(module: ModuleOp) -> tuple[func.FuncOp, func.FuncOp]:
    ...
    功能说明:
    - 要求 module 恰好包含两个 `func.func`：一个 wrapper，一个 device kernel。
    - wrapper 通过 `arch.launch` 识别；另一条 func 视作 device kernel。
    - 若结构不符合当前 S3 目标形态，显式失败，避免 main.py 静默打印错误区段。
    ...
    if len(func_ops) != 2:
        raise RuntimeError(f"expected exactly two func.func ops after npu-demo-lowering, got {len(func_ops)}")
    wrapper_func = next((func_op for func_op in func_ops if _is_npu_demo_wrapper(func_op)), None)
    if wrapper_func is None:
        raise RuntimeError("npu-demo-lowering module does not contain a unique arch.launch wrapper")
    body_func = next(func_op for func_op in func_ops if func_op is not wrapper_func)
...
def _strip_npu_demo_prelude(source: str) -> str:
    ...
    使用示例:
    - body_source = _strip_npu_demo_prelude(gen_kernel(wrapper_func, EmitCContext(target="npu_demo")))
...
def matmul_kernel(
    ...
    使用示例:
    - `dsl_run(matmul_kernel, (out, lhs, rhs), "npu-demo-lowering", EmitCContext(target="npu_demo"))`
