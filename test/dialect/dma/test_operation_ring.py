...
def test_dma_ring_type_and_ops_verify_success() -> None:
    ...
    num, offset = _make_symbol_operands([3, 256])

    DmaMakeRingOp(storage, num, offset, ring_type).verify()
...
def test_dma_make_ring_verifier_edges() -> None:
    backing_type = _make_memory_type(shape=_dim_array([32]), stride=_dim_array([1]), space="tsm", element_type=i8)
    slot_type = _make_memory_type(shape=_dim_array([2, 2]), stride=_dim_array([2, 1]), space="tsm")
    ring_type = DmaRingType(slot_type)
    storage = _TestOp(result_types=[backing_type]).results[0]
    num, offset = _make_symbol_operands([2, 16])
    DmaMakeRingOp(storage, num, offset, ring_type).verify()

    with pytest.raises(KernelCodeError, match="count must be > 0") as count_error:
        DmaMakeRingOp(storage, _make_symbol_operands([0])[0], offset, ring_type).verify()
    ...
    with pytest.raises(KernelCodeError, match="offset must be > 0"):
        DmaMakeRingOp(storage, num, _make_symbol_operands([0])[0], ring_type).verify()
    with pytest.raises(KernelCodeError, match="slot span bytes.*offset"):
        DmaMakeRingOp(storage, num, _make_symbol_operands([15])[0], ring_type).verify()
    with pytest.raises(KernelCodeError, match="backing memory bytes"):
        DmaMakeRingOp(
            _TestOp(result_types=[_make_memory_type(shape=_dim_array([31]), stride=_dim_array([1]), space="tsm", element_type=i8)]).results[0],
            num,
            offset,
            ring_type,
        ).verify()
    ...
    with pytest.raises(KernelCodeError, match="one-dimensional i8"):
        DmaMakeRingOp(
            _TestOp(result_types=[_make_memory_type(shape=_dim_array([32]), stride=_dim_array([1]), space="tsm")]).results[0],
            num,
            offset,
            ring_type,
        ).verify()
