...
def test_dma_copy_shape_mismatch() -> None:
    ...
    op = DmaCopyOp(target, source)
    with pytest.raises(KernelCodeError, match="shape mismatch"):
        op.verify()
...
def test_dma_cast_layout_or_space_mismatch() -> None:
    ...
    op = DmaCastOp(_TestOp(result_types=[target_type]).results[0], source)
    with pytest.raises(KernelCodeError, match="dma.cast stride mismatch"):
        op.verify()

    target_type = _make_memory_type(space="shared")
    op = DmaCastOp(_TestOp(result_types=[target_type]).results[0], source)
    with pytest.raises(KernelCodeError, match="dma.cast space mismatch"):
        op.verify()
...
def test_dma_copy_rejects_stride_or_element_type_mismatch() -> None:
    ...
    with pytest.raises(KernelCodeError, match="dma.copy source/target stride mismatch"):
        op.verify()
    ...
    with pytest.raises(KernelCodeError, match="dma.copy source/target element_type mismatch"):
        op.verify()
...
def test_dma_transfer_ops_reject_element_space_or_result_mismatch() -> None:
    ...
    with pytest.raises(KernelCodeError, match="dma.load element_type mismatch"):
        op.verify()
    ...
    with pytest.raises(KernelCodeError, match="dma.store element_type mismatch"):
        op.verify()
    ...
    with pytest.raises(KernelCodeError, match="dma.slice element_type mismatch"):
        op.verify()
...
def test_dma_broadcast_rejects_static_shape_mismatch() -> None:
    ...
    with pytest.raises(KernelCodeError, match="dma.broadcast shape mismatch"):
        DmaBroadcastOp(target, source).verify()
...
def test_dma_broadcast_rejects_scalar_type_mismatch() -> None:
    ...
    with pytest.raises(KernelCodeError, match="dma.broadcast scalar type mismatch"):
        DmaBroadcastOp(target, scalar).verify()
...
def test_dma_transpose_rejects_invalid_perm() -> None:
    ...
    with pytest.raises(KernelCodeError, match="dma.transpose perm"):
