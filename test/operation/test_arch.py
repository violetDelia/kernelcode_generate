...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_block_id_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_block_num_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_thread_id_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_thread_num_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_subthread_id_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_subthread_num_returns_symbol_dim
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_dynamic_memory_returns_dynamic_int8_memory
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_get_dynamic_memory_rejects_invalid_space
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_get_dynamic_memory_rejects_invalid_space() -> None:
    with pytest.raises(KernelCodeError, match="space must be MemorySpace"):
        get_dynamic_memory("SM")
    with pytest.raises(KernelCodeError, match="space must be on-chip MemorySpace"):
        get_dynamic_memory(MemorySpace.GM)


# TC-OP-ARCH-009
# 测试目的: 验证 barrier 接受合法 visibility/scope 并返回 None。
# 使用示例: pytest -q test/operation/test_arch.py -k test_barrier_accepts_valid_arguments
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_barrier_rejects_invalid_arguments
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_barrier_rejects_invalid_arguments() -> None:
    ...
    with pytest.raises(KernelCodeError, match="visibility must be list\\[BarrierVisibility\\] or tuple\\[BarrierVisibility, \\.\\.\\.\\]"):
        barrier(visibility="TSM/TLM", scope=BarrierScope.BLOCK)
    with pytest.raises(KernelCodeError, match="visibility items must be BarrierVisibility"):
    ...
    with pytest.raises(KernelCodeError, match="visibility must contain TSM and TLM exactly once"):
        barrier(visibility=[BarrierVisibility.TSM], scope=BarrierScope.BLOCK)
    with pytest.raises(KernelCodeError, match="scope must be BarrierScope"):
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_launch_kernel_accepts_valid_extents_and_kernel_args
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_launch_kernel_accepts_valid_extents_and_kernel_args() -> None:
    captured: dict[str, tuple[str, str, str] | str | int] = {}

    def add_barrier_body(lhs: str, rhs: str, out: str) -> None:
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_launch_kernel_rejects_invalid_arguments
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_launch_kernel_rejects_invalid_arguments() -> None:
    ...
    with pytest.raises(KernelCodeError, match="callee must be function object"):
        launch_kernel[1, 1, 1, 0]("my_kernel")
    with pytest.raises(KernelCodeError, match="callee must be function object"):
        launch_kernel[1, 1, 1, 0](object())
    with pytest.raises(KernelCodeError, match="block must be int or SymbolDim"):
        launch_kernel["1", 1, 1, 0](kernel_body)
    with pytest.raises(KernelCodeError, match="thread must be int or SymbolDim"):
        launch_kernel[1, object(), 1, 0](kernel_body)
    with pytest.raises(KernelCodeError, match="subthread must be int or SymbolDim"):
        launch_kernel[1, 1, [], 0](kernel_body)
    with pytest.raises(KernelCodeError, match="block must be > 0"):
        launch_kernel[0, 1, 1, 0](kernel_body)
    with pytest.raises(KernelCodeError, match="thread must be > 0"):
        launch_kernel[1, -1, 1, 0](kernel_body)
    with pytest.raises(KernelCodeError, match="subthread must be > 0"):
        launch_kernel[1, 1, 0, 0](kernel_body)
    with pytest.raises(KernelCodeError, match="shared_memory_size must be >= 0"):
        launch_kernel[1, 1, 1, -1](kernel_body)


# TC-OP-ARCH-013
# 测试目的: 验证 launch_kernel 的公开入口固定为下标式，旧直调用与未知关键字都在调用边界报 KernelCodeError。
# 使用示例: pytest -q test/operation/test_arch.py -k test_launch_kernel_call_signature_errors
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_launch_kernel_call_signature_errors() -> None:
    ...
    with pytest.raises(KernelCodeError, match="launch_kernel extents must contain block, thread, subthread, shared_memory_size"):
        launch_kernel[1, 1, 1](kernel_body)


# TC-OP-ARCH-014
# 测试目的: 验证 launch_kernel[...] 是唯一公开调用形态，旧关键字直调用不再成功。
# 使用示例: pytest -q test/operation/test_arch.py -k test_launch_kernel_subscript_form_is_public_api
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_launch_queries_and_memory_prefer_launch_or_hardware_context
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_barrier_and_launch_helpers_reject_unsupported_target_ops
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_barrier_and_launch_helpers_reject_unsupported_target_ops() -> None:
    ...
    try:
        ...
        with pytest.raises(KernelCodeError, match="arch.get_block_num"):
            get_block_num()
        with pytest.raises(KernelCodeError, match="arch.get_thread_id"):
            get_thread_id()
        with pytest.raises(KernelCodeError, match="arch.get_thread_num"):
            get_thread_num()
        with pytest.raises(KernelCodeError, match="arch.get_subthread_num"):
            get_subthread_num()
        with pytest.raises(KernelCodeError, match="arch.get_dynamic_memory"):
            get_dynamic_memory(MemorySpace.SM)
        with pytest.raises(KernelCodeError, match="arch.barrier"):
            barrier(visibility=[BarrierVisibility.TSM, BarrierVisibility.TLM], scope=BarrierScope.BLOCK)
        with pytest.raises(KernelCodeError, match="arch.launch"):
...
# 使用示例: pytest -q test/operation/test_arch.py -k test_query_helpers_reject_missing_target_hardware_fields
# 对应功能实现文件路径: kernel_gen/operation/arch.py
# 对应 spec 文件路径: spec/operation/arch.md
# 对应测试文件路径: test/operation/test_arch.py
def test_query_helpers_reject_missing_target_hardware_fields() -> None:
    ...
    try:
        with pytest.raises(KernelCodeError, match=r"hardware\.block_num"):
            get_block_num()
        with pytest.raises(KernelCodeError, match=r"hardware\.thread_num"):
            get_thread_num()
        with pytest.raises(KernelCodeError, match=r"hardware\.subthread_num"):
            get_subthread_num()
        with pytest.raises(KernelCodeError, match=r"hardware\.sm_memory_size"):
            get_dynamic_memory(MemorySpace.SM)
    finally:
        target_registry.set_current_target(None)
