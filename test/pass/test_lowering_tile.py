"""tile helper path tests.

创建者: 金铲铲大作战
最后一次更改: 金铲铲大作战

功能说明:
- 锁定 `kernel_gen.passes.lowering.tile` 已退回为兼容 helper module，不再公开旧 `TilePass`。
- 覆盖 canonical helper path 已收口到 `kernel_gen.tile.common`，旧路径不再作为默认共享入口。
...
tile_package = importlib.import_module("kernel_gen.tile")
...
def test_tile_helper_module_drops_legacy_public_contract() -> None:
    assert legacy_tile_module.__all__ == ["TilePassError", "_raise_tile_error"]
    assert not hasattr(legacy_tile_module, "TilePass")
    assert not hasattr(legacy_tile_module, "TileAnalysisPass")
    assert not hasattr(legacy_tile_module, "_TileStepValueOp")
    assert not hasattr(legacy_tile_module, "_TileSymbolLiteralOp")
    assert not hasattr(legacy_tile_module, "_read_kernel_split_tile_spec")
    assert not hasattr(legacy_tile_module, "_build_elementwise_loop_specs")
    assert not hasattr(legacy_tile_module, "_build_matmul_loop_specs")
...
# 对应 spec 文件路径: spec/pass/lowering/tile.md
# 对应测试文件路径: test/pass/test_lowering_tile.py
def test_tile_helper_module_keeps_canonical_shared_analysis_helpers() -> None:
    roles = [["elewise", "elewise"], ["expand", "elewise"]]
    attr = tile_common_module._tile_analysis_attr(roles)
    ...
    assert tile_common_module._tile_param_hint("TILE_M0") == "m0"
    assert tile_common_module._tile_param_hint("TILE_D0") is None
    assert tile_package.common is tile_common_module
...
def test_tile_helper_module_validates_input_contract_and_materialization() -> None:
    ...
    assert tile_common_module._is_bool_memory(DmaAllocOp([], bool_type).result)
    assert tile_common_module._is_bool_memory(DmaAllocOp([], bool_numeric_type).result)
    assert tile_common_module._is_allowed_input_contract_op(DmaAllocOp([], mem_type))
    assert tile_common_module._is_allowed_input_contract_op(func.ReturnOp())
    ...
    tile_common_module._validate_input_contract(func_op, allowed_block)

    with pytest.raises(tile_common_module.TilePassError, match="still returns nn.memory results"):
        tile_common_module._validate_input_contract(
            SimpleNamespace(
                sym_name=StringAttr("tile_kernel"),
    ...
    with pytest.raises(tile_common_module.TilePassError, match="still contains nn.add"):
        tile_common_module._validate_input_contract(func_op, nn_block)

    call_block = Block()
    call_block.add_op(func.CallOp("callee", [], []))
    with pytest.raises(tile_common_module.TilePassError, match="must not contain func.call"):
        tile_common_module._validate_input_contract(func_op, call_block)

    unsupported_block = Block()
    unsupported_block.add_op(arith.ConstantOp(IntegerAttr(1, 32)))
    with pytest.raises(tile_common_module.TilePassError, match="contains unsupported op arith.constant"):
        tile_common_module._validate_input_contract(func_op, unsupported_block)
    ...
    with pytest.raises(tile_common_module.TilePassError, match="without dma.alloc carry memory"):
        tile_common_module._validate_intermediate_materialization(carry_func, carry_block)
    ...
    with pytest.raises(tile_common_module.TilePassError, match="without later consumption"):
        tile_common_module._validate_intermediate_materialization(carry_func, dead_block)
...
def test_tile_helper_module_normalizes_compare_and_symbol_helpers() -> None:
    ...
    tile_common_module._normalize_binary_elewise_compare_compat(block)
    ...
    assert tile_common_module._symbol_expr_from_value(SymbolConstOp(7).result) == "7"
    with pytest.raises(tile_common_module.TilePassError, match="symbol value type must be !symbol.int"):
        tile_common_module._symbol_expr_from_value(DmaAllocOp([], out_type).result)

    assert tile_common_module._expr_to_dim_attr("8") == IntAttr(8)
    assert tile_common_module._expr_to_dim_attr("-3") == IntAttr(-3)
    assert tile_common_module._expr_to_dim_attr("TILE_D0") == StringAttr("TILE_D0")
    assert tile_common_module._row_major_stride_exprs([]) == []
    assert tile_common_module._row_major_stride_exprs(["M"]) == ["1"]
    assert tile_common_module._row_major_stride_exprs(["M", "N"]) == ["N", "1"]

    built_symbol = tile_common_module._build_symbol_const(7)
