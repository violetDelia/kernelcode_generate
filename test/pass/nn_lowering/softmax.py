"""nn_lowering softmax tests.

创建者: 金铲铲大作战
最后一次更改: 金铲铲大作战

功能说明:
- 使用 ircheck 文本验证 `nn.softmax` lowering 目标为 `kernel.softmax`。
- 补充输出形态不一致时的错误断言。
...
// CASE: 正例：静态 nn.softmax 输入应 lower 为 dma.alloc + kernel.softmax + func.return。
// CHECK: builtin.module {
// CHECK-NEXT: func.func @softmax_kernel(%arg0 : !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>) -> !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>> {
// CHECK-NEXT: %0 = "dma.alloc"() <{operandSegmentSizes = array<i32: 0>}> : () -> !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>
// CHECK-NEXT: "kernel.softmax"(%arg0, %0) {axis = 1 : i64, space = #nn.space<global>} : (!nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>, !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>) -> ()
// CHECK-NEXT: func.return %0 : !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>
// CHECK-NEXT: }
// CHECK-NEXT: }
// CHECK-NOT: nn.softmax

builtin.module {
  func.func @softmax_kernel(%arg0: !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>) -> !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>> {
    %0 = "nn.softmax"(%arg0) {axis = #builtin.int<1>, space = #nn.space<global>} : (!nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>) -> !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>
    func.return %0 : !nn.memory<[2, 4], [4, 1], f32, #nn.space<global>>
  }
}
"""

CASE_TEXT_DYNAMIC = """// COMPILE_ARGS: --pass lower-nn
// CASE: 正例：符号维度 nn.softmax 输入应 lower 为 dma.alloc + kernel.softmax + func.return。
// CHECK: builtin.module {
// CHECK-NEXT: func.func @softmax_kernel(%arg0 : !nn.memory<[M, N], [N, 1], f32, #nn.space<global>>) -> !nn.memory<[M, N], [N, 1], f32, #nn.space<global>> {
...
// CHECK-NEXT: "kernel.softmax"(%arg0, %2) {axis = 1 : i64, space = #nn.space<global>} : (!nn.memory<[M, N], [N, 1], f32, #nn.space<global>>, !nn.memory<[M, N], [N, 1], f32, #nn.space<global>>) -> ()
...
builtin.module {
  func.func @softmax_kernel(%arg0: !nn.memory<[M, N], [N, 1], f32, #nn.space<global>>) -> !nn.memory<[M, N], [N, 1], f32, #nn.space<global>> {
    %0 = "nn.softmax"(%arg0) {axis = #builtin.int<1>, space = #nn.space<global>} : (!nn.memory<[M, N], [N, 1], f32, #nn.space<global>>) -> !nn.memory<[M, N], [N, 1], f32, #nn.space<global>>
...
def _assert_ircheck_ok(case_text: str, source_path: str) -> None:
    ...
    assert "kernel.softmax" in result.actual_ir, "actual_ir must contain kernel.softmax"
    assert "nn.softmax" not in result.actual_ir, "actual_ir must not contain residual nn.softmax"
...
def _build_module(input_type: NnMemoryType, result_type: NnMemoryType) -> ModuleOp:
    """构造包含单个 nn.softmax 的 module。

    创建者: 金铲铲大作战
    最后一次更改: 金铲铲大作战

    功能说明:
    - 用于 nn.softmax 负例中构造不一致输出类型。
    ...
    block.add_ops([NnSoftmaxOp(block.args[0], result_type, axis=1, space=space), func.ReturnOp()])
    func_op = func.FuncOp(
        "softmax_kernel",
...
def test_nn_lowering_softmax_shape_mismatch() -> None:
    ...
    result_type = _make_memory_type([2, 5])
    module = _build_module(input_type, result_type)
