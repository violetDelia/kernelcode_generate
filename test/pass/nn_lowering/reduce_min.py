...
// CHECK-NEXT: "kernel.reduce"(%arg0, %0) {kind = "min", axis = 1 : i64, keepdim = true, space = #nn.space<global>} : (!nn.memory<[4, 8], [8, 1], f32, #nn.space<global>>, !nn.memory<[4, 1], [1, 1], f32, #nn.space<global>>) -> ()
...
// CHECK-NEXT: "kernel.reduce"(%arg0, %1) {kind = "min", axis = 1 : i64, keepdim = true, space = #nn.space<global>} : (!nn.memory<[M, N], [N, 1], f32, #nn.space<global>>, !nn.memory<[M, 1], [1, 1], f32, #nn.space<global>>) -> ()
...
// CHECK-NEXT: "kernel.reduce"(%arg0, %1) {kind = "min", axis = 0 : i64, keepdim = true, space = #nn.space<global>} : (!nn.memory<[M, N], [N, 1], f32, #nn.space<global>>, !nn.memory<[1, N], [N, 1], f32, #nn.space<global>>) -> ()
