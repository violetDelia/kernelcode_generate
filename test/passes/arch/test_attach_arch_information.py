...
功能说明:
- 覆盖 kernel_gen/passes/arch/attach_arch_information.py 的 launch extent 回写与校验行为。
...
覆盖率命令:
- `pytest -q test/passes/arch/test_attach_arch_information.py`
...
- Spec 文档: spec/pass/arch/attach_arch_information.md
- 测试文件: test/passes/arch/test_attach_arch_information.py
"""
...
def test_public_import_path_exposes_attach_arch_information_pass_only() -> None:
    ...
    attach_module = importlib.import_module("kernel_gen.passes.arch.attach_arch_information")
...
def test_attach_arch_information_attaches_single_entry_without_patterns() -> None:
    ...
    使用示例:
    - pytest -q test/passes/arch/test_attach_arch_information.py -k single_entry_without_patterns
    """
