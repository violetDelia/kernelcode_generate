...
使用示例:
- pytest -q test/passes/arch/test_arch_parallelize.py

关联文件:
- 功能实现: kernel_gen/passes/arch_parallelize/__init__.py
- 功能实现: kernel_gen/passes/arch/arch_parallelize.py
- Spec 文档: spec/pass/arch/arch_parallelize.md
- 测试文件: test/passes/arch/test_arch_parallelize.py
"""
...
# 功能说明: 验证 arch-parallelize canonical module、旧 shim 和 package root 公开 API。
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_package_public_api_shape
def test_arch_parallelize_package_public_api_shape() -> None:
    repo_root = Path(__file__).resolve().parents[3]
    canonical_module = importlib.import_module("kernel_gen.passes.arch.arch_parallelize")
    family_module = importlib.import_module("kernel_gen.passes.arch")
    compat_package_module = importlib.import_module("kernel_gen.passes.arch_parallelize")
    compat_submodule = importlib.import_module("kernel_gen.passes.arch_parallelize.arch_parallelize")
    passes_module = importlib.import_module("kernel_gen.passes")
    source_path = repo_root / "kernel_gen/passes/arch/arch_parallelize.py"

    assert canonical_module.__file__ is not None
    assert Path(canonical_module.__file__).resolve() == source_path
    assert source_path.is_file()
    assert not (repo_root / "kernel_gen/passes/arch_parallelize.py").exists()
    assert canonical_module.__all__ == ["ArchParallelizePass"]
    assert canonical_module.ArchParallelizePass is ArchParallelizePass
    assert family_module.ArchParallelizePass is ArchParallelizePass
    assert compat_package_module.ArchParallelizePass is ArchParallelizePass
    assert compat_submodule.ArchParallelizePass is ArchParallelizePass
    assert ArchParallelizePass.__module__ == "kernel_gen.passes.arch.arch_parallelize"
    assert hasattr(canonical_module, "_ArchParallelizeFuncPattern")
    assert not hasattr(family_module, "_ArchParallelizeFuncPattern")
    assert not hasattr(compat_package_module, "_ArchParallelizeFuncPattern")
    assert not hasattr(compat_submodule, "_ArchParallelizeFuncPattern")
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_func_pattern_ast_gate
def test_arch_parallelize_func_pattern_ast_gate() -> None:
    source_path = Path(__file__).resolve().parents[3] / "kernel_gen/passes/arch/arch_parallelize.py"
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rewrites_single_top_level_loop
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_wraps_no_loop_body_with_block0_guard
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_processes_each_non_declaration_func
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_skips_existing_block_parallel_func
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_skips_entry_point_host_dispatcher
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_multiple_top_level_loops
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_loop_carried_symbol_for
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_invalid_options_and_targets
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_target_contract_gaps
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_from_options_rejects_unknown_option
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_return_values
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_multi_block_func_body
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_unsupported_loop_structure
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_allows_memory_pool_setup_before_single_loop
...
# 使用示例: pytest -q test/passes/arch/test_arch_parallelize.py -k test_arch_parallelize_rejects_memory_pool_setup_after_loop
