"""nn_lowering element binary truediv tests.


功能说明:
- 验证 nn.truediv lower 为 kernel.binary_elewise(kind="div")。
...
- 测试文件: test/passes/lowering/nn_lowering/test_element_binary_truediv.py
"""
...
# TC-PASS-NNL-S2-005
# 测试目的: 验证 nn.truediv lower 为 kernel.binary_elewise(kind="div")。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_element_binary_truediv.py -k test_lower_truediv_to_kernel_binary_elewise
...
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_element_binary_truediv.py
def test_lower_truediv_to_kernel_binary_elewise() -> None:
