...
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_exp.py -k test_nn_lowering_exp_static
...
def test_nn_lowering_exp_static() -> None:
    _assert_ircheck_ok(CASE_TEXT_STATIC, "test/passes/lowering/nn_lowering/test_exp.py:static")


# TC-PASS-NNL-010
# 测试目的: 验证 nn.exp lowering 目标为 kernel.exp（符号维度）。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_exp.py -k test_nn_lowering_exp_dynamic
# 对应功能实现文件路径: kernel_gen/passes/lowering/nn_lowering/nn_lowering.py
# 对应 spec 文件路径: spec/pass/lowering/nn_lowering/spec.md
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_exp.py
def test_nn_lowering_exp_dynamic() -> None:
    _assert_ircheck_ok(CASE_TEXT_DYNAMIC, "test/passes/lowering/nn_lowering/test_exp.py:dynamic")


# TC-PASS-NNL-013
# 测试目的: 验证 nn.exp 输出形态不一致时必须抛 KernelCodeError。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_exp.py -k test_nn_lowering_exp_shape_mismatch
# 对应功能实现文件路径: kernel_gen/passes/lowering/nn_lowering/nn_lowering.py
# 对应 spec 文件路径: spec/pass/lowering/nn_lowering/spec.md
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_exp.py
