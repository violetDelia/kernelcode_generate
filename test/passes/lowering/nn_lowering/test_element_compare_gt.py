"""nn_lowering element compare gt tests.


功能说明:
- 验证 nn.gt lower 为 kernel.binary_elewise(kind="gt")。
...
- 测试文件: test/passes/lowering/nn_lowering/test_element_compare_gt.py
"""
...
from kernel_gen.dialect.nn import NnGtOp, NnMemorySpaceAttr, NnMemoryType
...
def _make_memory_type(element_type: Attribute = i32) -> NnMemoryType:
    ...
    - test: test/passes/lowering/nn_lowering/test_element_compare_gt.py
    - 功能实现: kernel_gen/passes/lowering/nn_lowering/element_binary_lowering.py
    """
...
# TC-PASS-NNL-S2-010
# 测试目的: 验证 nn.gt lower 为 kernel.binary_elewise(kind="gt")。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_element_compare_gt.py -k test_lower_gt_to_kernel_binary_elewise
...
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_element_compare_gt.py
def test_lower_gt_to_kernel_binary_elewise() -> None:
    ...
    assert kernel_ops and kernel_ops[0].kind.data == "gt"
