"""nn_lowering element binary mul tests.


功能说明:
- 验证 nn.mul lower 为 kernel.binary_elewise(kind="mul")。
...
- 测试文件: test/passes/lowering/nn_lowering/test_element_binary_mul.py
"""
...
from kernel_gen.dialect.nn import NnMemorySpaceAttr, NnMemoryType, NnMulOp
...
def _make_memory_type(element_type: Attribute = f32) -> NnMemoryType:
    ...
    - test: test/passes/lowering/nn_lowering/test_element_binary_mul.py
    - 功能实现: kernel_gen/passes/lowering/nn_lowering/element_binary_lowering.py
    """
...
# TC-PASS-NNL-S2-003
# 测试目的: 验证 nn.mul lower 为 kernel.binary_elewise(kind="mul")。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_element_binary_mul.py -k test_lower_mul_to_kernel_binary_elewise
...
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_element_binary_mul.py
def test_lower_mul_to_kernel_binary_elewise() -> None:
    ...
    module, block = _build_module(
        [lhs_type, rhs_type],
        result_type,
        lambda block: [NnMulOp(block.args[0], block.args[1], result_type, space)],
    )
    NnLoweringPass().apply(Context(), module)

    ops = list(block.ops)
    kernel_ops = [op for op in ops if isinstance(op, KernelBinaryElewiseOp)]
    assert kernel_ops and kernel_ops[0].kind.data == "mul"
