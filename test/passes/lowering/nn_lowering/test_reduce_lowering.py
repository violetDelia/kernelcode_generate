...
- 测试文件: [test/passes/lowering/nn_lowering/test_reduce_lowering.py](test/passes/lowering/nn_lowering/test_reduce_lowering.py)
"""
...
def _assert_ircheck_ok(case_text: str, source_path: str, residual_op_name: str) -> None:
    ...
    使用示例:
    - _assert_ircheck_ok(CASE_TEXT_SUM_STATIC, "test/passes/lowering/nn_lowering/test_reduce_lowering.py:sum_static", "nn.reduce_sum")
...
REDUCE_CASES = [
    pytest.param(
        ...
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:sum_static",
        "nn.reduce_sum",
        id="sum-static",
    ),
    pytest.param(
        CASE_TEXT_SUM_DYNAMIC,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:sum_dynamic",
        "nn.reduce_sum",
        id="sum-dynamic",
    ),
    pytest.param(
        CASE_TEXT_MIN_STATIC,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:min_static",
        "nn.reduce_min",
        id="min-static",
    ),
    pytest.param(
        CASE_TEXT_MIN_DYNAMIC,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:min_dynamic",
        "nn.reduce_min",
        id="min-dynamic",
    ),
    pytest.param(
        CASE_TEXT_MIN_SYMBOL_DIM,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:min_symbol_dim",
        "nn.reduce_min",
        id="min-symbol-dim",
    ),
    pytest.param(
        CASE_TEXT_MAX_STATIC,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:max_static",
        "nn.reduce_max",
        id="max-static",
    ),
    pytest.param(
        CASE_TEXT_MAX_DYNAMIC,
        "test/passes/lowering/nn_lowering/test_reduce_lowering.py:max_dynamic",
        "nn.reduce_max",
        id="max-dynamic",
    ),
]
...
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_reduce_lowering.py -k test_nn_lowering_reduce_ircheck
# 对应功能实现文件路径: kernel_gen/passes/lowering/nn_lowering/nn_lowering.py
# 对应 spec 文件路径: spec/pass/lowering/nn_lowering/spec.md
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_reduce_lowering.py
...
def test_nn_lowering_reduce_ircheck(case_text: str, source_path: str, residual_op_name: str) -> None:
    ...
    使用示例:
    - pytest -q test/passes/lowering/nn_lowering/test_reduce_lowering.py -k test_nn_lowering_reduce_ircheck
...
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_reduce_lowering.py -k test_nn_lowering_reduce_shape_mismatch
# 对应功能实现文件路径: kernel_gen/passes/lowering/nn_lowering/nn_lowering.py
# 对应 spec 文件路径: spec/pass/lowering/nn_lowering/spec.md
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_reduce_lowering.py
@pytest.mark.nn_lowering
@pytest.mark.parametrize("op_cls, kind", REDUCE_MISMATCH_CASES)
def test_nn_lowering_reduce_shape_mismatch(op_cls: ReduceOpType, kind: str) -> None:
    ...
    使用示例:
    - pytest -q test/passes/lowering/nn_lowering/test_reduce_lowering.py -k test_nn_lowering_reduce_shape_mismatch
