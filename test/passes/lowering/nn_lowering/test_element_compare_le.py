"""nn_lowering element compare le tests.


功能说明:
- 验证 nn.le lower 为 kernel.binary_elewise(kind="le")。
...
- 测试文件: test/passes/lowering/nn_lowering/test_element_compare_le.py
"""
...
from kernel_gen.dialect.nn import NnLeOp, NnMemorySpaceAttr, NnMemoryType
...
def _make_memory_type(element_type: Attribute = i32) -> NnMemoryType:
    ...
    - test: test/passes/lowering/nn_lowering/test_element_compare_le.py
    - 功能实现: kernel_gen/passes/lowering/nn_lowering/element_binary_lowering.py
    """
...
# TC-PASS-NNL-S2-009
# 测试目的: 验证 nn.le lower 为 kernel.binary_elewise(kind="le")。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_element_compare_le.py -k test_lower_le_to_kernel_binary_elewise
...
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_element_compare_le.py
def test_lower_le_to_kernel_binary_elewise() -> None:
    ...
    assert kernel_ops and kernel_ops[0].kind.data == "le"
