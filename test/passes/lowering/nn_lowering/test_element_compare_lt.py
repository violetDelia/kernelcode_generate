"""nn_lowering element compare lt tests.


功能说明:
- 验证 nn.lt lower 为 kernel.binary_elewise(kind="lt")。
...
- 测试文件: test/passes/lowering/nn_lowering/test_element_compare_lt.py
"""
...
from kernel_gen.dialect.nn import NnLtOp, NnMemorySpaceAttr, NnMemoryType
...
def _make_memory_type(element_type: Attribute = i32) -> NnMemoryType:
    ...
    - test: test/passes/lowering/nn_lowering/test_element_compare_lt.py
    - 功能实现: kernel_gen/passes/lowering/nn_lowering/element_binary_lowering.py
    """
...
# TC-PASS-NNL-S2-008
# 测试目的: 验证 nn.lt lower 为 kernel.binary_elewise(kind="lt")。
# 使用示例: pytest -q test/passes/lowering/nn_lowering/test_element_compare_lt.py -k test_lower_lt_to_kernel_binary_elewise
...
# 对应测试文件路径: test/passes/lowering/nn_lowering/test_element_compare_lt.py
def test_lower_lt_to_kernel_binary_elewise() -> None:
    ...
    module, block = _build_module(
        [lhs_type, rhs_type],
        result_type,
        lambda block: [NnLtOp(block.args[0], block.args[1], result_type, space)],
    )
    NnLoweringPass().apply(Context(), module)

    ops = list(block.ops)
    kernel_ops = [op for op in ops if isinstance(op, KernelBinaryElewiseOp)]
    assert kernel_ops and kernel_ops[0].kind.data == "lt"
