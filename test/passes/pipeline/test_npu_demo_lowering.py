...
MultiBufferAnalysisPass = multi_buffer_module.MultiBufferAnalysisPass
MultiBufferApplyPass = multi_buffer_module.MultiBufferApplyPass
MultiBufferPass = multi_buffer_module.MultiBufferPass
...
LoopSoftPipelinePass = importlib.import_module("kernel_gen.passes.schedule.loop_soft_pipeline").LoopSoftPipelinePass
...
def _record_loop_soft_pipeline(self, ctx: Context, target: ModuleOp) -> None:
    """记录 loop-soft-pipeline pass 执行。

    功能说明:
    - 为 pipeline 顺序测试记录 `LoopSoftPipelinePass.apply(...)` 被调用。
    - 锁定 pass 位于 `multi-buffer-apply` 与 `producer-consumer-analysis` 之间。

    使用示例:
    - monkeypatch.setattr(LoopSoftPipelinePass, "apply", _record_loop_soft_pipeline)
    """

    assert isinstance(self, LoopSoftPipelinePass)
    _ = self
    _ = ctx
    _ = target
    _PIPELINE_PASS_ORDER.append("loop-soft-pipeline")


def _record_multi_buffer_analysis(self, ctx: Context, target: ModuleOp) -> None:
    """记录 multi-buffer analysis pass 执行。

    功能说明:
    - 为 pipeline 顺序测试记录 `MultiBufferAnalysisPass.apply(...)` 被调用。
    - 锁定 pipeline 传入的 `memory_stage=2` 与 `target=npu_demo`。

    使用示例:
    - monkeypatch.setattr(MultiBufferAnalysisPass, "apply", _record_multi_buffer_analysis)
    """

    _ = ctx
    _ = target
    _PIPELINE_PASS_ORDER.append(f"multi-buffer-analysis:{self.memory_stage}:{self.target}")
    assert self.memory_stage == 2
    assert self.target == "npu_demo"


def _record_multi_buffer_apply(self, ctx: Context, target: ModuleOp) -> None:
    """记录 multi-buffer apply pass 执行。

    功能说明:
    - 为 pipeline 顺序测试记录 `MultiBufferApplyPass.apply(...)` 被调用。
    - 锁定 pipeline 传入的 `target=npu_demo` 与 `alignment=1024`。

    使用示例:
    - monkeypatch.setattr(MultiBufferApplyPass, "apply", _record_multi_buffer_apply)
    """

    _ = ctx
    _ = target
    _PIPELINE_PASS_ORDER.append(f"multi-buffer-apply:{self.target}:{self.alignment}")
    assert self.target == "npu_demo"
    assert self.alignment == 1024
...
# TC-PIPELINE-101
# 功能说明: 验证 npu-demo-lowering 的固定顺序包含三次 memory-plan、两次顶层 CSE、两次顶层 canonicalize、pre-pool multi-buffer analysis/apply / loop-soft-pipeline / producer-consumer-analysis 和 late attach。
...
def test_npu_demo_lowering_pipeline_pass_order(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert _PIPELINE_PASS_ORDER == [
        ...
        "memory-plan:True:True:False:True",
        "symbol-hoist-pipeline",
        "multi-buffer-analysis:2:npu_demo",
        "multi-buffer-apply:npu_demo:1024",
        "loop-soft-pipeline",
...
def test_npu_demo_lowering_pipeline_dynamic_acc_kernel_decompose_dump_shows_lifecycle_and_pool(
    ...
    loop_soft_pipeline_text = _dump_stage_text_by_marker(tmp_path, "loop-soft-pipeline")
    ...
    post_pool_cse_text = _dump_stage_text_by_marker(tmp_path, "cse", occurrence=2)
    post_pool_canonicalize_text = _dump_stage_text_by_marker(tmp_path, "canonicalize", occurrence=2)
    ...
    assert first_symbol_hoist_text.splitlines()[0] == "symbol-hoist-pipeline{fold=true cse=true canonicalize=true}"
    ...
    assert second_symbol_hoist_text.splitlines()[0] == "symbol-hoist-pipeline{fold=true cse=true canonicalize=true}"
    ...
    assert third_symbol_hoist_text.splitlines()[0] == "symbol-hoist-pipeline{fold=true cse=true canonicalize=true}"
    assert loop_soft_pipeline_text.splitlines()[0] == "loop-soft-pipeline{fold=true}"
    ...
    assert markers.count("cse") == 2
    assert markers.count("canonicalize") == 2
    ...
    assert _dump_stage_index(tmp_path, "tile-analysis") == 9
    assert _dump_stage_index(tmp_path, "kernel-pattern-attach") == 10
    assert _dump_stage_index(tmp_path, "transform-apply") == 11
    assert _dump_stage_index(tmp_path, "memory-plan", occurrence=2) == 12
    assert _dump_stage_index(tmp_path, "symbol-hoist-pipeline", occurrence=2) == 13
    assert _dump_stage_index(tmp_path, "kernel-aggregate") == 14
    assert _dump_stage_index(tmp_path, "kernel-decompose") == 15
    assert _dump_stage_index(tmp_path, "memory-plan", occurrence=3) == 16
    assert _dump_stage_index(tmp_path, "symbol-hoist-pipeline", occurrence=3) == 17
    assert _dump_stage_index(tmp_path, "multi-buffer-analysis") == 18
    assert _dump_stage_index(tmp_path, "multi-buffer-apply") == 19
    assert _dump_stage_index(tmp_path, "loop-soft-pipeline") == 20
    assert _dump_stage_index(tmp_path, "producer-consumer-analysis") == 21
    assert _dump_stage_index(tmp_path, "memory-pool") == 22
    assert _dump_stage_index(tmp_path, "cse", occurrence=2) == 23
    assert _dump_stage_index(tmp_path, "canonicalize", occurrence=2) == 24
    assert _dump_stage_index(tmp_path, "arch-parallelize") == 25
    ...
    assert markers.count("multi-buffer-analysis") == 1
    assert markers.count("multi-buffer-apply") == 1
    assert markers.count("loop-soft-pipeline") == 1
    assert markers[6:11] == [
        "memory-plan",
        "symbol-hoist-pipeline",
        "tile-analysis",
...
def test_npu_demo_lowering_pipeline_static_dump_runs_multi_buffer_before_pool(tmp_path: Path) -> None:
    """验证静态 tile matmul dump 运行 multi-buffer analysis/apply 后再进入 memory-pool。
    ...
    - 断言当前 npu-demo-lowering 在第三段 cleanup 后接入 `multi-buffer-analysis -> multi-buffer-apply -> loop-soft-pipeline`，并在 memory-pool 前生成 ring IR。
    ...
    kernel_aggregate_text = _dump_stage_text_by_marker(tmp_path, "kernel-aggregate")
    decompose_text = _dump_stage_text_by_marker(tmp_path, "kernel-decompose")
    multi_buffer_analysis_text = _dump_stage_text_by_marker(tmp_path, "multi-buffer-analysis")
    multi_buffer_apply_text = _dump_stage_text_by_marker(tmp_path, "multi-buffer-apply")
    loop_soft_pipeline_text = _dump_stage_text_by_marker(tmp_path, "loop-soft-pipeline")
    producer_consumer_text = _dump_stage_text_by_marker(tmp_path, "producer-consumer-analysis")
    memory_pool_text = _dump_stage_text_by_marker(tmp_path, "memory-pool")
    arch_parallelize_text = _dump_stage_text_by_marker(tmp_path, "arch-parallelize")
    ...
    assert "dma.advance_ring" not in multi_buffer_analysis_text
    assert multi_buffer_apply_text.splitlines()[0] == 'multi-buffer-apply{fold=true target="npu_demo" alignment=1024}'
    assert "multi_buffer." not in multi_buffer_apply_text
    assert "dma.make_ring" in multi_buffer_apply_text
    assert "dma.current_ring" in multi_buffer_apply_text
    assert "dma.advance_ring" in multi_buffer_apply_text
    assert "dma.alloc" in multi_buffer_apply_text
    assert loop_soft_pipeline_text.splitlines()[0] == "loop-soft-pipeline{fold=true}"
    assert producer_consumer_text.splitlines()[0] == "producer-consumer-analysis{fold=true}"
    assert "dma.alloc" in producer_consumer_text
    ...
    assert _dump_stage_index(tmp_path, "symbol-hoist-pipeline", occurrence=3) + 1 == _dump_stage_index(
        tmp_path, "multi-buffer-analysis"
    )
    assert _dump_stage_index(tmp_path, "multi-buffer-analysis") + 1 == _dump_stage_index(
        tmp_path, "multi-buffer-apply"
    )
    assert _dump_stage_index(tmp_path, "multi-buffer-apply") + 1 == _dump_stage_index(
        tmp_path, "loop-soft-pipeline"
    )
    assert _dump_stage_index(tmp_path, "loop-soft-pipeline") + 1 == _dump_stage_index(
...
# 测试目的: 通过公开 demo kernel 与 dump marker 锁定 `memory-plan(auto_pad=true) -> symbol-hoist-pipeline` 内嵌 cleanup 后的 lifecycle 外提与 dead-fill 合同。
...
def test_npu_demo_lowering_pipeline_matmul_demo_allocs_hoist_for_static_and_dynamic_tiles(tmp_path: Path) -> None:
    ...
    - 断言第三段 `symbol-hoist-pipeline` 默认 cleanup 后 static/static 与 static/dynamic 不残留 dead fill，
...
def test_npu_demo_lowering_pipeline_symbol_hoist_pipeline_pattern_dump(
    ...
    assert first_hoist_text.splitlines()[0] == "symbol-hoist-pipeline{fold=true cse=true canonicalize=true}"
    assert final_hoist_text.splitlines()[0] == "symbol-hoist-pipeline{fold=true cse=true canonicalize=true}"
    assert markers.count("symbol-hoist-pipeline") == 3
    assert "symbol-loop-hoist" not in markers
    assert "hoist-dma-alias-ops" not in markers
