"""cuda-sm89-lowering pipeline tests.


功能说明:
- 覆盖 `build_cuda_sm89_lowering_pipeline(...)` 的公开 builder、registry 名称和 pass 顺序。
- 验证 CUDA pipeline 不接入 `MemoryPoolPass(rewrite=True)`，避免 TLM fragment 被改写为 dynamic byte pool。

使用示例:
- pytest -q test/passes/pipeline/test_cuda_sm89_lowering.py

关联文件:
- 功能实现: kernel_gen/pipeline/cuda_sm89_lowering.py
...
def test_cuda_sm89_lowering_pipeline_builds_pass_manager() -> None:
    """验证 cuda-sm89-lowering 公开 builder 和 registry 名称。
    ...
    使用示例:
    - pytest -q test/passes/pipeline/test_cuda_sm89_lowering.py -k builds_pass_manager
    """

    direct_pm = build_cuda_sm89_lowering_pipeline()
    load_builtin_passes()
    registry_pm = build_registered_pipeline("cuda-sm89-lowering")

    assert direct_pm.name == "cuda-sm89-lowering"
    assert registry_pm.name == "cuda-sm89-lowering"
    assert "cuda-sm89-lowering" in list_registered_pipelines()


def test_cuda_sm89_lowering_pipeline_rejects_non_cuda_target() -> None:
    """验证 CUDA pipeline 只接受 cuda_sm89 target。
    ...
    使用示例:
    - pytest -q test/passes/pipeline/test_cuda_sm89_lowering.py -k rejects_non_cuda_target
    """

    with pytest.raises(KernelCodeError, match="target must be cuda_sm89"):
        build_cuda_sm89_lowering_pipeline({"target": "npu_demo"})
    with pytest.raises(KernelCodeError, match="only accepts target option"):
        build_cuda_sm89_lowering_pipeline({"only-kernel": "true"})


def test_cuda_sm89_lowering_pipeline_order_has_no_memory_pool(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    使用示例:
    - pytest -q test/passes/pipeline/test_cuda_sm89_lowering.py -k no_memory_pool
    """
...
def test_cuda_sm89_lowering_pipeline_dump_pass_order_markers(
    ...
    使用示例:
    - pytest -q test/passes/pipeline/test_cuda_sm89_lowering.py -k dump_pass_order
    """
    ...
    assert raw_markers[26] == 'attach-arch-information{target="cuda_sm89" fold=true}'
