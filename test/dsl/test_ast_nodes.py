...
功能说明:
- 覆盖 `kernel_gen.dsl.ast.nodes` 的节点构造与 `kernel_gen.dsl.ast` facade re-export。
...
覆盖率命令:
- `pytest -q --cov=kernel_gen.dsl.ast.nodes --cov-branch --cov-report=term-missing test/dsl/test_ast_nodes.py`
...
# 对应功能实现文件路径: kernel_gen/dsl/ast/nodes.py
