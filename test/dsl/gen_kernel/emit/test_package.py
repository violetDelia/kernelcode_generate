...
def test_cuda_sm89_emit_spec_uses_generated_kernel_contract() -> None:
    """验证通用 emit spec 中的 CUDA SM89 合同不保留旧 source fragment 口径。

    功能说明:
    - 读取 `spec/dsl/gen_kernel/emit.md` 的 `target="cuda_sm89"` 模块级补充。
    - 锁定当前合同为 generated kernel、device body、wrapper call 与 operand binding。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_package.py -k cuda_sm89_emit_spec_uses_generated_kernel_contract
    """

    spec_text = (REPO_ROOT / "spec" / "dsl" / "gen_kernel" / "emit.md").read_text(encoding="utf-8")
    cuda_sm89_lines = [line for line in spec_text.splitlines() if 'target="cuda_sm89"' in line]

    assert len(cuda_sm89_lines) == 1
    cuda_sm89_contract = cuda_sm89_lines[0]
    assert "source marker" not in cuda_sm89_contract
    assert "source fragment" not in cuda_sm89_contract
    assert "executable trace wrapper" not in cuda_sm89_contract
    assert "hash marker" in cuda_sm89_contract
    assert "trace comment" in cuda_sm89_contract
    assert "generated kernel" in cuda_sm89_contract
    assert "device body" in cuda_sm89_contract
    assert "wrapper call" in cuda_sm89_contract
    assert "operand binding" in cuda_sm89_contract
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_public_entry_lowers_func_and_single_func_module_consistently
...
def test_emit_c_context_type_helpers_use_target_registry() -> None:
    ...
    assert ctx.dispatch_type(f64) == "double"
    assert ctx.dispatch_type(IndexType()) == "long long"
    assert ctx.dispatch_type(SymbolValueType.from_expr("N")) == "long long"
    with pytest.raises(ValueError, match="unsupported memory space"):
        ctx.dispatch_attr("bad_space")
...
def test_emit_c_context_public_state_and_dispatch_edges() -> None:
    """EmitCContext 公开状态 API 与统一 dispatch 入口保持一致。"""

    ctx = _ctx()
    cache_key = ("shape", 0, None)
    assert ctx.allocate_name("tmp") == "tmp0"
    assert ctx.allocate_name("tmp") == "tmp1"
    assert ctx.lookup_cached_name("layout", cache_key) is None
    assert ctx.bind_cached_name("layout", cache_key, "layout0") == "layout0"
    assert ctx.lookup_cached_name("layout", cache_key) == "layout0"
    with pytest.raises(KernelCodeError, match="cached name must be non-empty str"):
        ctx.bind_cached_name("layout", "empty", "")
    ...
    assert ctx.is_target("cpu")
    assert not ctx.is_target("npu_demo")
    assert ctx.target_entry({"cpu": "selected"}, "fallback") == "selected"
    assert ctx.target_entry({"npu_demo": "selected"}, "fallback") == "fallback"
    assert "target=cpu: emit_c" in str(ctx.emit_error("emit_c", ""))

    mem_type = _make_memory_type([2], [1], element_type=i32)
    block = Block(arg_types=[mem_type])
    assert ctx.dispatch(block.args[0]) == "arg0"
    assert ctx.dispatch("global") == "GM"
    assert ctx.dispatch_attr("global") == "GM"
    assert ctx.dispatch_type(i32) == "int32_t"
    alloc_stmt = ctx.dispatch(DmaAllocOp([], mem_type))
    assert alloc_stmt is not None
    assert "Memory<GM, int32_t> v0" in alloc_stmt

    view_ctx = _npu_ctx()
    view_mem_type = _make_memory_type([2], [1], space="tsm", element_type=i32)
    view_block = Block(arg_types=[view_mem_type, view_mem_type, view_mem_type])
    view_ctx.bind_name(view_block.args[0], "0bad")
    view_ctx.bind_name(view_block.args[1], "view")
    fallback_view = DmaViewOp(view_block.args[0], [], [], [], view_mem_type)
    assert view_ctx.create_or_get_name(fallback_view.result) == "view_1"

    suffix_ctx = _npu_ctx()
    suffix_ctx.bind_name(view_block.args[0], "src")
    suffix_ctx.bind_name(view_block.args[1], "src_1")
    suffix_ctx.bind_name(view_block.args[2], "src_2")
    suffix_view = DmaViewOp(view_block.args[0], [], [], [], view_mem_type)
    assert suffix_ctx.create_or_get_name(suffix_view.result) == "src_3"

    with pytest.raises(KernelCodeError, match="unsupported type"):
        ctx.dispatch_type(ArrayAttr([IntAttr(1)]))
...
def _lower_single_op_func(
    ...
    build_ops: Callable[[Block], list[Operation]],
) -> Block:
    ...
    - 功能实现: test/dsl/gen_kernel/emit/test_package.py
    """
...
def _lower_built_func(fn: Callable[..., DslFunctionReturn], *runtime_args: DslRuntimeArg) -> Block:
    """对 `mlir_gen(...)` 生成的 `func.func` 执行 pass 并返回 entry block。


    功能说明:
    - 用于 I4 的公开链路 smoke：`mlir_gen -> NnLoweringPass -> emit_c`。
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_arith_add
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_value_lowers_compare
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_value_unbound_block_argument_uses_index
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_scf_for
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_memory_access
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_rejects_unsupported_op
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_value_rejects_invalid_dependency
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_symbol_add
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_rejects_symbol_add_on_non_cpu
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
def test_emit_c_op_lowers_npu_demo_symbol_const_cast_and_to_float() -> None:
    ...
    assert emit_c_op(const, ctx) == "S_INT c_0 = 7;"
    assert emit_c_op(cast, ctx) == "int32_t c_0_cast_int32_t = c_0;"
    assert emit_c_op(to_float, ctx) == "float v0 = c_0;"


# EC-008A-CONST-EDGES
# 功能说明: 验证 npu_demo 下 `symbol.const` 的公开 value 复用与 dma.load 内联消费边界。
# 测试目的: 锁定 `emit_c_value(...)` 已绑定名称优先，以及只被 `dma.load` 消费的 const 不额外生成 `S_INT` 声明。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_symbol_const_value_and_dma_load_edges
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/symbol/const.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit/npu_demo/symbol/__init__.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_lowers_npu_demo_symbol_const_value_and_dma_load_edges() -> None:
    ctx = _npu_ctx()
    value_const = SymbolConstOp(13)
    assert emit_c_value(value_const.result, ctx) == "13"
    ctx.bind_name(value_const.result, "bound_symbol")
    assert emit_c_value(value_const.result, ctx) == "bound_symbol"

    source_type = _make_memory_type([4], [1], space="global")
    tile_type = _make_memory_type([1], [1], space="tsm")
    block = Block(arg_types=[tile_type, source_type])
    offset = SymbolConstOp(0)
    size = SymbolConstOp(1)
    stride = SymbolConstOp(1)
    load = DmaLoadOp(block.args[0], block.args[1], [offset.result], [size.result], [stride.result])

    assert load.name == "dma.load"
    assert emit_c_op(offset, ctx) == ""
    assert emit_c_op(size, ctx) == ""
    assert emit_c_op(stride, ctx) == ""
...
# EC-008D
# 测试目的: 验证 npu_demo 下 symbol.for 使用 `S_INT` 迭代变量，并引用前置 symbol.const 名称。
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# EC-008E
# 功能说明: 验证 `mlir_gen -> pass -> emit_c` 的 add 三条 CPU 路径可生成 lowered IR 节点片段。
# 测试目的: 清理 raw `NnAddOp` 节点级直出源码的旧成功口径，锁定公开成功链路来自 pass 后 `kernel.binary_elewise` / `dma.fill`。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_mlir_gen_nn_add_variants_after_pass
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_op_lowers_mlir_gen_nn_add_variants_after_pass() -> None:
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_keeps_nn_add_unsupported_without_prebound_result_or_on_non_cpu
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_memory_space_template_alloc
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
def test_emit_c_op_lowers_kernel_family_and_rejects_unsupported_reduce_kind() -> None:
    ...
    def _bind(*pairs: tuple[SSAValue, str]) -> EmitCContext:
        ctx = _npu_ctx()
...
def test_emit_c_lowers_npu_demo_dma_reshape_rank6_via_brace_list() -> None:
    ...
    - 锁定源码不使用 `Vector(...)` 或局部 `long long reshape_shape_*`。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_dma_reshape_rank6_via_brace_list
    """
...
# EC-S7-001
# 功能说明: 覆盖 emit_c 上下文命名冲突、symbol.const 独立发射与公开发射接口的边界。
# 测试目的: 锁定 `EmitCContext` 与 `emit_c_op(...)` 的公开行为，不再依赖包根 private helper。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_private_helper_and_context_edges
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_private_helper_and_context_edges(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    assert emit_c_op(positive_const, npu_ctx) == "S_INT c_0 = 7;"
    assert emit_c_op(duplicate_const, npu_ctx) == "S_INT c_1 = 7;"
    assert npu_ctx.lookup_name(duplicate_const.result) == "c_1"
    assert emit_c_op(negative_const, npu_ctx) == "S_INT c_2 = -3;"
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_private_layout_and_operand_helpers
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_private_additional_error_matrix
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_private_additional_error_matrix(
    ...
    broadcast_op = DmaBroadcastOp(load_block.args[0], load_block.args[1])
    for op in (slice_op, deslice_op, img2col_op):
        with pytest.raises(KernelCodeError, match="cpu-only"):
            emit_c_op(op, _target_ctx("gpu"))
    with pytest.raises(KernelCodeError, match="unsupported op"):
        emit_c_op(broadcast_op, _target_ctx("gpu"))
    ...
    dynamic_alloc_ctx = _ctx()
    dynamic_dim = SymbolConstOp(4)
    emit_c_op(dynamic_dim, dynamic_alloc_ctx)
    ...
    with pytest.raises(KernelCodeError, match="only static memory layout is supported"):
        emit_c_op(DmaAllocOp([], dynamic_shape_type), _ctx())
    ...
    cpu_kernel_ctx = _ctx()
    for index, arg in enumerate(kernel_block.args[:3]):
        cpu_kernel_ctx.bind_name(arg, f"cpu_m{index}")
    for kind, helper in (
        ("add", "add"),
        ("sub", "sub"),
        ("mul", "mul"),
        ...
        assert stmt == f"cpu::{helper}(cpu_m1, cpu_m2, cpu_m0);"
    cpu_bad_kind = KernelBinaryElewiseOp(
        kernel_block.args[0],
        kernel_block.args[1],
        kernel_block.args[2],
        kind="add",
    ...
    cpu_bad_kind.attributes["kind"] = StringAttr("pow")
    with pytest.raises(KernelCodeError, match="unsupported kind=pow"):
        emit_c_op(cpu_bad_kind, cpu_kernel_ctx)
    cpu_add_ctx = _ctx()
    cpu_add_block = Block(arg_types=[mem_type, mem_type])
    cpu_add_ctx.bind_name(cpu_add_block.args[0], "lhs")
    cpu_add_ctx.bind_name(cpu_add_block.args[1], "rhs")
    cpu_add_op = NnAddOp(
        cpu_add_block.args[0],
        cpu_add_block.args[1],
        mem_type,
        NnMemorySpaceAttr.from_name("global"),
    )
    cpu_add_ctx.bind_name(cpu_add_op.result, "out")
    assert emit_c_op(cpu_add_op, cpu_add_ctx) == "cpu::add(lhs, rhs, out);"
    with pytest.raises(KernelCodeError, match="unsupported op"):
...
def test_emit_c_lowers_npu_demo_dma_indexed_and_fill_helpers() -> None:
    ...
    fill_const_name = ctx.lookup_name(fill_value.result)
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_dma_fill_infinity_literal
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_passed_mixed_add_pipeline_with_dma_fill
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_op_lowers_passed_mixed_add_pipeline_with_dma_fill() -> None:
    ...
    def _build_const_ops(block: Block) -> list[Operation]:
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_dma_alloc_and_view
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_op_lowers_dma_alloc_and_view() -> None:
    ...
    assert "{2, 3} /*shape*/, {3, 1} /*stride*/" in npu_reinterpret_stmt
    assert "pool.format()" in npu_reinterpret_stmt
    assert "long long v0_shape" not in npu_reinterpret_stmt
    assert "long long v0_stride" not in npu_reinterpret_stmt
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_lowers_img2col2d_dma_loop_pipeline
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_op_lowers_img2col2d_dma_loop_pipeline() -> None:
    ...
    def img2col_loop(x: Memory, y: Memory, start: SymbolDim, end: SymbolDim, step: SymbolDim) -> None:
        for i in loop(start, end, step):
            tile = slice(x, [i, 0, 0, 0], [1, 1, 4, 4], [1, 1, 1, 1], MemorySpace.LM)
            col = img2col2d(tile, kh=2, kw=2)
            deslice(y, col, [i, 0, 0, 0, 0, 0], [1, 1, 2, 2, 3, 3], [1, 1, 1, 1, 1, 1])
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_op_assigns_unique_helper_names_for_repeated_dma_slice_and_deslice
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_package_registers_tuner_cost_op
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_add
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_binary_elewise
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_lowers_npu_demo_tuner_cost_kernel_binary_elewise() -> None:
    ...
    for index, (kernel_kind, helper) in enumerate(cases):
        ctx = _npu_ctx()
        ctx.bind_name(block.args[0], "out")
        ctx.bind_name(block.args[1], "lhs")
        ctx.bind_name(block.args[2], "rhs")
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_exp_select_reduce
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_matmul
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_img2col2d
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# EC-019B
# 功能说明: 验证 npu_demo 下 `tuner.cost(op_name="kernel.img2col1d")` 发射为 `cost::img2col1d`。
# 测试目的: 锁定一维 im2col cost helper 的公开参数顺序与 cost kind 原样透传。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_kernel_img2col1d
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_lowers_npu_demo_tuner_cost_kernel_img2col1d() -> None:
    out_type = _make_memory_type([1, 3, 3, 6], [54, 18, 6, 1], space="tsm", element_type=f32)
    input_type = _make_memory_type([1, 3, 8], [24, 8, 1], space="tsm", element_type=f32)
    block = Block(arg_types=[out_type, input_type])
    ctx = _npu_ctx()
    ctx.bind_name(block.args[0], "out")
    ctx.bind_name(block.args[1], "input")
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_dma_copy
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tuner_cost_dma_slice_and_deslice
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_symbol_add_with_tuner_cost_value
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_rejects_unknown_npu_demo_tuner_cost_op_name
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_preserves_raw_npu_demo_tuner_cost_kind_and_rejects_invalid_memory_type
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# EC-023A
# 功能说明: 验证 npu_demo 下 `tuner.cost` 公开错误语义覆盖 metadata、operand、kind、axis 与空间/类型不匹配矩阵。
# 测试目的: 锁定 cost emitter 对公开 `tuner.cost` metadata 与 operands 的拒绝边界，不依赖内部注册 helper。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_rejects_npu_demo_tuner_cost_public_error_matrix
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/npu_demo/tuner/cost.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_rejects_npu_demo_tuner_cost_public_error_matrix() -> None:
    memory_type = _make_memory_type([4, 4], [4, 1], space="global", element_type=f32)
    tsm_type = _make_memory_type([4, 4], [4, 1], space="tsm", element_type=f32)
    int_type = _make_memory_type([4, 4], [4, 1], space="global", element_type=i32)
    bool_type = _make_memory_type([4, 4], [4, 1], space="global", element_type=i1)
    reduce_out_type = _make_memory_type([4, 1], [1, 1], space="global", element_type=f32)
    big_type = _make_memory_type([2, 2, 2, 2, 2], [16, 8, 4, 2, 1], space="global", element_type=f32)
    block = Block(arg_types=[memory_type, memory_type, tsm_type, int_type, bool_type, reduce_out_type, big_type])
    scalar_block = Block(arg_types=[i32, i32])
    c0 = SymbolConstOp(0)
    c1 = SymbolConstOp(1)
    ctx = _npu_ctx()
    cases = [
        (
            TunerCostOp([block.args[0]], cost_kind=IntegerAttr(1, i32), op_name=StringAttr("kernel.add")),
        ...
        (
            ...
            r"tuner\.cost: op_name=kernel\.binary_elewise requires out, lhs, rhs",
        ),
        (
            _make_tuner_cost_op(
                "kernel.binary_elewise",
            ...
            r"tuner\.cost: kernel\.binary_elewise unsupported kind=pow",
        ),
        (
            _make_tuner_cost_op(
                "kernel.binary_elewise",
        ...
        (
            ...
            r"tuner\.cost: kernel\.binary_elewise operands must share memory space",
        ),
        (
            _make_tuner_cost_op(
                "kernel.binary_elewise",
        ...
        (
            ...
            r"tuner\.cost: op_name=kernel\.reduce requires out, input",
        ),
        (
            _make_tuner_cost_op(
                "kernel.reduce",
            ...
            r"tuner\.cost: kernel\.reduce unsupported kind=prod",
        ),
        (
            _make_tuner_cost_op(
                "kernel.reduce",
        ...
        (
            ...
            r"tuner\.cost: kernel\.reduce operands must share memory space",
        ),
        (
            _make_tuner_cost_op(
                "kernel.reduce_min",
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_maps_nn_space_to_template_param
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
def test_emit_c_maps_nn_space_to_template_param() -> None:
    ...
    gm_type = _make_memory_type([16], [1], space="global", element_type=f32)
    ...
    value_tsm = ArchGetDynamicMemoryOp(NnMemorySpaceAttr.from_name("tsm"), tsm_type)
    assert emit_c_value(value_tsm.result, _npu_ctx()) == "npu_demo::get_dynamic_memory<TSM>()"
    assert "load<" not in tsm_stmt
    assert "store<" not in tlm_stmt

    gm = ArchGetDynamicMemoryOp(NnMemorySpaceAttr.from_name("global"), gm_type)
    with pytest.raises(KernelCodeError, match="unsupported dynamic memory space"):
        emit_c_op(gm, _npu_ctx())
    with pytest.raises(KernelCodeError, match="unsupported dynamic memory space"):
        emit_c_value(gm.result, _npu_ctx())
...
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_slice_deslice_add_pipeline
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
...
# EC-027
# 功能说明: 验证 npu_demo 下 tiled matmul 可发射 symbol.for + 2D slice/deslice + kernel.matmul 管线，且多维 slice/deslice 采用 brace-list 绑定合同。
# 测试目的: 锁定 target=npu_demo 的二维 tile helper 形态与 `matmul<...>(...)` 调用，确保多维参数不回退到 Vector 文本并保持 deslice brace-list 调用链稳定。
# 使用示例: pytest -q test/dsl/gen_kernel/emit/test_package.py -k test_emit_c_lowers_npu_demo_tiled_matmul_pipeline
# 对应功能实现文件路径: kernel_gen/dsl/gen_kernel/emit/__init__.py
# 对应 spec 文件路径: spec/dsl/gen_kernel/emit.md
# 对应测试文件路径: test/dsl/gen_kernel/emit/test_package.py
