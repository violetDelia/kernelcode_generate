"""CUDA SM86 memory hierarchy source tests.

功能说明:
- 通过公开 final IR / emit 入口验证 CUDA SM86 memory space marker 与 C5 all-TLM1 materialization。
- 锁定 `kernel.matmul` staging 必须写回原 out descriptor，避免内存层级语义只停留在 comment。

使用示例:
- pytest -q test/dsl/gen_kernel/emit/test_cuda_sm86_memory_hierarchy.py
...
def _emit_cuda_sm86_source(
    ...
    try:
        ...
        if "// kg.cuda.ir.memory_spaces:" not in source:
            raise AssertionError("missing CUDA SM86 memory-space marker")
        return source
    finally:
...
def _make_minimal_c5_matmul_module(
    *,
    ...
    - 不依赖 CUDA SM86 package-local helper，测试只通过公开 `emit_c(...)` 观察行为。
...
def test_cuda_sm86_lowered_matmul_exposes_memory_spaces_and_c5_writeback() -> None:
    """验证 lowered matmul source 暴露 memory hierarchy 与 C5 write-back。

    功能说明:
    - 从公开 matmul demo 入口生成 CUDA SM86 source。
    - 锁定 global/tlm1/tsm marker、all-TLM1 materialization marker 和 generated memory wrapper calls。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm86_memory_hierarchy.py -k memory_spaces
    """
    ...
    assert "cuda_sm86::alloc<MemorySpace::TLM1" in source
...
def test_cuda_sm86_matmul_materialization_fails_fast_for_invalid_c5() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm86_memory_hierarchy.py -k invalid_c5
    """
