"""CUDA SM89 API-aligned generated source tests.

功能说明:
- 通过公开 DSL / pipeline / emit 入口验证 `cuda_sm89` SourceBundle 形态。
- 锁定 hash 专属 entry、slot C ABI、generated wrapper calls、dynamic symbol/control-flow 参数和三类 demo source 差异。

使用示例:
- pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py
...
def _emit_cuda_sm89_source(
    ...
    try:
        ...
        if 'extern "C" int kg_execute_entry' not in source:
            raise AssertionError("missing CUDA SM89 C ABI entry")
        return source
    finally:
...
def _extract_cuda_sm89_ir_hash(source: str) -> str:
    ...
    for line in source.splitlines():
        if line.startswith(marker_prefix):
            ...
            if not stable_hash:
                raise AssertionError("empty CUDA SM89 IR hash")
            return stable_hash
    raise AssertionError("missing CUDA SM89 IR hash")
...
def _write_fake_nvcc(path: Path, log_path: Path) -> Path:
    ...
    功能说明:
    - 记录 CUDA compile strategy 传入的参数。
    - 解析 `-o <path>` 并创建输出文件，模拟成功编译。
    - 只服务 API-aligned SourceBundle compile strategy 断言。
...
def test_cuda_sm89_api_aligned_source_uses_hash_entry_and_slot_abi() -> None:
    """验证 generated source 使用 hash entry 与 slot C ABI。

    功能说明:
    - 通过 matmul demo lowered final IR 生成 SourceBundle。
    - 锁定 `kg_execute_entry` 只转发到 hash 专属 entry，且 generated source 使用 CUDA SM89 slot ABI 与 per-op wrapper calls。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py -k hash_entry
    """
    ...
    assert "kg_cuda_sm89_mma_observable" not in source
    ...
    assert "kg_cuda_sm89_execute_" + "matmul_ir" not in source
    for forbidden in (
...
def test_cuda_sm89_demo_sources_are_final_ir_specific() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py -k final_ir_specific
    """
    ...
    assert "kg_cuda_sm89_execute_" + "matmul_ir" not in matmul_source
    assert "kg_cuda_sm89_execute_" + "img2col2d_ir" not in conv2d_source
    assert "kg_cuda_sm89_execute_" + "reduce_exp_ir" not in flash_source
    assert matmul_source != conv2d_source
    assert matmul_source != flash_source
    assert conv2d_source != flash_source


def test_cuda_sm89_dynamic_matmul_source_uses_symbol_operands_and_control_flow() -> None:
    """验证 dynamic matmul 的 symbol/control-flow 进入 generated body。

    功能说明:
    - 从公开 dynamic matmul demo 生成 CUDA SM89 source。
    - 锁定 runtime symbol slot、`symbol.for` 循环、descriptor/window 参数和 deslice write-back 都来自 SSA operand。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py -k dynamic_matmul_source
    """
    ...
    assert "cuda_sm89::detail::int_arg_or(slots, count, 4, 6)" in source
    assert "cuda_sm89::detail::int_arg_or(slots, count, 5, 7)" in source
    assert "cuda_sm89::detail::int_arg_or(slots, count, 6, 5)" in source
    assert "for (S_INT kg_arg_" in source
    assert "cuda_sm89::detail::fragment_alias<MemorySpace::GM, float>" in source
    assert "Vector{kg_arg_" in source
    assert "Vector{kg_v_" in source
    assert "cuda_sm89::deslice<MemorySpace::GM, MemorySpace::TSM" in source
...
def test_cuda_sm89_dynamic_conv2d_source_uses_symbol_operands_and_img2col() -> None:
    """验证 dynamic conv2d 的 symbol/control-flow 驱动 img2col 与搬运参数。

    功能说明:
    - 从公开 dynamic conv2d demo 生成 CUDA SM89 source。
    - 锁定 dynamic tile/runtime padding/stride/dilation operand 进入 `img2col2d`，view/slice/deslice 使用 SSA vector。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py -k dynamic_conv2d_source
    """
    ...
    assert "cuda_sm89::detail::int_arg_or(slots, count, 4, 1)" in source
    assert "cuda_sm89::detail::int_arg_or(slots, count, 16, 7)" in source
    assert "for (S_INT kg_arg_" in source
    assert "cuda_sm89::img2col2d<MemorySpace::TSM, MemorySpace::TSM, float, float>" in source
    assert "if (cuda_sm89::img2col2d<MemorySpace::TSM, MemorySpace::TSM, float, float>" in source
    assert "(void)cuda_sm89::img2col2d" not in source
    assert "cuda_sm89::slice<MemorySpace::TSM, MemorySpace::GM" in source
    assert "cuda_sm89::deslice<MemorySpace::GM, MemorySpace::TSM" in source
    assert "delete[] kg_v_" in source
    assert "kg_cuda_sm89_host_status" in source
    assert "Vector{kg_arg_" in source
    assert "Vector{kg_v_" in source
...
def test_cuda_sm89_dynamic_conv2d_symbol_source_uses_cuda_compile_strategy(tmp_path: Path) -> None:
    """验证 dynamic conv2d SymbolDim SourceBundle 可通过 CUDA compile strategy。

    功能说明:
    - 使用公开 dynamic conv2d demo 与 `SymbolDim` 编译实参生成 CUDA SM89 SourceBundle。
    - 通过公开 `ExecutionEngine(target="cuda_sm89").compile(...)` 与 fake nvcc 验证 SourceBundle 写盘和 SM89 编译命令。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_api_aligned_codegen.py -k dynamic_conv2d_symbol_source
    """
    ...
    assert "cuda_sm89::img2col2d<MemorySpace::TSM, MemorySpace::TSM, float, float>" in source
    assert "cuda_sm89::matmul<MemorySpace::TLM1, MemorySpace::TLM1, MemorySpace::TLM1, float, float, float>" in source
    assert source.count("cuda_sm89::detail::memory_from_slot<MemorySpace::GM, float>(slots, count, 0)") == 1
    log_path = tmp_path / "nvcc_args.txt"
    fake_nvcc = _write_fake_nvcc(tmp_path / "nvcc", log_path)
    compiled = ExecutionEngine(target="cuda_sm89", compiler=str(fake_nvcc)).compile(
        source=source,
        function=conv2d_inputs_dynamic_tile_dynamic_kernel.__name__,
    )
    try:
        ...
        assert "include/cuda_sm89/generated_entry.cuh" in source
    finally:
        compiled.close()
