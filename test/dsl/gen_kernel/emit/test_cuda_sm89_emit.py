"""CUDA SM89 emit backend tests.


功能说明:
- 通过公开 `gen_kernel(...)` / `emit_c(...)` 入口验证 CUDA SM89 backend 自动加载与 SourceBundle 输出。
- 覆盖 generated CUDA source 的 final IR comments、stable hash、entry ABI 和 npu_demo 残留扫描。

使用示例:
- pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py

关联文件:
- 功能实现: kernel_gen/dsl/gen_kernel/emit/cuda_sm89/__init__.py
- Spec 文档: spec/dsl/gen_kernel/emit/cuda_sm89.md
...
def _make_module(name: str = "cuda_matmul") -> ModuleOp:
    ...
    - 用于触发 CUDA SM89 target-specific ModuleOp handler。
...
def _make_lowered_demo_module(
    ...
    - 运行公开 `build_cuda_sm89_lowering_pipeline()`，让 emit 测试基于真实 final IR traversal 判定 source。
...
OLD_CUDA_SM89_SOURCE_TOKENS = (
    "kg_cuda_sm89_" + "selected_kernel_kind",
    "kg_cuda_sm89_" + "run_" + "matmul",
    "kg_cuda_sm89_" + "run_" + "conv2d",
    "kg_cuda_sm89_" + "run_" + "flash_attention",
    "emit_" + "matmul_source",
    "emit_" + "conv2d_source",
...
def _extract_cuda_sm89_ir_hash(source: str) -> str:
    ...
    使用示例:
    - stable_hash = _extract_cuda_sm89_ir_hash(source)
    """
...
def _extract_cuda_sm89_device_body(source: str) -> str:
    ...
    使用示例:
    - body = _extract_cuda_sm89_device_body(source)
    """

    lines = source.splitlines()
    start_index = -1
    for index, line in enumerate(lines):
        if line.startswith("__device__ void kg_cuda_sm89_device_body_"):
            start_index = index
            break
    if start_index < 0:
        raise AssertionError("missing generated CUDA SM89 device body")
    depth = 0
    in_body = False
...
def test_cuda_sm89_emit_module_returns_source_bundle() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k source_bundle
    """
    ...
    assert '// __KG_BUNDLE_FILE__:include/cuda_sm89/generated_entry.cuh' in source
    assert '#include "include/cuda_sm89/cuda_sm89.cuh"' in source
    assert "// cuda_sm89 generated from final IR" in source
    assert "// kg.cuda.ir.hash: " in source
    stable_hash = _extract_cuda_sm89_ir_hash(source)
    assert f"// kg.cuda.ir.entry_symbol: kg_cuda_sm89_execute_{stable_hash}_ir" in source
    assert f"// kg.cuda.ir.generated_kernel_symbol: kg_cuda_sm89_generated_kernel_{stable_hash}" in source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{stable_hash}" in source
    ...
    assert f"__device__ void kg_cuda_sm89_device_body_{stable_hash}" in source
    assert f"__global__ void kg_cuda_sm89_generated_kernel_{stable_hash}" in source
    assert f"cuda_sm89::launch<1, 256, 1, 49152, kg_cuda_sm89_generated_kernel_{stable_hash}>" in source
    assert "if (threadIdx.x == 0)" in source
    assert "cuda_sm89::ArgSlot* kg_cuda_sm89_device_slots = nullptr" in source
    assert "cuda_sm89::ArgSlot* kg_cuda_sm89_host_device_slots = nullptr" in source
    assert "long long** kg_cuda_sm89_device_shapes = nullptr" in source
    assert "long long** kg_cuda_sm89_device_strides = nullptr" in source
    assert "void** kg_cuda_sm89_device_data = nullptr" in source
    assert "cuda_sm89::detail::copy_host_to_device<long long>" in source
    assert "cuda_sm89::detail::copy_host_to_device<float>" in source
    assert "cuda_sm89::detail::copy_host_to_device<cuda_sm89::ArgSlot>" in source
    assert "cuda_sm89::detail::copy_device_to_host<float>" in source
    assert "kg_cuda_sm89_host_device_slots[kg_cuda_sm89_slot_index].shape = kg_cuda_sm89_device_shapes" in source
    assert "kg_cuda_sm89_host_device_slots[kg_cuda_sm89_slot_index].data = kg_cuda_sm89_device_data" in source
    assert "host_ctx, kg_cuda_sm89_device_slots, count, kg_cuda_sm89_device_status" in source
    assert "host_ctx, slots, count, kg_cuda_sm89_device_status" not in source
    assert "int* kg_cuda_sm89_device_status = nullptr" in source
    assert "cudaMemcpy(&kg_cuda_sm89_host_status, kg_cuda_sm89_device_status, sizeof(int), cudaMemcpyDeviceToHost)" in source
    assert "mma.sync.aligned.m16n8k8" in source
    assert "tensor_core_probe" not in source
    assert "acc += lhs" not in source
    assert "cuda_sm89::detail::memory_from_slot" in source
    assert "cuda_sm89::load<" in source
    assert "cuda_sm89::deslice<MemorySpace::GM" in source
    assert "cuda_sm89::matmul<" in source
    assert "(void)cuda_sm89::" not in source
    assert "delete[] kg_v_" in source
    assert 'extern "C" int kg_execute_entry(cuda_sm89::ArgSlot* slots, unsigned long long count)' in source
    assert f"int kg_cuda_sm89_execute_{stable_hash}_ir(cuda_sm89::ArgSlot* slots, unsigned long long count)" in source
    assert f"return kg_cuda_sm89_execute_{stable_hash}_ir(slots, count);" in source
    fixed_matmul_entry = "kg_cuda_sm89_execute_" + "matmul_ir"
    assert f"return {fixed_matmul_entry}(slots, count);" not in source
    assert f"int {fixed_matmul_entry}(" not in source
    assert "kg_cuda_sm89_ir_record_words" in _extract_cuda_sm89_device_body(source)
    for token in OLD_CUDA_SM89_SOURCE_TOKENS:
        assert token not in source
    include_text = Path("include/cuda_sm89/cuda_sm89.cuh").read_text(encoding="utf-8")
    core_text = Path("include/cuda_sm89/Core.h").read_text(encoding="utf-8")
    memory_text = Path("include/cuda_sm89/Memory.h").read_text(encoding="utf-8")
    dma_text = Path("include/cuda_sm89/Dma.h").read_text(encoding="utf-8")
    kernel_text = Path("include/cuda_sm89/Kernel.h").read_text(encoding="utf-8")
    arch_text = Path("include/cuda_sm89/Arch.h").read_text(encoding="utf-8")
    source_builder_text = Path("kernel_gen/dsl/gen_kernel/emit/cuda_sm89/source_bundle.py").read_text(encoding="utf-8")
    core_api_block = core_text.split("API 列表:", 1)[1].split("helper 清单:", 1)[0]
    dma_api_block = dma_text.split("API 列表:", 1)[1].split("helper 清单:", 1)[0]
    arch_api_block = arch_text.split("API 列表:", 1)[1].split("helper 清单:", 1)[0]
    backend_api_blocks = "\n".join([core_api_block, dma_api_block, arch_api_block])
    spec_text = Path("spec/include/cuda_sm89/cuda_sm89.md").read_text(encoding="utf-8")
    ...
    assert '#include "include/cuda_sm89/Arch.h"' in include_text
    assert "template <typename T>" not in include_text
    assert "`struct cuda_sm89::ArgSlot`" in core_api_block
    assert "cuda_sm89::detail" not in backend_api_blocks
    assert "`struct cuda_sm89::ArgSlot`" in spec_api_block
    assert "cuda_sm89::detail" not in spec_api_block
    dmar_ring_ctor_signature = "`template <MemorySpace Space, typename SlotT, typename BackingT> __device__ cuda_sm89::DmaRing<Space, SlotT, BackingT>::DmaRing(Memory<Space, BackingT>& backing, S_INT num, S_INT offset_bytes, const Vector& shape, const Vector& stride, MemoryFormat format)`"
    dmar_ring_current_signature = "`template <MemorySpace Space, typename SlotT, typename BackingT> __device__ Memory<Space, SlotT> cuda_sm89::DmaRing<Space, SlotT, BackingT>::current() const`"
    dmar_ring_advance_signature = "`template <MemorySpace Space, typename SlotT, typename BackingT> __device__ Memory<Space, SlotT> cuda_sm89::DmaRing<Space, SlotT, BackingT>::advance()`"
    dmar_ring_detail_block = spec_text.split("### `cuda_sm89::DmaRing<Space, SlotT, BackingT>`", 1)[1].split(
        "## 后端实现层边界",
        1,
    )[0]
    for signature in (dmar_ring_ctor_signature, dmar_ring_current_signature, dmar_ring_advance_signature):
        assert signature in dma_api_block
    ...
    assert "is_f32_memory" in memory_text
    assert "const unsigned long long requested_rank = size.size() == 0 ? 1 : size.size();" in memory_text
    assert "const unsigned long long view_rank = requested_rank < kMaxDim ? requested_rank : kMaxDim;" in memory_text
    assert "const long long source_stride = axis < rank_ ? stride_[axis] : 1;" in memory_text
    assert "const long long axis_stride = axis < stride.size() ? stride[axis] : 1;" in memory_text
    assert "stride_buf[axis] = source_stride * axis_stride;" in memory_text
    assert "if (offset.size() == 1 && shape.size() > 1)" in memory_text
    assert "linear_offset = offset[0];" in memory_text
    assert 'if alias_kind == "reinterpret":' in source_builder_text
    assert "cuda_sm89::detail::fragment_alias<{result_space_cpp}, float>" in source_builder_text
    assert "detail::copy_from_source_window(target, source, offset, size, stride)" in dma_text
    assert "detail::copy_to_target_window(target, source, offset, size, stride)" in dma_text
    assert "return detail::transpose_memory(target, source, perm);" in dma_text
    assert "const long long source_axis_value = perm[target_axis];" in dma_text
    assert "source_indices[static_cast<unsigned long long>(perm[target_axis])] = target_indices[target_axis];" in dma_text
    assert "target.at(target_indices) = source.at(source_indices)" in dma_text
    assert "detail::matmul_memory(out, lhs, rhs, acc)" in kernel_text
    assert 'asm volatile(\n      "mma.sync.aligned.m16n8k8.row.col.f32.tf32.tf32.f32 "' in kernel_text
    assert "(void)tensor_core_matmul_path(out, lhs, rhs, acc)" not in kernel_text
    assert "kg_cuda_sm89_mma_observable" not in kernel_text
    assert "const bool kg_cuda_sm89_tensor_core_used = !acc && tensor_core_matmul_path(out, lhs, rhs, acc);" in kernel_text
    assert "out.at(out_indices) = mma_d0" in kernel_text
    assert "out.at(out_indices) = mma_d1" in kernel_text
    assert "out.at(out_indices) = mma_d2" in kernel_text
    assert "out.at(out_indices) = mma_d3" in kernel_text
    assert "const bool kg_cuda_sm89_mma_prefix" in kernel_text
    assert "const float kg_cuda_sm89_mma_seed = static_cast<float>(out.at(out_indices));" in kernel_text
    assert "sum += kg_cuda_sm89_mma_seed - kg_cuda_sm89_mma_seed;" in kernel_text
    assert "const unsigned long long k_start = kg_cuda_sm89_mma_prefix" not in kernel_text
    assert "if (kg_cuda_sm89_mma_prefix && depth <= kCudaSm89MmaK)" not in kernel_text
    assert "tensor_core_seeded_output" not in kernel_text
    assert "linear == 0" not in kernel_text
    assert "mma_tail" not in kernel_text
    assert "out.at(out_indices) = base + mma_d0" not in kernel_text
    assert "out.at(out_indices) = sum" in kernel_text
    assert "detail::img2col2d_memory(out, input, kh, kw, sh, sw, dh, dw, ph, pw, pl, pr)" in kernel_text
    assert "(out.rank() != 2 && out.rank() != 6)" in kernel_text
    assert "for (unsigned long long index = 0; index < count; ++index)" in kernel_text
    assert "static_cast<unsigned long long>(threadIdx.x); index < count; index += blockDim.x" not in kernel_text
    assert "(void)out;\n  (void)lhs;\n  (void)rhs;\n  (void)acc;\n  return kOk;" not in kernel_text
    assert "(void)target;\n  (void)source;\n  (void)offset;\n  (void)size;\n  (void)stride;\n  return kOk;" not in dma_text
    assert "include/npu_demo" not in source
    assert "npu_demo::" not in source
    assert "get_dynamic_memory<TLM" not in source


def test_cuda_sm89_emit_dma_ring_byte_pool_source_shape() -> None:
    """验证 CUDA ring lowering 使用 i8 backing。

    功能说明:
    - 通过公开 `emit_c(...)` 构造含 `DmaMakeRingOp/DmaCurrentRingOp/DmaAdvanceRingOp` 的 final IR。
    - 锁定 i8 byte-pool backing 不再被发射为 `float` alloc，并保留 ring offset / shape / stride source shape。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k dma_ring_byte_pool_source_shape
    """
    ...
    assert "cuda_sm89::alloc<MemorySpace::TLM1, unsigned char>(ctx, Vector{64}, Vector{1})" in body
    assert "cuda_sm89::make_ring<float, MemorySpace::TLM1, unsigned char>" in body
    assert ".current();" in body
    assert ".advance();" in body
    assert "cuda_sm89::alloc<MemorySpace::TLM1, float>(ctx, Vector{64}, Vector{1})" not in body

    dma_text = Path("include/cuda_sm89/Dma.h").read_text(encoding="utf-8")
    assert "normalized_cursor * offset_bytes_" in dma_text
    assert "shape_size_" in dma_text
    assert "stride_[axis]" in dma_text


def test_cuda_sm89_emit_selects_different_sources_from_lowered_entry() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k different_sources
    """
    ...
    matmul_hash = _extract_cuda_sm89_ir_hash(matmul_source)
    conv2d_hash = _extract_cuda_sm89_ir_hash(conv2d_source)
    flash_hash = _extract_cuda_sm89_ir_hash(flash_source)
    assert f"// kg.cuda.ir.entry_symbol: kg_cuda_sm89_execute_{matmul_hash}_ir" in matmul_source
    assert f"// kg.cuda.ir.entry_symbol: kg_cuda_sm89_execute_{conv2d_hash}_ir" in conv2d_source
    assert f"// kg.cuda.ir.entry_symbol: kg_cuda_sm89_execute_{flash_hash}_ir" in flash_source
    assert f"// kg.cuda.ir.generated_kernel_symbol: kg_cuda_sm89_generated_kernel_{matmul_hash}" in matmul_source
    assert f"// kg.cuda.ir.generated_kernel_symbol: kg_cuda_sm89_generated_kernel_{conv2d_hash}" in conv2d_source
    assert f"// kg.cuda.ir.generated_kernel_symbol: kg_cuda_sm89_generated_kernel_{flash_hash}" in flash_source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{matmul_hash}" in matmul_source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{conv2d_hash}" in conv2d_source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{flash_hash}" in flash_source
    ...
    assert "cuda_sm89::matmul<" in matmul_source
    assert "cuda_sm89::img2col2d<" in conv2d_source
    assert "cuda_sm89::reduce_" in flash_source
    assert "cuda_sm89::exp<" in flash_source
    assert "cuda_sm89::transpose<MemorySpace::TSM, MemorySpace::TSM, float, float>(ctx" in conv2d_source
    assert "Vector{1, 0, 2, 3}" in conv2d_source
    assert "Vector{1, 0, 1, 1}" not in conv2d_source
    assert "cuda_sm89::transpose<MemorySpace::TSM, MemorySpace::TSM, float, float>(ctx" in flash_source
    assert "cuda_sm89::detail::fragment_alias<MemorySpace::GM, float>" in flash_source
    assert "Vector{1, 0}" in flash_source
    assert "cuda_sm89::transpose<MemorySpace::TSM, MemorySpace::TSM, float, float>(ctx, kg_v_199_0, kg_v_198_0, Vector{1, 1})" not in flash_source
    assert "-INFINITY" in flash_source
    assert "float kg_v_155_0 = 0.0f;" not in flash_source
    assert f"__global__ void kg_cuda_sm89_generated_kernel_{matmul_hash}" in matmul_source
    assert f"__global__ void kg_cuda_sm89_generated_kernel_{conv2d_hash}" in conv2d_source
    assert f"__global__ void kg_cuda_sm89_generated_kernel_{flash_hash}" in flash_source
...
def test_cuda_sm89_ir_hash_is_stable_and_op_sequence_specific() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k cuda_sm89_ir_hash
    """
    ...
    first_hash = _extract_cuda_sm89_ir_hash(first_matmul_source)
    second_hash = _extract_cuda_sm89_ir_hash(second_matmul_source)
    conv2d_hash = _extract_cuda_sm89_ir_hash(conv2d_source)
...
def test_cuda_sm89_executable_trace_changes_with_same_entry_final_ir_op_sequence() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k same_entry_final_ir_op_sequence
    """
    ...
    base_body = _extract_cuda_sm89_device_body(base_source)
    sequence_body = _extract_cuda_sm89_device_body(sequence_source)

    base_hash = _extract_cuda_sm89_ir_hash(base_source)
    sequence_hash = _extract_cuda_sm89_ir_hash(sequence_source)
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{base_hash}" in base_source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{sequence_hash}" in sequence_source
    assert base_hash != sequence_hash
    assert base_body != sequence_body
    assert "cuda_sm89::matmul<" in base_body
    assert "cuda_sm89::matmul<" in sequence_body
    assert "cuda_sm89::load<" in base_body
    assert "cuda_sm89::load<" in sequence_body
    assert "cuda_sm89::store<" in base_body
    assert "cuda_sm89::store<" in sequence_body
    assert sequence_body.count("cuda_sm89::load<") == base_body.count("cuda_sm89::load<") + 1


def test_cuda_sm89_executable_trace_changes_with_same_type_dataflow() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k same_type_dataflow
    """
    ...
    base_body = _extract_cuda_sm89_device_body(base_source)
    swapped_body = _extract_cuda_sm89_device_body(swapped_source)
    base_loads = [line for line in base_body.splitlines() if "cuda_sm89::load<MemorySpace::TLM1, MemorySpace::GM" in line]
    swapped_loads = [line for line in swapped_body.splitlines() if "cuda_sm89::load<MemorySpace::TLM1, MemorySpace::GM" in line]

    base_hash = _extract_cuda_sm89_ir_hash(base_source)
    swapped_hash = _extract_cuda_sm89_ir_hash(swapped_source)
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{base_hash}" in base_source
    assert f"// kg.cuda.ir.device_body_symbol: kg_cuda_sm89_device_body_{swapped_hash}" in swapped_source
    ...
    assert "cuda_sm89::matmul<" in base_body
    assert "cuda_sm89::matmul<" in swapped_body
    assert base_hash != swapped_hash
    assert base_body != swapped_body
    assert base_source != swapped_source


def test_cuda_sm89_matmul_materialization_rejects_non_all_tlm1() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k non_all_tlm1
    """
...
def test_cuda_sm89_matmul_materialization_rejects_missing_writeback() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k missing_writeback
    """
...
def test_cuda_sm89_emit_rejects_name_only_or_unknown_module(name: str) -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k name_only
    """

    set_target("cuda_sm89")
    with pytest.raises(KernelCodeError, match="unsupported cuda_sm89 final IR op: <none>"):
        emit_c(_make_module(name), EmitCContext())


def test_cuda_sm89_emit_rejects_printed_string_tokens_without_kernel_ops() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k printed_string_tokens
    """
...
def test_cuda_sm89_gen_kernel_dump_writes_bundle_artifacts(tmp_path: Path) -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k dump_writes
    """
    ...
    header_source = (tmp_path / "include" / "cuda_sm89" / "generated_entry.cuh").read_text(encoding="utf-8")
    assert "cuda_sm89 generated from final IR" in kernel_source
    ...
    assert "cuda_sm89::ArgSlot" in header_source


def test_cuda_sm89_emit_package_structure_matches_plan() -> None:
    """验证 CUDA SM89 emit package 结构符合计划。

    功能说明:
    - 只读取文件和 AST，不 import / direct call `cuda_sm89` package 内部 helper。
    - 锁定 root 聚合、唯一 `module.py` handler、依赖方向和空 `__all__`。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_emit.py -k package_structure
    """

    root = Path("kernel_gen/dsl/gen_kernel/emit/cuda_sm89")
    expected_files = {
        "__init__.py",
        "constants.py",
        "include.py",
    ...
    assert Path("include/cuda_sm89/Arch.h").is_file()
    ...
    assert emit_files == [
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/binary_elewise.py",
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/exp.py",
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/img2col2d.py",
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/matmul.py",
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/reduce.py",
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/module.py",
    ]
    assert include_files == ["kernel_gen/dsl/gen_kernel/emit/cuda_sm89/include.py"]
    allowed_imports = {
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/__init__.py": {".include", ".kernel", ".module"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/constants.py": set(),
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/module.py": {".constants", ".source_bundle"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/source_bundle.py": {".constants", ".runtime"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/include.py": {".constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/runtime.py": {".constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/__init__.py": {".binary_elewise", ".exp", ".img2col2d", ".matmul", ".reduce"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/binary_elewise.py": {"..constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/exp.py": {"..constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/img2col2d.py": {"..constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/matmul.py": {"..constants"},
        "kernel_gen/dsl/gen_kernel/emit/cuda_sm89/kernel/reduce.py": {"..constants"},
    }
    package_root = ".".join(("kernel_gen", "dsl", "gen_kernel", "emit", "cuda_sm89"))
