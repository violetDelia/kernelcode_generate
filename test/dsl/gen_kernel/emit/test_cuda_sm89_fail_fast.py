"""CUDA SM89 fail-fast boundary tests.
...
使用示例:
- pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_fail_fast.py
...
def test_cuda_sm89_emit_fails_fast_for_name_only_modules() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_fail_fast.py -k name_only
    """
...
def test_cuda_sm89_emit_fails_fast_for_spoofed_string_tokens() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_fail_fast.py -k spoofed
    """
...
def test_cuda_sm89_implementation_does_not_probe_or_switch_sm() -> None:
    """验证实现侧不做运行时 SM 探测或 target 切换。

    功能说明:
    - 只读取 CUDA SM89 implementation 相关文件，不读取任务记录或计划书文字。
    - 锁定 generated source、include 和 compile strategy 中没有设备属性探测或其它 SM flag。

    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_fail_fast.py -k probe_or_switch
    """

    implementation_paths = (
        Path("kernel_gen/dsl/gen_kernel/emit/cuda_sm89/source_bundle.py"),
        Path("kernel_gen/dsl/gen_kernel/emit/cuda_sm89/runtime.py"),
        Path("kernel_gen/execute_engine/builtin_strategy/cuda_sm89.py"),
        Path("include/cuda_sm89/Arch.h"),
        Path("include/cuda_sm89/cuda_sm89.cuh"),
    )
    ...
    assert "-arch=sm_89" in combined
    assert "-arch=sm_" + "86" not in combined
...
def test_cuda_sm89_runtime_gate_requires_sm89_before_runtime_execution() -> None:
    ...
    使用示例:
    - pytest -q test/dsl/gen_kernel/emit/test_cuda_sm89_fail_fast.py -k runtime_gate
    """

    runtime_test_source = Path("test/cuda/test_cuda_sm89_kernel_demos_runtime.py").read_text(encoding="utf-8")
