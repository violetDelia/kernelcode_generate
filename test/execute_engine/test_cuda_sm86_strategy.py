"""CUDA SM86 execute_engine strategy tests.


功能说明:
- 覆盖 `ExecutionEngine(target="cuda_sm86")` 的 compile strategy 注册、SourceBundle 写盘和 nvcc 命令合同。
...
def test_cuda_sm86_compile_strategy_registered() -> None:
    """验证 cuda_sm86 compile strategy 已注册。
    ...
    使用示例:
    - pytest -q test/execute_engine/test_cuda_sm86_strategy.py -k registered
    """
    ...
    with pytest.raises(KernelCodeError) as exc:
        ExecutionEngine(target="cuda_sm86", compiler="/not/found/nvcc").compile(source=source, function="cuda_matmul")

    assert exc.value.failure_phrase == "compile_failed"
    assert "compiler not found" in str(exc.value)


def test_cuda_sm86_compile_writes_source_bundle_and_nvcc_command(tmp_path: Path) -> None:
    """验证 CUDA SourceBundle 写盘与 nvcc 命令。

    功能说明:
    - 使用公开 `ExecutionEngine.compile(...)` 编译 SourceBundle aggregate。
    - fake nvcc 证明主 `.cu` 被选择，默认 `-arch=sm_86 -shared -Xcompiler -fPIC` 被追加。

    使用示例:
    - pytest -q test/execute_engine/test_cuda_sm86_strategy.py -k source_bundle
    """
    ...
    kernel = ExecutionEngine(target="cuda_sm86", compiler=str(fake_nvcc)).compile(source=source, function="cuda_matmul")
    try:
        ...
        assert (source_root / "include" / "cuda_sm86" / "generated_entry.cuh").is_file()
        ...
        assert "cuda-sm86 compile" in kernel.compile_stdout
    finally:
        kernel.close()


def test_cuda_sm86_compile_rejects_target_header_mismatch(tmp_path: Path) -> None:
    ...
    使用示例:
    - pytest -q test/execute_engine/test_cuda_sm86_strategy.py -k header_mismatch
    """
    ...
    with pytest.raises(KernelCodeError) as exc:
        ExecutionEngine(target="cuda_sm86", compiler=str(fake_nvcc)).compile(source=source, function="bad")

    assert exc.value.failure_phrase == "target_header_mismatch"


def test_cuda_sm86_execute_rejects_non_none_stream(tmp_path: Path) -> None:
    ...
    使用示例:
    - pytest -q test/execute_engine/test_cuda_sm86_strategy.py -k non_none_stream
    """

    soname = tmp_path / "libkernel.so"
    soname.write_text("", encoding="utf-8")
    kernel = CompiledKernel(target="cuda_sm86", soname_path=str(soname), function="cuda_matmul", entry_point="kg_execute_entry")

    with pytest.raises(KernelCodeError) as exc:
        kernel.execute(args=(), stream="stream-token")  # type: ignore[arg-type]

    assert exc.value.failure_phrase == "stream_not_supported"
    assert "cuda_sm86 does not support non-None stream" in str(exc.value)
