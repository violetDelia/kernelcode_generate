"""CUDA SM89 target kernel demo runtime integration tests.

功能说明:
- 在具备 CUDA toolkit 与 GPU 的环境中，通过 9 个现有 kernel demo 的公开 kernel 函数生成 IR。
- 每个 case 都执行 `cuda-sm89-lowering -> emit_c(target="cuda_sm89") -> ExecutionEngine(target="cuda_sm89") -> execute`。
- 测试只验证本计划确认的 matmul / conv2d / flash_attention demo 范围，不扩大到任意 DSL kernel。
- 当前任务正式 runtime 验收现场为 `nvcc + SM89 CUDA device`；`cuda_sm89` 仍是已确认 target/API 名。
...
def _compile_cuda_demo_kernel(case: KernelDemoCase) -> CompiledKernel:
    """从现有 demo 公开入口编译 CUDA SM89 kernel。

    功能说明:
    - 临时设置 demo 函数 annotations，让 runtime gate 使用小规模、可快速验证的合法 profile。
    - 通过公开 `mlir_gen(...)` 读取并调用现有 demo kernel 函数，随后执行公开 `cuda-sm89-lowering` pipeline。
    - 通过公开 `emit_c(...)` 与 `ExecutionEngine(target="cuda_sm89")` 完成 SourceBundle 编译。
    ...
    try:
        ...
        return ExecutionEngine(target="cuda_sm89").compile(source=source, function=case.kernel_fn.__name__)
    finally:
...
def _emit_cuda_demo_source(case: KernelDemoCase) -> str:
    """从现有 demo 公开入口生成 CUDA SM89 SourceBundle。

    功能说明:
    - 经公开 `mlir_gen(...)`、`cuda-sm89-lowering` 和 `emit_c(...)` 生成 CUDA source。
...
def _extract_cuda_sm89_ir_hash(source: str) -> str:
    """读取 CUDA SM89 SourceBundle 的 final IR hash。
...
def test_cuda_sm89_runtime_preflight_accepts_sm89(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k preflight_accepts_sm89
    """
...
def test_cuda_sm89_runtime_preflight_rejects_non_sm89(monkeypatch: pytest.MonkeyPatch) -> None:
    """验证 runtime preflight 不把非 SM89 现场记作通过。

    功能说明:
    - 用公开 `pytest` monkeypatch 模拟 `compute_cap=8.6`。
    - 锁定非 SM89 CUDA device 在进入 `kg_execute_entry` / `cuda_sm89::launch` 前 skip。

    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k preflight_rejects_non_sm89
    """
...
def test_cuda_sm89_demo_sources_are_lowered_ir_specific() -> None:
    ...
    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k sources_are_lowered
    """
    ...
    assert "__global__ void kg_cuda_sm89_generated_kernel_" in matmul_source
    assert "__global__ void kg_cuda_sm89_generated_kernel_" in conv2d_source
    assert "__global__ void kg_cuda_sm89_generated_kernel_" in flash_source
    assert "__device__ void kg_cuda_sm89_device_body_" in matmul_source
    assert "__device__ void kg_cuda_sm89_device_body_" in conv2d_source
    assert "__device__ void kg_cuda_sm89_device_body_" in flash_source
...
def test_cuda_sm89_matmul_demo_runtime_cases(case: KernelDemoCase) -> None:
    ...
    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k matmul
    """
...
def test_cuda_sm89_conv2d_demo_runtime_cases(case: KernelDemoCase) -> None:
    ...
    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k conv2d
    """
...
def test_cuda_sm89_flash_attention_demo_runtime_cases(case: KernelDemoCase) -> None:
    ...
    使用示例:
    - pytest -q test/cuda/test_cuda_sm89_kernel_demos_runtime.py -m cuda -k flash_attention
    """
